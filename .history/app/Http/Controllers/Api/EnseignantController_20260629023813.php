...
class EnseignantController extends Controller
{
    ...
    public function completeProfile(Request $request){
        ...
        // AJOUT : check de sécurité, évite un crash 500 si jamais
        // l'enseignant n'existe pas encore en base à ce stade.
        if (!$enseignant) {
            return response()->json([
    ...
    public function update(Request $request){
        // ===========================================================
        // FIX PRINCIPAL : on récupère l'enseignant AVANT de valider,
        // et on vérifie qu'il existe réellement. Sans ce check, un
        // ->update() sur null fait planter Laravel avec une erreur 500
        // "brute" (sans JSON propre) → le frontend affiche alors le
        ...
        // FIX : ajout de "sometimes" pour ne valider un champ QUE s'il
        // est réellement envoyé dans la requête. Sans ça, si un champ
        // arrive vide ou absent (ex: pendant un chargement partiel côté
        // React), la validation peut échouer avec un 422 inattendu.
        $request->validate([
        ...
        // AJOUT : check de sécurité supplémentaire
        if (!$user) {
            return response()->json([
        ...
        // FIX : on recharge la relation "user" fraîche sur l'enseignant
        // avant de la renvoyer, pour que le JSON retourné au frontend
        // (profilRes.data.user dans Profile.jsx) soit bien à jour.
