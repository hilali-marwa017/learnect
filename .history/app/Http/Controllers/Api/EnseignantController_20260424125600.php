...
class EnseignantController extends Controller
{
    ...
    public function uplpoadPhoto(Request $request){
        $request->validate([
            'photo'=>'nullable|required|image|mimes:mimes:jpg,jpeg,png|max:2048'
        ]);
        $enseignant = Enseignant::where('utilisateur_id', Auth::id())->first();
        if($user->photo){
        ...
        $filename = 'enseignant_' . Auth::id() . '_' . time() . '.' . $file->getClientOriginalExtension();//recupere l'extension d'image (jpg,png...) || enseignant_5_1700000000.jpg
        $path = $file->storeAs('enseignants/photos',$filename,'public');
