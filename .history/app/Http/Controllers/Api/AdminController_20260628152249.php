...
class AdminController extends Controller
{
    ...
    public function enseignantsEnAttente(){
        // correction : on exclut seulement les refuses, plutot que d'exiger
        // statut == 'en_attente' strictement (qui excluait mriwa par erreur)
    ...
    // correction : lookup explicite par utilisateur_id au lieu du binding
    // implicite, pour eviter tout 404 silencieux et donner un message clair
    public function validerEnseignant($id){
        $enseignant = Enseignant::where('utilisateur_id', $id)->first();

        if (!$enseignant) {
            return response()->json([
                'message' => "Aucun dossier enseignant trouvé pour l'utilisateur #{$id}. Vérifiez que la ligne existe bien dans la table enseignants."
            ], 404);
        }
    ...
    // correction : meme lookup explicite que validerEnseignant
