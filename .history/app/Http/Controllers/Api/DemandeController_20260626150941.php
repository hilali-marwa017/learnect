...
{
    // liste de toutes les demandes actives (publiques)
    public function index(){
        $demandes = Demande::with('etudiant')->where('statut','active')>where('expire_at', '>', Carbon::now())
            ->get();
        return response()->json($demandes);
    }

    // mes demandes (etudiant connecte)
    public function mesDemandes(){
        // charge les offres recues pour chaque demande
        $demandes = Demande::with('offres.enseignant.user')
