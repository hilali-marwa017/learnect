...
class EnseignantController extends Controller
{
    ...
    public function index(Request $request){
        ...
        // ne pas afficher les enseignants bloques
        $query->whereHas('user', function($q){
            $q->where('statut','actif');
        });

        if ($request->filled('ville')){
            $query->join('users','enseignants.utilisateur_id','=','users.utilisateur_id')->where('users.ville', 'like', '%' . $request->ville . '%')->select('enseignants.*');
        }
    ...
    // completer profil enseignant (inscription)
    public function completeProfile(Request $request){
        $request->validate([
            'titre'=>'required|string|min:3', // ✅ CORRIGE : min:10 → min:3
            'description_cours'=>'required|string|min:10', // ✅ CORRIGE : min:30 → min:10
            'description_profil'=>'required|string|min:10', // ✅ CORRIGE : min:30 → min:10
    ...
    public function update(Request $request){
        $request->validate([
            'titre'=>'string|min:3', // ✅ CORRIGE : min:10 → min:3
            'description_cours'=>'string|min:10', // ✅ CORRIGE : min:30 → min:10
            'description_profil'=>'string|min:10', // ✅ CORRIGE : min:30 → min:10
        ...
        // ✅ NOUVEAU : mettre a jour l'utilisateur (nom, prenom, telephone, ville)
    ...
    public function uploadPhoto(Request $request){
        ...
        // ✅ NOUVEAU : retourner l'utilisateur mis a jour
        return response()->json([
