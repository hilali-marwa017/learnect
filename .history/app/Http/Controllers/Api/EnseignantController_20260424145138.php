...
class EnseignantController extends Controller
{
    ...
    public function uplpoadPhoto(Request $request){
        ...
        public function dashboard(){$enseignant = Enseignant::with('creneaux', 'offres', 'avis')    ->where('utilisateur_id', Auth::id())    ->first()if (!$enseignant) {    return response()->json([        'message' => 'Enseignant non trouvé'    ], 404);$stats = [    'total_avis' => $enseignant->avis()->count(),    'note_moyenne' => $enseignant->noteMoyenne,    'creneaux_dispos' => $enseignant->creneaux()->where('estDisponible', true)->count(),    'offres_en_attente' => $enseignant->offres()->where('statut', 'en_attente')->count(),]return response()->json([    'enseignant' => $enseignant,    'stats' => $stats,]);
