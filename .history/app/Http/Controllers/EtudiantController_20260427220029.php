...
class EtudiantController extends Controller
{
    ...
    public function uploadPhoto(Request $request){
        ...
        if($request->hasFile('photo')){
            ...
            $filenam = 'photo_' .Auth::id() . '_' .time() . '_' . $file->getClientOriginalExtension();
    ...
    public function dashboard(){
        $etudiant = Etudiant::with('reservations', 'demandes', 'avis')->where('utilisateur_id', Auth::id())
                            ->first();

        // Securite : si etudiant pas trouve → erreur 404
        if (!$etudiant) {
            return response()->json([
                'message' => 'Etudiant introuvable !'
            ], 404);
        ...
        // count() = compte le nombre d'elements
        // sans () car deja charge avec with() !
        $stats = [
            // ex: Marwa a reserve 3 cours → total_reservations = 3
            'total_reservations' => $etudiant->reservations->count(),

            // ex: Marwa a publie 2 demandes → total_demandes = 2
            'total_demandes'     => $etudiant->demandes->count(),

            // ex: Marwa a laisse 1 avis → total_avis = 1
            'total_avis'         => $etudiant->avis->count(),
        ];

        // React affiche le dashboard :
        // "Bonjour Marwa 👋"
        // 3 reservations | 2 demandes | 1 avis
        return response()->json([
            'etudiant' => $etudiant,
            'stats'    => $stats,
        ]);
