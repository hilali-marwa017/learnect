...
export default function Navbar({ isDark, onToggleTheme, user, onLogout }) {
  ...
  // Récupérer les initiales (première lettre du prénom et du nom)
  const getInitials = () => {
    if (!user) return '?';
    ...
    <nav style={{ background: isDark ? '#0a0a0c' : '#ffffff', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)'}`, padding: '0 2rem', height: '70px', position: 'sticky', top: 0, zIndex: 100, display: 'flex', alignItems: 'center' }}>
            ...
            <>
              ...
              <div style={{ position: 'relative' }}>
                <button onClick={() => setIsProfileOpen(!isProfileOpen)} style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'none', border: 'none', cursor: 'pointer', padding: '4px 8px', borderRadius: '8px' }}>
                  {/* Affichage des initiales au lieu de l'icône User */}
                  ...
                  <span style={{ color: isDark ? '#ffffff' : '#07090d', fontSize: '0.9rem' }}>{user?.prenom || user?.name || 'Compte'}</span>
                ...
                {isProfileOpen && (
                  <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: '8px', background: isDark ? '#0a0a0c' : '#ffffff', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)'}`, borderRadius: '12px', padding: '0.5rem', minWidth: '200px', zIndex: 100, boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
                    <button onClick={() => { handleProfileClick(); setIsProfileOpen(false); }} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 12px', width: '100%', background: 'none', border: 'none', cursor: 'pointer', color: isDark ? '#ffffff' : '#07090d', textAlign: 'left', borderRadius: '8px' }}>
                      Mon profil
                    </button>
                    <button onClick={() => { handleDashboardClick(); setIsProfileOpen(false); }} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 12px', width: '100%', background: 'none', border: 'none', cursor: 'pointer', color: isDark ? '#ffffff' : '#07090d', textAlign: 'left', borderRadius: '8px' }}>
                      Tableau de bord
                    </button>
                    <div style={{ height: '1px', background: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)', margin: '8px 0' }} />
                    <button onClick={onLogout} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 12px', width: '100%', background: 'none', border: 'none', cursor: 'pointer', color: '#dc2626', textAlign: 'left', borderRadius: '8px' }}>
      ...
      {isOpen && (
        <div className="mobile-menu" style={{ position: 'fixed', top: '70px', left: 0, right: 0, background: isDark ? '#0a0a0c' : '#ffffff', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)'}`, padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem', zIndex: 99 }}>
          ...
          {isLoggedIn ? (
            <>
              <button onClick={() => { handleProfileClick(); setIsOpen(false); }} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px', cursor: 'pointer', color: isDark ? '#ffffff' : '#07090d', textAlign: 'left', background: 'none', border: 'none' }}>Mon profil</button>
              <button onClick={() => { handleDashboardClick(); setIsOpen(false); }} style={{ padding: '10px', cursor: 'pointer', color: isDark ? '#ffffff' : '#07090d', textAlign: 'left', background: 'none', border: 'none' }}>Tableau de bord</button>
