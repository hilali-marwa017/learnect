...
export default function Navbar({ theme, onToggleTheme }) {
  const { user, isAuthenticated, isAdmin, isEnseignant, isEtudiant, logout } = useAuth()
    ...
    <nav className="sticky top-0 z-50 bg-canvas/80 backdrop-blur-xl border-b border-hairline">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-8">
            <a href="/#tutors-section" className="text-sm text-charcoal hover:text-ink transition-colors">Professeurs</a>
            <a href="/#how-it-works" className="text-sm text-charcoal hover:text-ink transition-colors">Comment ça marche</a>
            <a href="/#faq-section" className="text-sm text-charcoal hover:text-ink transition-colors">FAQ</a>
            ...
            {isAuthenticated ? (
                  ...
                  {profileOpen && (
                    <div className="absolute right-0 mt-2 w-56 bg-surface-card border border-hairline-strong rounded-xl shadow-xl py-2 animate-fade-in">
                      <div className="px-4 py-3 border-b border-hairline">
                        ...
                        <span className="inline-block mt-1 px-2 py-0.5 bg-accent-blue/10 text-accent-blue text-[10px] font-bold uppercase rounded">{user?.role}</span>
                      </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <Link to="/login" className="text-sm font-medium text-charcoal hover:text-ink transition-colors">Connexion</Link>
                <Link to="/register" className="text-sm font-bold bg-ink text-canvas px-4 py-2 rounded-lg hover:bg-ash transition-colors">S'inscrire</Link>
              </div>
            )}
          </div>
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-hairline bg-canvas px-6 py-4 space-y-3 animate-slide-up">
          <a href="/#tutors-section" onClick={() => setMobileOpen(false)} className="block text-sm text-charcoal py-2">Professeurs</a>
          <a href="/#how-it-works" onClick={() => setMobileOpen(false)} className="block text-sm text-charcoal py-2">Comment ça marche</a>
          <a href="/#faq-section" onClick={() => setMobileOpen(false)} className="block text-sm text-charcoal py-2">FAQ</a>
          {isAuthenticated ? (
            <>
              <Link to={getDashboardLink()} onClick={() => setMobileOpen(false)} className="block text-sm text-charcoal py-2">Tableau de bord</Link>
              {isAdmin && <Link to="/admin/dashboard" onClick={() => setMobileOpen(false)} className="block text-sm text-accent-blue py-2">Admin</Link>}
              <button onClick={handleLogout} className="block text-sm text-accent-red py-2">Déconnexion</button>
            </>
          ) : (
            <>
              <Link to="/login" onClick={() => setMobileOpen(false)} className="block text-sm text-charcoal py-2">Connexion</Link>
              <Link to="/register" onClick={() => setMobileOpen(false)} className="block text-sm font-bold text-accent-orange py-2">S'inscrire</Link>
            </>
          )}
        </div>
      )}
