...
export default function Footer({ isDark }) {
  ...
  const textMuted = isDark ? '#6b7280' : 'rgba(255,255,255,0.45)';
  const border = 'rgba(255,255,255,0.07)';
  const orange = '#e04f00';

  return (
    <footer style={{ background: bg, padding: '3.5rem 2rem 1.5rem' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: '2rem', marginBottom: '2.5rem' }}>
          ...
          <div>
            ...
            {['Nos professeurs', 'Comment ça marche', 'FAQ', 'Devenir tuteur'].map(function(item) {
              return (
                <div key={item} style={{ fontSize: '0.78rem', color: textMuted, marginBottom: 10, cursor: 'pointer', transition: 'color 0.15s' }}
          ...
          <div>
            ...
            {['Conditions d\'utilisation', 'Politique de confidentialité', 'Mentions légales'].map(function(item) {
              return (
                <div key={item} style={{ fontSize: '0.78rem', color: textMuted, marginBottom: 10, cursor: 'pointer' }}>
                  {item}
                </div>
              );
            })}
