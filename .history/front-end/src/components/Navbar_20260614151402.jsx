...
export default function Navbar({ isDark, onToggleTheme, user, onLogout }) {
  ...
  return (
    <nav style={{ 
      ...
      {/* Mobile Menu Drawer */}
      {isOpen && (
        <div style={{ position: 'absolute', top: '60px', left: 0, right: 0, background: isDark ? '#0a0a0c' : '#ffffff', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)'}`, padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem', zIndex: 99 }} className="mobile-menu">
          ...
          {isLoggedIn ? (
            <>
              <button onClick={() => { navigate('/notifications'); setIsOpen(false); }} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px', cursor: 'pointer', color: isDark ? '#ffffff' : '#07090d' }}>
                <Bell size={16} /> Notifications
              </button>
              <button onClick={() => { navigate('/profile'); setIsOpen(false); }} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px', cursor: 'pointer', color: isDark ? '#ffffff' : '#07090d' }}>
                <User size={16} /> Mon profil
              </button>
              <button onClick={() => { navigate('/dashboard'); setIsOpen(false); }} style={{ padding: '10px', cursor: 'pointer', color: isDark ? '#ffffff' : '#07090d', textAlign: 'left' }}>
                Tableau de bord
              </button>
              <button onClick={onLogout} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px', cursor: 'pointer', color: '#dc2626', textAlign: 'left' }}>
                <LogOut size={16} /> Déconnexion
              </button>
            </>
          ) : (
