    ...
    <nav style={{ 
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '100%', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: '1.5rem' }} className="nav-links-desktop">
          <button onClick={() => scrollTo('tutors-section')} style={{ color: isDark ? '#a1a4a5' : '#4a5568', cursor: 'pointer', fontSize: '0.85rem' }}>Trouver un Prof</button>
          <button onClick={() => scrollTo('how-it-works')} style={{ color: isDark ? '#a1a4a5' : '#4a5568', cursor: 'pointer', fontSize: '0.85rem' }}>Comment ça marche</button>
          <button onClick={() => scrollTo('features-section')} style={{ color: isDark ? '#a1a4a5' : '#4a5568', cursor: 'pointer', fontSize: '0.85rem' }}>Qualité certifiée</button>
        </div>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }} className="nav-buttons-desktop">
          ...
          <button onClick={onToggleTheme} style={{ background: isDark ? '#1a1a1c' : '#e5e7eb', borderRadius: '8px', padding: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', width: '36px', height: '36px' }}>
            {isDark ? <Sun size={16} color={isDark ? '#ffffff' : '#07090d'} /> : <Moon size={16} color={isDark ? '#ffffff' : '#07090d'} />}
          </button>
              ...
              <div style={{ position: 'relative' }}>
                ...
                {isProfileOpen && (
                  <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: '8px', background: isDark ? '#0a0a0c' : '#ffffff', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)'}`, borderRadius: '12px', padding: '0.5rem', minWidth: '180px', zIndex: 100, boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
                    <button onClick={() => { navigate('/profile'); setIsProfileOpen(false); }} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px', width: '100%', background: 'none', border: 'none', cursor: 'pointer', color: isDark ? '#ffffff' : '#07090d', textAlign: 'left', borderRadius: '8px' }}>Mon profil</button>
                    <button onClick={() => { navigate('/dashboard'); setIsProfileOpen(false); }} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px', width: '100%', background: 'none', border: 'none', cursor: 'pointer', color: isDark ? '#ffffff' : '#07090d', textAlign: 'left', borderRadius: '8px' }}>Tableau de bord</button>
                    <div style={{ height: '1px', background: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)', margin: '4px 0' }} />
      ...
      {isOpen && (
        <div style={{ 
          ...
          <button onClick={() => { scrollTo('tutors-section'); setIsOpen(false); }} style={{ color: isDark ? '#a1a4a5' : '#4a5568', padding: '8px', textAlign: 'left', cursor: 'pointer' }}>Trouver un Prof</button>
          <button onClick={() => { scrollTo('how-it-works'); setIsOpen(false); }} style={{ color: isDark ? '#a1a4a5' : '#4a5568', padding: '8px', textAlign: 'left', cursor: 'pointer' }}>Comment ça marche</button>
          <button onClick={() => { scrollTo('features-section'); setIsOpen(false); }} style={{ color: isDark ? '#a1a4a5' : '#4a5568', padding: '8px', textAlign: 'left', cursor: 'pointer' }}>Qualité certifiée</button>
          <hr style={{ borderColor: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)', margin: '4px 0' }} />
          ...
          {isLoggedIn ? (
            <>
              ...
              <button onClick={() => { navigate('/dashboard'); setIsOpen(false); }} style={{ padding: '10px', cursor: 'pointer', color: isDark ? '#ffffff' : '#07090d' }}>Tableau de bord</button>
            ...
            <>
              <button onClick={() => { navigate('/login'); setIsOpen(false); }} style={{ padding: '10px', cursor: 'pointer', color: isDark ? '#ffffff' : '#374151', textAlign: 'center', border: `1px solid ${isDark ? 'rgba(255,255,255,0.2)' : '#d1d5db'}`, borderRadius: '8px', marginBottom: '4px' }}>Connexion</button>
              <button onClick={() => { navigate('/register'); setIsOpen(false); }} style={{ background: isDark ? '#ffffff' : '#111827', color: isDark ? '#111827' : '#ffffff', border: 'none', borderRadius: '8px', padding: '10px', cursor: 'pointer', fontWeight: 600, textAlign: 'center' }}>Devenir Tuteur / Élève</button>
            </>
          )}
        </div>
      )}
