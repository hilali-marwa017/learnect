...
export default function Navbar({ isDark, onToggleTheme, user, onLogout }) {
  ...
  function handleDashboardClick() {
    if (user?.role === 'enseignant') navigate('/teacher');
            ...
            <>
              ...
              <div style={{ position: 'relative' }}>
                <button
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'none', border: 'none', cursor: 'pointer', padding: '4px 8px', borderRadius: '8px' }}
                >
                    ...
                    {!isAdmin && (
                      <>
                        ...
                        <button
                          ...
                          style={{ display: 'flex', alignItems: 'center', padding: '10px 12px', width: '100%', background: 'none', border: 'none', cursor: 'pointer', color: ddText, textAlign: 'left', borderRadius: '8px', fontSize: '0.9rem', fontWeight: 600 }}
                        >
                          {user?.role === 'enseignant' ? '🎓 Mon espace prof' : '📊 Mon tableau de bord'}
                        </button>

                        {/* Profil */}
                        <button
                          onClick={handleProfileClick}
                          style={{ display: 'flex', alignItems: 'center', padding: '10px 12px', width: '100%', background: 'none', border: 'none', cursor: 'pointer', color: ddText, textAlign: 'left', borderRadius: '8px', fontSize: '0.9rem' }}
                        >
                          {user?.role === 'enseignant' ? '⚙️ Modifier mon profil' : '👤 Mon profil'}
                        </button>
                      </>
                    )}

                    <div style={{ height: '1px', background: ddBorder, margin: '4px 0' }} />
            ...
            <>
              {!isAdmin && (
                <>
                  <button
                    onClick={handleDashboardClick}
                    style={{ padding: '10px', cursor: 'pointer', color: isDark ? '#ffffff' : '#07090d', textAlign: 'left', background: 'none', border: 'none', fontSize: '0.9rem', fontWeight: 600 }}
                  >
                    {user?.role === 'enseignant' ? '🎓 Mon espace prof' : '📊 Mon tableau de bord'}
                  </button>
                  <button
                    onClick={handleProfileClick}
                    style={{ padding: '10px', cursor: 'pointer', color: isDark ? '#ffffff' : '#07090d', textAlign: 'left', background: 'none', border: 'none', fontSize: '0.9rem' }}
                  >
                    {user?.role === 'enseignant' ? '⚙️ Modifier mon profil' : '👤 Mon profil'}
                  </button>
                </>
