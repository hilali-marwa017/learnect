          ...
          {user ? (
            <>
              <div style={{ position: 'relative' }}>
                <button onClick={handleToggleNotif} style={{ width: 40, height: 40, border: '1.5px solid #e5e7eb', borderRadius: '50%', background: '#f8fafc', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
              ...
              <Link to={getDashboardLink()} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 10px', borderRadius: 50, border: '1.5px solid #e5e7eb', background: '#f8fafc', textDecoration: 'none' }}>
                <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'linear-gradient(135deg, #0d6efd, #0a58ca)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 800, fontSize: '0.8rem' }}>
                  {user?.prenom?.toUpperCase() || user?.nom?.[0]?.toUpperCase() || 'A'}
                </div>
                <div>
              ...
              <button onClick={handleLogout} disabled={isLoggingOut} style={{ ...btnStyle, cursor: isLoggingOut ? 'not-allowed' : 'pointer', opacity: isLoggingOut ? 0.7 : 1 }}>
                {isLoggingOut
                  ? <><span className="spinner-border spinner-border-sm" style={{ width: '14px', height: '14px' }}></span><span>Déconnexion...</span></>
                  : <><i className="bi bi-box-arrow-right"></i><span>Déconnexion</span></>
                }
              </button>
            </>
          ) : (
              ...
              <div style={{ position: 'relative' }}>
                ...
                {showRegisterMenu && (
                  <div style={{ position: 'absolute', top: 48, right: 0, background: 'white', borderRadius: 12, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', border: '1px solid #e5e7eb', width: 260, zIndex: 200 }}>
                    <Link to="/register?role=etudiant" onClick={function() { setShowRegisterMenu(false); }} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px', textDecoration: 'none', color: '#0f172a', borderBottom: '1px solid #f1f5f9' }}>
                    ...
                    <Link to="/register?role=enseignant" onClick={function() { setShowRegisterMenu(false); }} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px', textDecoration: 'none', color: '#0f172a' }}>
