          ...
          <form onSubmit={handleSubmit} style={{ marginBottom: '2rem' }}>
            <div style={{ background: 'white', borderRadius: '60px', boxShadow: '0 20px 35px -10px rgba(0,0,0,0.1)', padding: '6px', display: 'flex', alignItems: 'center', gap: '4px' }}>
              ...
              <div style={{ flex: 2, position: 'relative', display: 'flex', alignItems: 'center' }}>
                <i className="bi bi-search" style={{ position: 'absolute', left: '20px', color: '#94A3B8', fontSize: '1rem', zIndex: 1 }}></i>
                <input
                  type="text"
                  ...
                  style={{ width: '100%', padding: '14px 20px 14px 48px', border: 'none', borderRadius: '60px', fontSize: '0.95rem', outline: 'none', background: '#F8FAFC' }}
                />
                {showMatiere && matieresFiltrees.length > 0 && (
                  <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: 'white', border: '1px solid #E2E8F0', borderRadius: '16px', marginTop: '8px', zIndex: 1000, maxHeight: '280px', overflowY: 'auto', boxShadow: '0 10px 25px rgba(0,0,0,0.1)' }}>
                    {matieresFiltrees.slice(0, 10).map(m => (
                      <div key={m.id_matiere} onMouseDown={(e) => { e.preventDefault(); selectMatiere(m.nom); }} style={{ padding: '12px 18px', cursor: 'pointer', borderBottom: '1px solid #F1F5F9', fontSize: '0.9rem', fontWeight: 500 }}>
                        {m.nom}
                      </div>
                    ))}
                  </div>
              ...
              <div style={{ flex: 2, position: 'relative', display: 'flex', alignItems: 'center' }}>
                <i className="bi bi-geo-alt" style={{ position: 'absolute', left: '20px', color: '#94A3B8', fontSize: '1rem', zIndex: 1 }}></i>
                <input
                  type="text"
                  className="form-control"
                ...
                {showVille && villesFiltrees.length > 0 && (
                  <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: 'white', border: '1px solid #E2E8F0', borderRadius: '16px', marginTop: '8px', zIndex: 1000, maxHeight: '280px', overflowY: 'auto', boxShadow: '0 10px 25px rgba(0,0,0,0.1)' }}>
                    <div onMouseDown={(e) => { e.preventDefault(); selectVille('En ligne'); }} style={{ padding: '12px 18px', cursor: 'pointer', borderBottom: '1px solid #F1F5F9', fontSize: '0.9rem', fontWeight: 500 }}>
                      <i className="bi bi-wifi me-2" style={{ color: '#0d6efd' }}></i>En ligne (webcam)
                    </div>
                    {villesFiltrees.filter(v => v.nom !== 'En ligne').slice(0, 15).map(v => (
                      <div key={v.id_ville} onMouseDown={(e) => { e.preventDefault(); selectVille(v.nom); }} style={{ padding: '12px 18px', cursor: 'pointer', borderBottom: '1px solid #F1F5F9', fontSize: '0.9rem', fontWeight: 500 }}>
              ...
              <button type="submit" style={{ padding: '14px 32px', background: 'linear-gradient(135deg, #0d6efd, #0a58ca)', color: 'white', border: 'none', borderRadius: '60px', fontSize: '0.95rem', fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap', boxShadow: '0 2px 8px rgba(13,110,253,0.3)' }}>
