...
export default function StatsSection() {
  ...
  useEffect(function() {
    ...
    // Récupérer le nombre d'étudiants (route publique ? sinon utilise users)
    api.get('/users')
      .then(function(response) {
      ...
      .catch(function(err) {
        ...
        setStats(function(prev) {
          return { ...prev, totalEtudiants: 25 };
        });
      });
  }, []);

  var statItems = [
    { label: 'Tuteurs Accrédités', value: stats.totalEnseignants + '+', color: 'text-accent-orange' },
    { label: 'Matières Couvertes', value: stats.totalMatieres + '+', color: 'text-accent-blue' },
    { label: 'Élèves Accompagnés', value: stats.totalEtudiants + '+', color: 'text-accent-green' },
    { label: 'Satisfaction Client', value: stats.noteMoyenne + '/5', color: 'text-accent-yellow' }
  ];
