...
export default function HowItWorks({ isDark }) {
  const steps = [
    { num: '01', icon: Search, title: 'Cherchez le tuteur parfait', desc: 'Renseignez la matière et votre ville pour filtrer les professeurs.' },
    { num: '02', icon: MessageCircle, title: 'Échangez et offrez le diagnostic', desc: "Première heure d'évaluation totalement offerte." },
    { num: '03', icon: TrendingUp, title: 'Apprenez et progressez', desc: 'Cours à domicile ou en ligne, sans engagement.' },
  ];
          ...
          {steps.map(function(step) {
            ...
            return (
              <div key={step.num} style={{ textAlign: 'left' }}>
                <div style={{ width: 60, height: 60, background: isDark ? '#1a1a1a' : '#ffffff', border: `1px solid ${borderColor}`, borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1rem', fontFamily: 'monospace', fontSize: '1.2rem', fontWeight: 800, color: orange }}>{step.num}</div>
                <Icon size={28} color={orange} style={{ marginBottom: '0.75rem' }} />
