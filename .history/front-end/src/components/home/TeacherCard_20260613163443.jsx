export default function TeacherCard({ tutor, onSelect, isFavorite, onToggleFavorite }) {
  ...
  var note = parseFloat(tutor.noteMoyenne) || 0;  // ← Conversion en nombre
  var nbAvis = tutor.avis ? tutor.avis.length : 0;
  var tarif = parseFloat(tutor.tarifHeure) || 0;  // ← Conversion en nombre
  ...
  return (
    <div onClick={function() { onSelect(tutor); }} className="bg-white border rounded-xl overflow-hidden cursor-pointer hover:shadow-lg transition-all">
      <div className="relative h-40 bg-gradient-to-r from-orange-400 to-blue-400 flex items-center justify-center">
        ...
        {badgeText && (<div className="absolute top-2 left-2 bg-green-500 text-white text-xs px-2 py-1 rounded">{badgeText}</div>)}
      ...
      <div className="p-4">
        ...
        <p className="text-gray-600 text-sm mt-2">{tutor.description_profil || 'Cours particuliers de qualité'}</p>
        <div className="mt-3 pt-3 border-t flex justify-between items-center">
          <div>
            <span className="text-2xl font-bold text-gray-900">{tarif} MAD</span>
