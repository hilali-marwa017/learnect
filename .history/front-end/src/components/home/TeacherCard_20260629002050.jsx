// ====== FICHIER: src/components/home/TeacherCard.jsx ======
...
export default function TeacherCard({ teacher, user }) {
  ...
  const id = teacher.utilisateur_id || teacher.id;
  const prenom = teacher.user?.prenom || teacher.prenom || '';
  const nom = teacher.user?.nom || teacher.nom || '';
  const nomComplet = `${prenom} ${nom}`.trim() || 'Professeur';

  const photo = teacher.user?.photo || teacher.photo || teacher.avatar;
  const photoUrl = photo
    ? (photo.startsWith('http') ? photo : `http://localhost:8000/storage/${photo}`)
    : null;
  const avatar = photoUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(nomComplet)}&background=e04f00&color=fff`;

  const ville = teacher.user?.ville || teacher.ville || teacher.city || '';
  const tarifHeure = teacher.tarifHeure || teacher.rate || 0;
  const noteMoyenne = teacher.noteMoyenne || teacher.rating || 0;
  const titre = teacher.titre || teacher.role || '';
  const estVerifie = teacher.estVerifie || teacher.isVerified;
    ...
    >
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '1.2rem', zIndex: 2 }}>
        ...
        <div style={{ display: 'flex', gap: '6px', marginBottom: '12px', flexWrap: 'wrap' }}>
          ...
          {coursDeplacement && (
            <span style={{ fontSize: '0.62rem', background: 'rgba(255,255,255,0.12)', backdropFilter: 'blur(6px)', color: '#fff', padding: '4px 10px', borderRadius: '6px', display: 'flex', alignItems: 'center', gap: '5px', border: '1px solid rgba(255,255,255,0.15)' }}>
              <Navigation size={10} /> Déplacement
            </span>
          )}
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <div style={{ fontWeight: 800, color: '#ffffff', fontSize: '1.15rem', display: 'flex', alignItems: 'baseline', gap: '3px' }}>
              <span style={{ color: '#e04f00' }}>{tarifHeure}</span>
