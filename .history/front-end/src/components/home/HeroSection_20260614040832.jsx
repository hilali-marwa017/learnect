...
export default function HeroSection({ isDark, querySubject, setQuerySubject, queryCity, setQueryCity, activePill, setActivePill }) {
  ...
  return (
    <section className='py-5' style={{ background: bgColor, padding: '4rem 2rem', textAlign: 'center' }}>
      
      {/* TITRE AMÉLIORÉ */}
      <div className='px-3' style={{ maxWidth: 800, margin: '0 auto 1rem' }}>
        <h1 style={{ 
      ...
      {/* Barre de recherche */}
      <div style={{ display: 'flex', alignItems: 'center', background: searchBg, border: `1px solid ${searchBorder}`, borderRadius: 60, padding: '6px', maxWidth: 750, margin: '0 auto', boxShadow: isDark ? '0 8px 32px rgba(0,0,0,0.3)' : '0 8px 32px rgba(0,0,0,0.1)' }}>
        ...
        <div style={{ flex: 1, position: 'relative' }}>
          ...
          {showSubjectDrop && !loading && getFilteredSubjects().length > 0 && (
            <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: 8, background: dropBg, border: `1px solid ${searchBorder}`, borderRadius: 16, zIndex: 200, maxHeight: 300, overflowY: 'auto' }}>
              {getFilteredSubjects().map((subject, index) => (
                <div key={index} onMouseDown={() => onSelectSubject(subject.name)} style={{ padding: '14px 18px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 12, color: inputColor }} onMouseEnter={(e) => e.currentTarget.style.background = dropHover} onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
          ...
          {showCityDrop && !loading && getFilteredCities().length > 0 && (
            <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: 8, background: dropBg, border: `1px solid ${searchBorder}`, borderRadius: 16, zIndex: 200, maxHeight: 300, overflowY: 'auto' }}>
              {getFilteredCities().map((city, index) => (
                <div key={index} onMouseDown={() => onSelectCity(city)} style={{ padding: '14px 18px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 12, color: inputColor }} onMouseEnter={(e) => e.currentTarget.style.background = dropHover} onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
