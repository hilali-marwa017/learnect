...
export default function TeacherCard({ teacher, isDark, isFav, onToggleFav }) {
  ...
  const textColor = isDark ? '#000000' : '#07090d';
  ...
  return (
    <div onClick={handleCardClick} style={{ background: bgCard, border: `1px solid ${borderColor}`, borderRadius: 16, overflow: 'hidden', cursor: 'pointer', transition: 'transform 0.2s' }} onMouseEnter={(e) => e.currentTarget.style.transform = 'translateY(-4px)'} onMouseLeave={(e) => e.currentTarget.style.transform = 'translateY(0)'}>
      ...
      <div style={{ height: 160, background: isDark ? '#2a2a2a' : '#f3f4f6', position: 'relative' }}>
        ...
        {/* Badge 1er cours offert - VERT */}
        {teacher.isFirstFree && (
          <span style={{ position: 'absolute', top: 10, left: 10, background: green, color: '#fff', fontSize: '0.6rem', fontWeight: 700, padding: '3px 8px', borderRadius: 4 }}>
            1er cours offert
          </span>
        )}
      ...
      <div style={{ padding: '1rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <p style={{ fontSize: '0.7rem', color: textMuted, display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        <div style={{ display: 'flex', gap: 8, margin: '8px 0' }}>
          {teacher.delivery === 'face à face & webcam' && (
            <>
              <span style={{ fontSize: '0.6rem', background: isDark ? '#2a2a2a' : '#f3f4f6', padding: '2px 6px', borderRadius: 4, display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        <p style={{ fontSize: '0.7rem', color: textMuted, marginBottom: 12, lineHeight: 1.4 }}>
          {teacher.role.substring(0, 80)}...
        </p>
