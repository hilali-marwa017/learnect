...
function HeroSection() {
  ...
  const selectMatiere = (nom) => {
    ...
    // Utiliser setTimeout pour permettre à l'état de se mettre à jour
    setTimeout(() => {
      navigate(`/teachers?search=${nom}&ville=${villeInput}&enligne=false`);
    }, 50);
  };
  ...
  const selectVille = (nom) => {
    ...
    setShowVille(false);
    setTimeout(() => {
      navigate(`/teachers?search=${matiereInput}&ville=${nom}&enligne=false`);
    }, 50);
  };
          ...
          {tags.map(m => (
            <button
              ...
              onClick={() => {
                ...
                navigate(`/teachers?search=${m.nom}&ville=${villeInput}&enligne=false`);
              }}
            >
              {m.nom}
            </button>
