...
export default function TeacherCard({ teacher, user }) {
  ...
  function handleContact(e) {
    ...
    if (user.role === 'etudiant') { navigate('/student/messages'); return; }
    if (user.role === 'enseignant') { navigate('/teacher/dashboard'); return; }
    navigate('/login');
  }
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '1.2rem', zIndex: 2 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'linear-gradient(135deg, #e04f00, #c43d00)', color: '#fff', border: 'none', borderRadius: '10px', padding: '8px 16px', fontWeight: 700, fontSize: '0.78rem', cursor: 'pointer', boxShadow: '0 2px 10px rgba(224,79,0,0.4)', letterSpacing: '0.02em' }}
          >
