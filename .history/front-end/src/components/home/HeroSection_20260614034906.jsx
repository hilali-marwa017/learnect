          ...
          {showSubjectDrop && getFilteredSubjects().length > 0 && (
            <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: 8, background: dropBg, border: `1px solid ${searchBorder}`, borderRadius: 16, zIndex: 200, maxHeight: 280, overflowY: 'auto' }}>
              {getFilteredSubjects().map(function(subject, index) {
                return (
                  <div key={subject.name + index} onMouseDown={() => onSelectSubject(subject.name)} style={{ padding: '12px 16px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10, color: inputColor }} onMouseEnter={(e) => e.currentTarget.style.background = dropHover} onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
          ...
          {showCityDrop && getFilteredCities().length > 0 && (
            <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: 8, background: dropBg, border: `1px solid ${searchBorder}`, borderRadius: 16, zIndex: 200, maxHeight: 280, overflowY: 'auto' }}>
              {getFilteredCities().map(function(city, index) {
                return (
                  <div key={city + index} onMouseDown={() => onSelectCity(city)} style={{ padding: '12px 16px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10, color: inputColor }} onMouseEnter={(e) => e.currentTarget.style.background = dropHover} onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
