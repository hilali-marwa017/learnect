...
function FaqItem({ question, answer }) {
  ...
  return (
    <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #E2E8F0', marginBottom: '0.8rem', overflow: 'hidden' }}>
      <button onClick={() => setOpen(!open)} style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem 1.3rem', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
        <span style={{ fontWeight: 600, color: '#0F172A', fontSize: '0.9rem' }}>{question}</span>
        <i className={`bi bi-chevron-${open ? 'up' : 'down'}`} style={{ color: '#0d6efd', fontSize: '0.85rem' }}></i>
      </button>
      {open && <div style={{ padding: '0.8rem 1.3rem 1.1rem', color: '#6B7280', fontSize: '0.85rem', lineHeight: 1.6, borderTop: '1px solid #E2E8F0' }}>{answer}</div>}
    </div>
  );
}
    ...
    <section style={{ padding: '4rem 0', background: '#F8FAFC' }}>
      <div className="container">
        <div style={{ marginBottom: '2rem' }}>
          <div style={{ color: '#0d6efd', fontSize: '0.7rem', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase' }}>UNE QUESTION ?</div>
          <h2 style={{ fontSize: '1.6rem', fontWeight: 800, color: '#0F172A', marginTop: '6px' }}>Foire Aux Questions</h2>
          <p style={{ color: '#6B7280', fontSize: '0.85rem', marginTop: '6px' }}>Tout savoir sur les cours particuliers et notre plateforme</p>
        </div>
