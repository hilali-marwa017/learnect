    ...
    <section className="bg-light py-5">
      <div className="container text-center">
        ...
        <form onSubmit={handleSubmit} className="row justify-content-center g-2 mb-4">
            ...
            {showMatiere && matieresFiltrees.length > 0 && (
              <div className="dropdown-menu show position-absolute w-100 mt-1 p-0 shadow-sm" style={{ zIndex: 1000, maxHeight: '250px', overflowY: 'auto' }}>
                {matieresFiltrees.slice(0, 25).map(m => (
                  <div key={m.id_matiere} className="dropdown-item d-flex justify-content-between align-items-center" onMouseDown={(e) => { e.preventDefault(); selectMatiere(m.nom); }} style={{ cursor: 'pointer' }}>
                    <span>{m.nom}</span>
                    <span className="badge bg-light text-secondary ms-2">{m.categorie}</span>
                  </div>
                ))}
                {matieresFiltrees.length > 25 && (
                  <div className="dropdown-item text-muted small text-center">+ {matieresFiltrees.length - 25} autres matières</div>
                )}
              </div>
            )}
          </div>
          ...
          <div className="col-md-4 position-relative">
            ...
            {showVille && villesFiltrees.length > 0 && (
              <div className="dropdown-menu show position-absolute w-100 mt-1 p-0 shadow-sm" style={{ zIndex: 1000, maxHeight: '250px', overflowY: 'auto' }}>
                <div className="dropdown-item" onMouseDown={(e) => { e.preventDefault(); selectVille('En ligne'); }} style={{ cursor: 'pointer' }}>
                ...
                {villesFiltrees.filter(v => v.nom !== 'En ligne').slice(0, 25).map(v => (
                  <div key={v.id_ville} className="dropdown-item" onMouseDown={(e) => { e.preventDefault(); selectVille(v.nom); }} style={{ cursor: 'pointer' }}>
                    <i className="bi bi-geo-alt text-secondary me-2"></i>
                    {v.nom}
                  </div>
                ))}
                {villesFiltrees.filter(v => v.nom !== 'En ligne').length > 25 && (
                  <div className="dropdown-item text-muted small text-center">+ {villesFiltrees.filter(v => v.nom !== 'En ligne').length - 25} autres villes</div>
                )}
              </div>
            )}
          </div>
        ...
        <div className="d-flex flex-wrap justify-content-center gap-2 mt-3">
          {tags.map(m => (
            <button key={m.id_matiere} className="btn btn-outline-secondary btn-sm rounded-pill px-3" onClick={() => { setMatiereInput(m.nom); setShowMatiere(false); }}>
              {m.nom}
            </button>
          ))}
        </div>
        ...
        {/* Lien enseignant */}
        <div className="mt-4">
          <Link to="/register?role=enseignant" className="text-primary text-decoration-none">
