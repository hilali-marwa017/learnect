...
function HeroSection() {
  ...
  const tags = [
    'Anglais', 'Maths', 'Français', 'Arabe',
    'Coaching Sportif', 'Soutien scolaire', 'Physique - Chimie', 'Espagnol', 'SVT', 'Comptabilité'
  ];
      ...
      <div className="container">
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            <span style={{ fontSize: '2.5rem', fontWeight: 800, color: '#0F172A', letterSpacing: '-1px' }}>
              EN
            </span>
          </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
              ...
              <div style={{ flex: 2, minWidth: 250, position: 'relative' }}>
                <input
                  ...
                  style={{ width: '100%', padding: '14px 18px', border: '1px solid #E2E8F0', borderRadius: '12px', fontSize: '1rem', outline: 'none', background: 'white' }}
                />
              ...
              <button type="submit" style={{ padding: '14px 32px', background: '#0d6efd', color: 'white', border: 'none', borderRadius: '12px', fontSize: '1rem', fontWeight: 600, cursor: 'pointer' }}>
                Rechercher
              </button>
            </div>
          </form>

          {/* Tags en colonnes comme l'image */}
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', marginTop: '1rem' }}>
            {tags.map(tag => (
              <button
                key={tag}
                onClick={() => setMatiereInput(tag)}
                style={{ padding: '6px 0', background: 'none', border: 'none', fontSize: '0.95rem', color: '#4A5568', cursor: 'pointer', fontWeight: 500 }}
              >
                {tag}
              </button>
            ))}
