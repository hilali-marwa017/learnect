  ...
  return (
    <div onClick={handleCardClick} style={{ background: bgCard, border: `1px solid ${borderColor}`, borderRadius: 16, overflow: 'hidden', cursor: 'pointer', transition: 'transform 0.2s' }} onMouseEnter={(e) => e.currentTarget.style.transform = 'translateY(-4px)'} onMouseLeave={(e) => e.currentTarget.style.transform = 'translateY(0)'}>
      <div style={{ height: 160, background: isDark ? '#2a2a2a' : '#f3f4f6', position: 'relative' }}>
        ...
        {teacher.isFirstFree && <span style={{ position: 'absolute', top: 10, left: 10, background: green, color: '#fff', fontSize: '0.6rem', fontWeight: 700, padding: '3px 8px', borderRadius: 4 }}>1er cours offert</span>}
      ...
      <div style={{ padding: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: 8, margin: '8px 0' }}>
          ...
          {teacher.delivery === 'webcam' && <span style={{ fontSize: '0.6rem', background: isDark ? '#2a2a2a' : '#f3f4f6', padding: '2px 6px', borderRadius: 4, display: 'flex', alignItems: 'center', gap: 4 }}><Video size={10} /> Webcam</span>}
          {teacher.delivery === 'face à face' && <span style={{ fontSize: '0.6rem', background: isDark ? '#2a2a2a' : '#f3f4f6', padding: '2px 6px', borderRadius: 4, display: 'flex', alignItems: 'center', gap: 4 }}><Home size={10} /> Face à face</span>}
        </div>
