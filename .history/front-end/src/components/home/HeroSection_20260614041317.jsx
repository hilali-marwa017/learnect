    ...
    <section style={{ background: bgColor, padding: '4rem 2rem', textAlign: 'center' }}>
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto 1rem' }}>
        <h1 style={{ 
          ...
          <span style={{ display: 'block' }}>La réussite commence</span>
          <span style={{ display: 'block', color: orange }}>avec le bon professeur</span>
        </h1>
        <p style={{ fontSize: '1rem', color: textMuted, maxWidth: '550px', margin: '1rem auto 0', lineHeight: 1.5 }}>
          +450 professeurs certifiés • Cours à domicile ou en ligne • Premier cours offert
        </p>
      </div>

      {/* Barre de recherche */}
      <div style={{ display: 'flex', alignItems: 'center', background: searchBg, border: `1px solid ${borderColor}`, borderRadius: '60px', padding: '6px', maxWidth: '750px', margin: '0 auto', boxShadow: isDark ? '0 8px 32px rgba(0,0,0,0.3)' : '0 8px 32px rgba(0,0,0,0.1)' }}>
        ...
        <div style={{ flex: 1, position: 'relative' }}>
          ...
          {showSubjectDrop && !loading && getFilteredSubjects().length > 0 && (
            <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: '8px', background: dropBg, border: `1px solid ${borderColor}`, borderRadius: '16px', zIndex: 200, maxHeight: '300px', overflowY: 'auto', boxShadow: '0 8px 32px rgba(0,0,0,0.15)' }}>
              {getFilteredSubjects().map((subject, index) => (
                <div key={index} onMouseDown={() => onSelectSubject(subject.name)} style={{ padding: '14px 18px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '12px', color: inputColor, fontSize: '0.9rem' }} onMouseEnter={(e) => e.currentTarget.style.background = dropHover} onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
        ...
        {/* Séparateur */}
        <div style={{ width: '1px', height: '40px', background: separator }} />
          ...
          {showCityDrop && !loading && getFilteredCities().length > 0 && (
            <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: '8px', background: dropBg, border: `1px solid ${borderColor}`, borderRadius: '16px', zIndex: 200, maxHeight: '300px', overflowY: 'auto', boxShadow: '0 8px 32px rgba(0,0,0,0.15)' }}>
              {getFilteredCities().map((city, index) => (
                <div key={index} onMouseDown={() => onSelectCity(city)} style={{ padding: '14px 18px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '12px', color: inputColor, fontSize: '0.9rem' }} onMouseEnter={(e) => e.currentTarget.style.background = dropHover} onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
        ...
        {/* Bouton Rechercher */}
        <button onClick={scrollToTeachers} style={{ background: searchBtnBg, color: searchBtnText, border: 'none', borderRadius: '50px', padding: '14px 32px', fontWeight: 600, fontSize: '0.95rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          {subjectsList.slice(0, 8).map((pill, index) => {
            ...
            return (
              <button key={index} onClick={() => {
                ...
                scrollToTeachers();
              }} style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '8px 20px', borderRadius: '999px', fontSize: '0.85rem', cursor: 'pointer', border: `1px solid ${isActive ? orange : pillBorder}`, background: isActive ? orange : 'transparent', color: isActive ? '#fff' : pillText }}>
