  ...
  return (
    <section style={{ background: isDark ? '#000000' : '#f8f9fc', padding: '4rem 2rem', textAlign: 'center' }}>
      <div style={{ marginBottom: '2rem' }}>
        <h1 style={{ fontFamily: "'EB Garamond', serif", fontSize: 'clamp(2rem, 8vw, 5rem)', fontWeight: 700, color: isDark ? '#ffffff' : '#07090d', lineHeight: 1.1, marginBottom: '0.5rem' }}>
          Trouvez le professeur
        </h1>
        <h2 style={{ fontFamily: "'EB Garamond', serif", fontSize: 'clamp(1.4rem, 5vw, 3rem)', fontWeight: 600, color: '#e04f00', lineHeight: 1.2 }}>
          qui vous correspond
        </h2>
      </div>

      {/* Search Bar */}
      <div className="hero-search-bar" style={{ display: 'flex', alignItems: 'center', background: isDark ? '#1a1a1c' : '#ffffff', borderRadius: '50px', padding: '4px', maxWidth: '850px', margin: '0 auto', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
        {/* Input Matière */}
            ...
            <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: '8px', background: isDark ? '#1a1a1c' : '#ffffff', border: `1px solid ${isDark ? 'rgba(255,255,255,0.12)' : 'rgba(0,0,0,0.08)'}`, borderRadius: '16px', zIndex: 200, maxHeight: '300px', overflowY: 'auto', boxShadow: '0 8px 32px rgba(0,0,0,0.15)' }}>
              {getFilteredSubjects().map((s, i) => (
                <div key={i} onMouseDown={() => onSelectSubject(s.name)} style={{ padding: '12px 20px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', color: isDark ? '#ffffff' : '#07090d' }} onMouseEnter={(e) => e.currentTarget.style.background = isDark ? '#2a2a2a' : '#f5f5f5'} onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>{getIcon(s.category)} <span>{s.name}</span></div>
        ...
        <div className="search-divider" style={{ width: '1px', height: '24px', background: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)' }} />
        ...
        {/* Bouton Rechercher */}
        <button onClick={scrollToTeachers} style={{ background: '#e04f00', color: '#fff', border: 'none', borderRadius: '50px', padding: '14px 32px', fontWeight: 600, fontSize: '0.95rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px', marginRight: '4px', whiteSpace: 'nowrap' }}>
          <Search size={18} /> <span className="search-btn-text">Rechercher</span>
        </button>
      </div>
