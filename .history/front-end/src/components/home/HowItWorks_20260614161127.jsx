  ...
  return (
    <section style={{ background: bg, padding: '4rem 2rem', borderBottom: `1px solid ${border}` }}>
      <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
        <h2 style={{ fontSize: '2rem', fontWeight: 700, color: text, marginBottom: '0.5rem', textAlign: 'left' }}>Comment fonctionne Learnect ?</h2>
        <p style={{ color: muted, marginBottom: '2rem', textAlign: 'left' }}>Une mise en relation simple, rapide et sécurisée.</p>
        <div className="hiw-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '2rem' }}>
          {steps.map((step, i) => {
            const Icon = step.icon;
            return (
              <div key={i} style={{ textAlign: 'left' }}>
                <div style={{ width: '60px', height: '60px', background: card, border: `1px solid ${border}`, borderRadius: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1rem', fontFamily: 'monospace', fontSize: '1.2rem', fontWeight: 800, color: step.color }}>{step.num}</div>
