  ...
  return (
    <div style={{ background: bg, padding: '3rem 2rem', borderTop: `1px solid ${border}`, borderBottom: `1px solid ${border}` }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        {/* Stats - Icône au-dessus, très proche du chiffre */}
        <div className="stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem' }}>
          {stats.map((s, i) => {
            const Icon = s.icon;
            return (
              <div key={i} style={{ textAlign: 'center' }}>
                <Icon size={28} color={s.color} style={{ marginBottom: '0.25rem' }} />
                <div style={{ fontSize: '1.6rem', fontWeight: 'bold', color: text, marginBottom: '0.1rem', lineHeight: 1.2 }}>{s.value}</div>
                <div style={{ fontSize: '0.65rem', color: muted, letterSpacing: '0.05em', lineHeight: 1.3 }}>{s.label}</div>
              </div>
            );
          })}
        </div>
