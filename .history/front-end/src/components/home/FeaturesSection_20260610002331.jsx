    ...
    <section style={{ padding: '5rem 0', background: '#F8FAFC' }}>
      <div className="container">
        <div style={{ marginBottom: '3rem' }}>
          <div style={{ color: '#0d6efd', fontSize: '0.75rem', fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 6 }}>POURQUOI CHOISIR LEARNECT ?</div>
          <h2 style={{ fontSize: '2rem', fontWeight: 800, color: '#0F172A', letterSpacing: '-0.5px' }}>Une plateforme qui révolutionne</h2>
          <p style={{ color: '#6B7280', fontSize: '0.95rem', marginTop: 6, maxWidth: 600 }}>Découvrez les avantages exclusifs de notre solution de cours particuliers</p>
        </div>
              ...
              <div key={i} className="col-md-4 col-lg-4">
                <div style={{ background: 'white', borderRadius: 20, padding: '1.8rem', border: '1px solid #E2E8F0', height: '100%' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
                    ...
                    <span style={{ background: '#EFF6FF', color: '#0d6efd', padding: '4px 10px', borderRadius: 20, fontSize: '0.65rem', fontWeight: 700 }}>{f.badge}</span>
                  </div>
                  <h3 style={{ fontWeight: 700, color: '#0F172A', fontSize: '1.1rem', marginBottom: 8 }}>{f.title}</h3>
                  <p style={{ color: '#6B7280', fontSize: '0.85rem', lineHeight: 1.6, margin: 0 }}>{f.desc}</p>
                </div>
              </div>
            );
          })}
