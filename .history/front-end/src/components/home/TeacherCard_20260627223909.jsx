...
import { MapPin, Star, Video, Home, Gift, MessageCircle, Calendar } from 'lucide-react';
...
export default function TeacherCard({ teacher, user }) {
  ...
  return (
    ...
    >
      ...
      {teacher.isFirstFree && (
        <div style={{ position: 'absolute', top: '14px', left: '14px', zIndex: 2, display: 'flex', alignItems: 'center', gap: '6px', background: 'linear-gradient(135deg, #4e4e4e, #292f2d)', color: '#fff', fontSize: '0.65rem', fontWeight: 700, padding: '5px 12px', borderRadius: '999px', boxShadow: '0 2px 8px rgba(78, 83, 82, 0.45)', letterSpacing: '0.03em' }}>
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '1.2rem', zIndex: 2 }}>
        ...
        {/* ✅ 3 BADGES - AUCUN SUPPRIME */}
        <div style={{ display: 'flex', gap: '6px', marginBottom: '12px', flexWrap: 'wrap' }}>
          {teacher.cours_domicile === 1 && (
          ...
          {teacher.cours_enligne === 1 && (
            <span style={{ fontSize: '0.62rem', background: 'rgba(255,255,255,0.12)', backdropFilter: 'blur(6px)', color: '#fff', padding: '4px 10px', borderRadius: '6px', display: 'flex', alignItems: 'center', gap: '5px', border: '1px solid rgba(255,255,255,0.15)' }}>
              <Video size={10} /> Webcam
            </span>
          )}
          {teacher.cours_deplacement === 1 && (
            <span style={{ fontSize: '0.62rem', background: 'rgba(255,255,255,0.12)', backdropFilter: 'blur(6px)', color: '#fff', padding: '4px 10px', borderRadius: '6px', display: 'flex', alignItems: 'center', gap: '5px', border: '1px solid rgba(255,255,255,0.15)' }}>
              <MapPin size={10} /> Déplacement
            </span>
          )}
        </div>
