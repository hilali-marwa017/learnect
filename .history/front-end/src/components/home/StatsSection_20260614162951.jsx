          ...
          {stats.map((s, i) => {
            ...
            return (
              <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0' }}>
                ...
                <div style={{ fontSize: '1.6rem', fontWeight: 'bold', color: text, lineHeight: 1.1, marginTop: '2px' }}>{s.value}</div>
                <div style={{ fontSize: '0.65rem', color: muted, letterSpacing: '0.05em', lineHeight: 1.3, marginTop: '2px' }}>{s.label}</div>
              </div>
            );
          })}
        </div>
