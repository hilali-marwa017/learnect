...
import { matiereAPI } from '../../services/api.js'
...
export default function HeroSection({ onSearch }) {
  ...
  useEffect(() => {
    const fetchMatieres = async () => {
      try {
        const res = await matiereAPI.index()
        setMatieres(res.data)
      } catch (err) {
        console.log('Failed to fetch matieres', err)
        setMatieres([
          { id_matiere: 1, nom: 'Mathématiques', categorie: 'Sciences' },
          { id_matiere: 2, nom: 'Physique', categorie: 'Sciences' },
          { id_matiere: 3, nom: 'Chimie', categorie: 'Sciences' },
          { id_matiere: 4, nom: 'SVT', categorie: 'Sciences' },
          { id_matiere: 5, nom: 'Français', categorie: 'Langues' },
          { id_matiere: 6, nom: 'Anglais', categorie: 'Langues' },
          { id_matiere: 7, nom: 'Arabe', categorie: 'Langues' },
          { id_matiere: 8, nom: 'Philosophie', categorie: 'Lettres' },
          { id_matiere: 9, nom: 'Python', categorie: 'Informatique' },
          { id_matiere: 10, nom: 'Web Dev', categorie: 'Informatique' },
          { id_matiere: 11, nom: 'Économie', categorie: 'Sciences Humaines' },
        ])
      }
    }
    fetchMatieres()
            ...
            {isSubjectFocused && (
              <div className="absolute top-full left-0 right-0 mt-3 z-50 bg-surface-card border border-hairline-strong rounded-xl shadow-2xl overflow-hidden py-1 max-h-60 overflow-y-auto text-left">
                ...
                {getFilteredSubjects().map((matiere) => (
                  <button key={matiere.id_matiere} type="button"
                    onClick={() => { setQuerySubject(matiere.nom); setIsSubjectFocused(false); onSearch({ subject: matiere.nom, city: queryCity }) }}
              ...
              <div className="absolute top-full left-0 right-0 mt-3 z-50 bg-surface-card border border-hairline-strong rounded-xl shadow-2xl overflow-hidden py-1 max-h-60 overflow-y-auto text-left">
                ...
                {getFilteredCities().map((city) => (
                  <button key={city} type="button"
                    onClick={() => { setQueryCity(city); setIsCityFocused(false); onSearch({ subject: activePill || querySubject, city }) }}
            ...
            {SUBJECT_PILLS.map((pill) => {
              ...
              return (
                <button key={pill} onClick={() => handlePillClick(pill)}
                  className={`px-4 py-1.5 rounded-full border text-xs font-medium tracking-wide transition-all cursor-pointer ${isSelected ? 'bg-accent-orange text-canvas border-accent-orange font-bold scale-105' : 'border-hairline bg-surface-deep/40 text-charcoal hover:border-hairline-strong hover:text-ink'}`}
                >
                  {pill}
                </button>
              )
