...
export default function HeroSection({ isDark, querySubject, setQuerySubject, queryCity, setQueryCity, activePill, setActivePill }) {
  ...
  const getFilteredSubjects = () => {
    ...
    return subjectsList.filter(s => s.name.toLowerCase().includes(querySubject.toLowerCase())).slice(0, 7);
  };

  // Filtrer les villes
  const getFilteredCities = () => {
    if (!queryCity) return citiesList.slice(0, 6);
    return citiesList.filter(c => c.toLowerCase().includes(queryCity.toLowerCase())).slice(0, 6);
  };
