...
export default function TeacherCard({ teacher, user }) {
  ...
  const photo = teacher.user?.photo
    ? (teacher.user.photo.startsWith('http') ? teacher.user.photo : `http://localhost:8000/storage/${teacher.user.photo}`)
    : null;
  const avatar = photo || `https://ui-avatars.com/api/?name=${encodeURIComponent(nomComplet)}&background=e04f00&color=fff`;
