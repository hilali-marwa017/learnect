  ...
  return (
    <section style={{ background: isDark ? '#000000' : '#f8f9fc', padding: '4rem 2rem', textAlign: 'center' }}>
      ...
      <div className="search-bar-wrapper" style={{ display: 'flex', alignItems: 'center', background: isDark ? '#1a1a1c' : '#ffffff', borderRadius: '50px', padding: '4px', maxWidth: '850px', margin: '0 auto', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
        ...
        {/* Bouton Rechercher */}
        <button onClick={scrollToTeachers} style={{ background: '#e04f00', color: '#fff', border: 'none', borderRadius: '50px', padding: '14px 32px', fontWeight: 600, fontSize: '0.95rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px', marginRight: '4px', flexShrink: 0 }}>
          <Search size={18} /> <span className="search-btn-label">Rechercher</span>
        </button>
      </div>

      {/* ✅ Pills - UN SEUL BOUTON REINITIALISER */}
      {!loading && subjectsList.length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '10px', marginTop: '2rem' }}>
          ...
          {/* ✅ UN SEUL BOUTON REINITIALISER - ici */}
          {hasFilter && (
            <button 
              onClick={handleReset} 
              style={{ 
