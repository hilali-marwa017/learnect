...
export default function StatsSection({ isDark }) {
  ...
  return (
    <div style={{ background: bg, padding: '3rem 2rem', borderTop: `1px solid ${borderColor}`, borderBottom: `1px solid ${borderColor}` }}>
      <div style={{ maxWidth: 1000, margin: '0 auto', textAlign: 'center', marginBottom: '1.5rem' }}>
        <span style={{ fontSize: '0.65rem', letterSpacing: '0.2em', fontWeight: 700, textTransform: 'uppercase', color: orange }}>CHIFFRES CLÉS</span>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: textColor, marginTop: '0.5rem' }}>Learnect en quelques chiffres</h2>
      </div>
        ...
        {stats.map(function(stat, i) {
          ...
          return (
            <div key={i} style={{ textAlign: 'center' }}>
              <Icon size={24} color={stat.color} style={{ marginBottom: '0.25rem' }} />
              <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: textColor }}>{stat.value}</div>
              <div style={{ fontSize: '0.65rem', color: textMuted, letterSpacing: '0.05em' }}>{stat.label}</div>
            </div>
          );
        })}
      </div>
