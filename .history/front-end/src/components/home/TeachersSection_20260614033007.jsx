  ...
  if (filteredTutors.length === 0) {
    return (
      <section className='text-center' id="tutors-section" style={{ background: bgColor, padding: '3rem 2rem' }}>
        <p className='text-center' style={{ color: textMuted, textAlign: 'left' }}>Aucun professeur trouvé pour {(querySubject || activePill) && <strong>"{querySubject || activePill}"</strong>} {queryCity && <strong>à {queryCity}</strong>}</p>
