...
function TeachersSection() {
  ...
  const chargerEnseignants = async () => {
    ...
    setError('');
    try {
      // Pas de filtres - on récupère TOUS les enseignants
      const response = await api.get('/enseignants');
      
      console.log('Enseignants bruts:', response.data);
      
      // Afficher seulement les enseignants actifs (vérifiés + annonce en ligne)
      const actifs = response.data.filter(
        e => e.estVerifie === true && e.statut_annonce === 'en_ligne'
      );
      
      setEnseignants(actifs.slice(0, 6));
    } catch (err) {
