  ...
  return (
    <div style={{ background: bg, padding: '3rem 2rem', borderTop: `1px solid ${border}`, borderBottom: `1px solid ${border}` }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        {/* Stats - Icônes rapprochées des chiffres */}
        <div className="stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem' }}>
          {stats.map((s, i) => {
            const Icon = s.icon;
            return (
              <div key={i} style={{ textAlign: 'center' }}>
                {/* Icône plus petite et plus proche du chiffre */}
                <div style={{ marginBottom: '0.5rem' }}>
                  <Icon size={24} color={s.color} />
                </div>
                <div style={{ fontSize: '1.8rem', fontWeight: 'bold', color: text, marginBottom: '0.25rem', lineHeight: 1.2 }}>{s.value}</div>
                <div style={{ fontSize: '0.7rem', color: muted, letterSpacing: '0.05em', lineHeight: 1.3 }}>{s.label}</div>
              </div>
            );
          })}
        </div>
