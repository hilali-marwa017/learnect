...
export default function Navbar({ isDark, onToggleTheme, user, onLogout }) {
  ...
  }, [isLoggedIn, isAdmin, fetchUnread]);
  ...
  const avatarBg = isDark ? 'linear-gradient(135deg, #2a2a2c, #1a1a1c)' : 'linear-gradient(135deg, #374151, #111827)';
  ...
  return (
    <nav style={{ background: isDark ? '#0a0a0c' : '#ffffff', borderBottom: '1px solid ' + (isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)'), padding: '0 2rem', height: '70px', position: 'sticky', top: 0, zIndex: 100, display: 'flex', alignItems: 'center' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div onClick={() => { if (isAdmin) navigate('/admin'); else if (user?.role === 'enseignant') navigate('/teacher'); else if (user?.role === 'etudiant') navigate('/student'); else navigate('/'); }} style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
            ...
            <>
              ...
              <div style={{ position: 'relative' }}>
                <button onClick={() => setIsProfileOpen(!isProfileOpen)} style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'none', border: 'none', cursor: 'pointer', padding: '4px 8px', borderRadius: '8px' }}>
                  <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: avatarBg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                {isProfileOpen && (
                  <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: '8px', background: ddBg, border: '1px solid ' + ddBorder, borderRadius: '12px', padding: '0.5rem', minWidth: '220px', zIndex: 100, boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
                    <div style={{ padding: '10px 12px', borderBottom: '1px solid ' + ddBorder, marginBottom: '4px' }}>
                      <div style={{ fontWeight: 700, color: ddText, fontSize: '0.9rem' }}>{user?.prenom} {user?.nom}</div>
                      <div style={{ color: isDark ? '#a1a4a5' : '#718096', fontSize: '0.7rem' }}>{user?.email}</div>
                    </div>
                    <button onClick={handleLogout} disabled={loggingOut} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', width: '100%', background: 'none', border: 'none', cursor: loggingOut ? 'not-allowed' : 'pointer', color: '#dc2626', textAlign: 'left', borderRadius: '8px', fontSize: '0.9rem', fontWeight: 500, opacity: loggingOut ? 0.7 : 1 }} onMouseEnter={e => { if (!loggingOut) e.currentTarget.style.background = '#fef2f2'; }} onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; }}>
                      {loggingOut ? (
                        <><span style={{ display: 'inline-block', width: '16px', height: '16px', border: '2px solid rgba(220,38,38,0.25)', borderTopColor: '#dc2626', borderRadius: '50%', animation: 'spin 0.75s linear infinite', flexShrink: 0 }} /> Deconnexion...</>
                      ) : (
      ...
      {isOpen && (
        <div className="mobile-menu" style={{ position: 'fixed', top: '70px', left: 0, right: 0, background: isDark ? '#0a0a0c' : '#ffffff', borderBottom: '1px solid ' + (isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)'), padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem', zIndex: 99 }}>
          ...
          {isLoggedIn && !isAdmin && (
            <button onClick={() => { handleNotificationClick(); setIsOpen(false); }} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px', cursor: 'pointer', color: isDark ? '#ffffff' : '#07090d', textAlign: 'left', background: 'none', border: 'none', fontSize: '0.9rem' }}>
              <Bell size={18} /> Notifications
              {unreadCount > 0 && (
                <span style={{ marginLeft: 'auto', background: '#e04f00', color: '#fff', fontSize: '0.65rem', fontWeight: 700, padding: '2px 8px', borderRadius: '999px' }}>{unreadCount}</span>
              )}
            </button>
          )}
          {isLoggedIn ? (
            <button onClick={handleLogout} disabled={loggingOut} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px', cursor: loggingOut ? 'not-allowed' : 'pointer', color: '#dc2626', textAlign: 'left', background: 'none', border: 'none', opacity: loggingOut ? 0.7 : 1, fontSize: '0.9rem' }}>
              {loggingOut ? <><span style={{ display: 'inline-block', width: '16px', height: '16px', border: '2px solid rgba(220,38,38,0.25)', borderTopColor: '#dc2626', borderRadius: '50%', animation: 'spin 0.75s linear infinite', flexShrink: 0 }} /> Deconnexion...</> : <><LogOut size={16} /> Deconnexion</>}
            </button>
          ) : (
            <>
