...
import { GraduationCap, Sun, Moon, Menu, X, LogOut, Bell, ChevronDown, Sparkles, Loader } from 'lucide-react';
              ...
              <div style={{ position: 'relative' }}>
                ...
                {isProfileOpen && (
                  <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: '8px', background: isDark ? '#0a0a0c' : '#ffffff', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)'}`, borderRadius: '12px', padding: '0.5rem', minWidth: '200px', zIndex: 100, boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
                    ...
                    <button onClick={handleLogout} disabled={loggingOut} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 12px', width: '100%', background: 'none', border: 'none', cursor: 'pointer', color: '#dc2626', textAlign: 'left', borderRadius: '8px', opacity: loggingOut ? 0.6 : 1 }}>
                      {loggingOut
                        ? <><Loader size={16} style={{ animation: 'spin 1s linear infinite' }} /> Déconnexion...</>
      ...
      {isOpen && (
        <div className="mobile-menu" style={{ position: 'fixed', top: '70px', left: 0, right: 0, background: isDark ? '#0a0a0c' : '#ffffff', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)'}`, padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem', zIndex: 99 }}>
          ...
          {isLoggedIn ? (
            <>
              {!isAdmin && (
                <button onClick={() => { handleProfileClick(); setIsOpen(false); }} style={{ padding: '10px', cursor: 'pointer', color: isDark ? '#ffffff' : '#07090d', textAlign: 'left', background: 'none', border: 'none' }}>
                  Mon profil
                </button>
              )}
              <button onClick={() => { handleDashboardClick(); setIsOpen(false); }} style={{ padding: '10px', cursor: 'pointer', color: isDark ? '#ffffff' : '#07090d', textAlign: 'left', background: 'none', border: 'none' }}>
                Tableau de bord
              </button>
              <button onClick={handleLogout} disabled={loggingOut} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px', cursor: 'pointer', color: '#dc2626', textAlign: 'left', background: 'none', border: 'none', opacity: loggingOut ? 0.6 : 1 }}>
                {loggingOut
                  ? <><Loader size={16} style={{ animation: 'spin 1s linear infinite' }} /> Déconnexion...</>
                  : <><LogOut size={16} /> Déconnexion</>
                }
