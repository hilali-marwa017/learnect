...
export default function Footer({ isDark }) {
  ...
  return (
    <footer style={{ background: bg, borderTop: `1px solid ${border}`, padding: '3rem 2rem 1.5rem' }}>
      ...
      {/* Bottom bar: Liens légaux à GAUCHE, Copyright à DROITE */}
      <div style={{ 
        ...
        <div style={{ display: 'flex', gap: '1.5rem', fontSize: '0.7rem' }}>
          ...
          <Link to="/confidentialite" style={{ color: muted, textDecoration: 'none' }}>Confidentialité</Link>
          <Link to="/mentions-legales" style={{ color: muted, textDecoration: 'none' }}>Mentions légales</Link>
        </div>
      ...
      <style>{`
        @media (max-width: 768px) {
          .footer-grid { grid-templateColumns: repeat(2, 1fr) !important; }
        }
        @media (max-width: 480px) {
          .footer-grid { grid-templateColumns: 1fr !important; }
        }
      `}</style>
    </footer>
  );
