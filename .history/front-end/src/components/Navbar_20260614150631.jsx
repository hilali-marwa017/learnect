      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '100%', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }} className="nav-buttons-desktop">
          ...
          {isLoggedIn ? (
            ...
            <>
              <button onClick={() => navigate('/login')} style={{ color: text, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.85rem' }}>
                <LogIn size={16} /> Connexion
              </button>
              <button onClick={() => navigate('/register')} style={{ background: orange, color: '#fff', border: 'none', borderRadius: '8px', padding: '8px 16px', cursor: 'pointer', fontWeight: 600, fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
                <UserPlus size={16} /> Devenir Tuteur
              </button>
            </>
          )}
        </div>
      ...
      {isOpen && (
        <div style={{ position: 'absolute', top: '60px', left: 0, right: 0, background: bg, borderBottom: `1px solid ${border}`, padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem', zIndex: 99 }} className="mobile-menu">
          ...
          {isLoggedIn ? (
            ...
            <>
              <button onClick={() => { navigate('/login'); setIsOpen(false); }} style={{ padding: '10px', cursor: 'pointer', color: text, textAlign: 'center' }}>Connexion</button>
              <button onClick={() => { navigate('/register'); setIsOpen(false); }} style={{ background: orange, color: '#fff', border: 'none', borderRadius: '8px', padding: '10px', cursor: 'pointer', fontWeight: 600, textAlign: 'center' }}>Devenir Tuteur</button>
            </>
          )}
        </div>
      )}
