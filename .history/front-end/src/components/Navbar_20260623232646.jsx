...
import { GraduationCap, Sun, Moon, Menu, X, LogOut, Bell, ChevronDown, Sparkles, LayoutDashboard, User, Settings } from 'lucide-react';
...
export default function Navbar({ isDark, onToggleTheme, user, onLogout }) {
  ...
  const ddHover = isDark ? '#1a1a1c' : '#f1f3f5';
              ...
              <div style={{ position: 'relative' }}>
                ...
                {isProfileOpen && (
                  <div style={{ 
                    ...
                    {/* En-tête avec nom complet et email */}
                    <div style={{ 
                      padding: '10px 12px', 
                    ...
                    {!isAdmin && (
                      <>
                        ...
                        >
                          <LayoutDashboard size={16} color={isDark ? '#a1a4a5' : '#718096'} />
                        ...
                        <button
                          ...
                          onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                        >
                          <User size={16} color={isDark ? '#a1a4a5' : '#718096'} />
                          {user?.role === 'enseignant' ? 'Modifier mon profil' : 'Mon profil'}
                        </button>
                      </>
                    )}
            ...
            <>
              {!isAdmin && (
                <>
                  <button
                    onClick={handleDashboardClick}
                    style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px', cursor: 'pointer', color: isDark ? '#ffffff' : '#07090d', textAlign: 'left', background: 'none', border: 'none', fontSize: '0.9rem', fontWeight: 600 }}
                  >
                    <LayoutDashboard size={16} color={isDark ? '#a1a4a5' : '#718096'} />
                    {user?.role === 'enseignant' ? 'Mon espace prof' : 'Mon tableau de bord'}
                  </button>
                  <button
                    onClick={handleProfileClick}
                    style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px', cursor: 'pointer', color: isDark ? '#ffffff' : '#07090d', textAlign: 'left', background: 'none', border: 'none', fontSize: '0.9rem' }}
                  >
                    <User size={16} color={isDark ? '#a1a4a5' : '#718096'} />
                    {user?.role === 'enseignant' ? 'Modifier mon profil' : 'Mon profil'}
                  </button>
