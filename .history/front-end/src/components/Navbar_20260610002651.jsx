          ...
          {user ? (
            <>
              <Link to={getDashboardLink()} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 10px', borderRadius: 50, border: '1.5px solid #e5e7eb', background: '#f8fafc', textDecoration: 'none' }}>
                <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'linear-gradient(135deg, #0d6efd, #0a58ca)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 800, fontSize: '0.8rem' }}>{getInitials()}</div>
                <div>
              ...
              <button onClick={handleLogout} disabled={isLoggingOut} style={{ background: 'transparent', border: '1.5px solid #dc2626', borderRadius: 10, fontSize: 14, padding: '9px 20px', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8, color: '#dc2626', cursor: isLoggingOut ? 'not-allowed' : 'pointer', opacity: isLoggingOut ? 0.7 : 1 }}>
