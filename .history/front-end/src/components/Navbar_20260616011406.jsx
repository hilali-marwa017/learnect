              ...
              <div style={{ position: 'relative' }}>
                ...
                {isProfileOpen && (
                  <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: '8px', background: isDark ? '#0a0a0c' : '#ffffff', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)'}`, borderRadius: '12px', padding: '0.5rem', minWidth: '200px', zIndex: 100, boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
                    ...
                    {/* Mon profil masqué pour admin */}
                    {!isAdmin && (
                      <button onClick={handleProfileClick} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 12px', width: '100%', background: 'none', border: 'none', cursor: 'pointer', color: isDark ? '#ffffff' : '#07090d', textAlign: 'left', borderRadius: '8px' }}>
                        Mon profil
                      </button>
                    )}

                    <button onClick={handleDashboardClick} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 12px', width: '100%', background: 'none', border: 'none', cursor: 'pointer', color: isDark ? '#ffffff' : '#07090d', textAlign: 'left', borderRadius: '8px' }}>
                      Tableau de bord
                    </button>
      ...
      {isOpen && (
        <div className="mobile-menu" style={{ position: 'fixed', top: '70px', left: 0, right: 0, background: isDark ? '#0a0a0c' : '#ffffff', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)'}`, padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem', zIndex: 99 }}>
          ...
          {isLoggedIn ? (
            <>
              {!isAdmin && (
                <button onClick={handleProfileClick} style={{ padding: '10px', cursor: 'pointer', color: isDark ? '#ffffff' : '#07090d', textAlign: 'left', background: 'none', border: 'none' }}>
                  Mon profil
                </button>
              )}
              <button onClick={handleDashboardClick} style={{ padding: '10px', cursor: 'pointer', color: isDark ? '#ffffff' : '#07090d', textAlign: 'left', background: 'none', border: 'none' }}>
                Tableau de bord
              </button>
