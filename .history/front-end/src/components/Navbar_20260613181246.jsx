    ...
    <nav className="sticky top-0 z-50 bg-canvas/80 backdrop-blur-xl border-b border-hairline">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            {isAuthenticated ? (
              ...
              <div className="flex items-center gap-3">
                ...
                <Link
                  to="/register"
                  className="text-sm font-bold bg-ink text-canvas px-4 py-2 rounded-lg hover:bg-ash transition-colors"
                >
                  S'inscrire
                </Link>
              </div>
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-hairline bg-canvas px-6 py-4 space-y-3 animate-slide-up">
          ...
          {isAuthenticated ? (
            <>
              <Link to="/dashboard" onClick={() => setMobileOpen(false)} className="block text-sm text-charcoal py-2">
                Tableau de bord
              </Link>
              {isAdmin && (
                <Link to="/admin" onClick={() => setMobileOpen(false)} className="block text-sm text-accent-blue py-2">
                  Admin
                </Link>
              )}
