            ...
            <>
              <div style={{ position: 'relative' }}>
                <button onClick={() => setShowNotif(!showNotif)} style={{ width: 40, height: 40, border: '1.5px solid #e5e7eb', borderRadius: '50%', background: '#f8fafc', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                  ...
                  {unreadCount > 0 && <span style={{ position: 'absolute', top: -2, right: -2, width: 16, height: 16, borderRadius: '50%', background: '#ef4444', color: 'white', fontSize: 9, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid white' }}>{unreadCount}</span>}
                </button>
                {showNotif && (
              ...
              <button onClick={handleLogout} disabled={isLoggingOut} style={{ background: 'transparent', border: '1.5px solid #dc2626', borderRadius: 10, fontSize: 14, padding: '9px 20px', fontWeight: 600, cursor: isLoggingOut ? 'not-allowed' : 'pointer', opacity: isLoggingOut ? 0.7 : 1, display: 'flex', alignItems: 'center', gap: 8, color: '#dc2626' }}>
                {isLoggingOut ? <><span className="spinner-border spinner-border-sm"></span> Déconnexion...</> : <><i className="bi bi-box-arrow-right"></i> Déconnexion</>}
              </button>
            </>
          ) : (
            <>
              <div style={{ position: 'relative' }}>
                <button onClick={() => setShowRegisterMenu(!showRegisterMenu)} style={{ background: '#0d6efd', color: 'white', border: 'none', borderRadius: 10, fontSize: 14, padding: '9px 20px', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
                  S'inscrire <i className={`bi bi-chevron-${showRegisterMenu ? 'up' : 'down'}`} style={{ fontSize: 11, marginLeft: 4 }} />
                </button>
                {showRegisterMenu && (
