...
export default function PaymentSimulationModal({ 
  ...
  // ✅ FIX : handleSubmit ne fait QUE passer à success, PAS appeler onConfirmed
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      if (Math.random() < 0.9) {
        setStep('success'); // ← JUSTE ça, pas onConfirmed()
      } else {
        setStep('error');
  ...
  // ✅ FIX : handleOk est le SEUL endroit qui appelle onConfirmed
  const handleOk = () => {
    setStep('form');
    setError('');
    onConfirmed(); // ← SEULEMENT ici
  };
  ...
  // ✅ FIX : Reset step quand le modal s'ouvre
  React.useEffect(() => {
    if (open) {
      setStep('form');
      setError('');
      ...
      <div style={{
        ...
        {step === 'success' && (
          <div style={{
            ...
            {/* ✅ SEUL handleOk appelle onConfirmed */}
            <button
              onClick={handleOk}
              style={{
                marginTop: '0.5rem',
