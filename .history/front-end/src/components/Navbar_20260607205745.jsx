        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          ...
          {isAuthenticated ? (
            <>
              ...
              {/* Avatar - cliquable vers dashboard selon rôle */}
              <Link 
                to={getDashboardLink()} 
              ...
              {/* Déconnexion */}
              <button onClick={handleLogout} disabled={isLoggingOut} style={{ background: 'transparent', color: '#64748b', border: '1.5px solid #e5e7eb', borderRadius: 10, fontSize: 14, padding: '9px 18px', fontWeight: 600, cursor: isLoggingOut ? 'not-allowed' : 'pointer', opacity: isLoggingOut ? 0.7 : 1, display: 'flex', alignItems: 'center', gap: 8 }}>
                {isLoggingOut ? (
