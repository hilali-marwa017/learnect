...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    if (token && storedUser) {
      try {
        ...
        // ✅ FIX : ne pas restaurer un enseignant en_attente
        if (userData.role === 'enseignant' && userData.statut === 'en_attente') {
  ...
  async function registerMultipart(formData) {
    try {
      ...
      // ✅ FIX CRITIQUE : enseignant en_attente → NE PAS connecter
      // On ne stocke rien, on ne fait pas setUser
      // → l'enseignant voit l'écran de succès, pas le dashboard
      if (newUser.role === 'enseignant') {
        return { success: true, role: 'enseignant', pending: true };
      }

      // Etudiant → connecter normalement
      localStorage.setItem('token', token);
      localStorage.setItem('user', JSON.stringify(newUser));
      api.defaults.headers.common['Authorization'] = 'Bearer ' + token;
      setUser(newUser);
