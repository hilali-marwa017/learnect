...
export default function StudentMessages() {
      ...
      try {
        ...
        if (confirmed.length > 0) {
          if (targetReservationId) {
            const target = confirmed.find(r => r.id_reservation === Number(targetReservationId));
  ...
  // fonction pure pour filtrer les reservations
  ...
  // fonction pure pour le profil du professeur
  const getProfInfo = (reservation) => {
    const enseignant = reservation?.creneau?.enseignant;
    return {
      id: enseignant?.utilisateur_id || enseignant?.id_enseignant,
      prenom: enseignant?.user?.prenom || '',
      nom: enseignant?.user?.nom || '',
      jour: reservation?.creneau?.jour || '',
      heureDebut: reservation?.creneau?.heureDebut?.slice(0, 5) || '',
      heureFin: reservation?.creneau?.heureFin?.slice(0, 5) || '',
      date: reservation?.date || ''
    };
  };

  // fonction asynchrone pour envoyer un message
  const sendMessage = async (text, activeReservation) => {
    if (!text.trim() || !activeReservation) return null;
    try {
      const destinataire = activeReservation.creneau?.enseignant?.utilisateur_id || activeReservation.creneau?.id_enseignant;
  ...
  const handleSend = async (e) => {
    e.preventDefault();
    if (!typedText.trim() || !activeRes || sending) return;
    setSending(true);
    const newMsg = await sendMessage(typedText, activeRes);
    if (newMsg) {
  ...
  const filteredReservations = filterReservations(reservations, search);
  const profInfo = activeRes ? getProfInfo(activeRes) : null;
              ...
              <div style={{ flex: 1, overflowY: 'auto' }}>
                {loading ? (
                  ...
                  filteredReservations.map(r => {
                    ...
                    const prof = r.creneau?.enseignant?.user;
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: '0.82rem', fontWeight: 700, color: isActive ? '#e04f00' : text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginBottom: '3px' }}>
                            {fullName}
                          </div>
                          <div style={{ fontSize: '0.65rem', color: '#e04f00', fontWeight: 700, fontFamily: 'monospace', textTransform: 'uppercase', marginBottom: '2px' }}>
                            {profInfo.jour} {profInfo.heureDebut} — {profInfo.heureFin}
                          </div>
                          ...
                          {pId && (
                            <span
                              ...
                              style={{ fontSize: '0.62rem', color: '#2563eb', cursor: 'pointer', textDecoration: 'underline', display: 'inline-flex', alignItems: 'center', gap: '2px', marginTop: '2px' }}
                            >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: '#e04f00', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <span style={{ color: '#fff', fontSize: '0.8rem', fontWeight: 700 }}>
                        {getInitials(profInfo?.prenom, profInfo?.nom)}
                      </span>
                    </div>
                    <div>
                      <div style={{ fontSize: '0.85rem', fontWeight: 800, color: text }}>
                        {getFullName(profInfo?.prenom, profInfo?.nom)}
                      </div>
                      <div style={{ fontSize: '0.65rem', color: '#e04f00', fontWeight: 700, fontFamily: 'monospace', textTransform: 'uppercase' }}>
                        {profInfo?.jour} — {profInfo?.heureDebut} à {profInfo?.heureFin}
                      </div>
                    </div>
                  </div>
                  {profInfo?.id && (
                    <button
                      onClick={() => navigate('/teachers/' + profInfo.id)}
