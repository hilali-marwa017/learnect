...
export default function StudentReservations() {
  ...
  const context = (() => { try { return require('react').useContext; } catch { return null; } })();
  
  // Récupère isDark depuis outlet si disponible
  const [isDark, setIsDark] = useState(false);
  try {
    const { useOutletContext } = require('react-router-dom');
    const ctx = useOutletContext?.();
    if (ctx?.isDark !== undefined) setIsDark(ctx.isDark);
  } catch {}
  ...
  // { id_reservation, whatsapp, methode, montant }
  ...
  async function handleConfirm(id) {
    try {
      ...
      // Stocker les infos de contact retournées par le backend
      setConfirmedInfo({
        id_reservation: id,
              ...
              <div style={{ background: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.25)', borderRadius: '16px', padding: '1.25rem 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
                ...
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  ...
                  <button
                    ...
                    style={{ padding: '9px 14px', borderRadius: '10px', background: 'transparent', border: `1px solid ${border}`, color: muted, fontSize: '0.75rem', cursor: 'pointer' }}
                  >
                    ✕
                  </button>
                </div>
                    ...
                    <div key={r.id_reservation} style={{ background: bgCard, border: `1px solid ${isJustConfirmed ? 'rgba(34,197,94,0.3)' : border}`, borderRadius: '16px', padding: '1.25rem 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem', transition: 'border 0.3s' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        ...
                        <div style={{ fontSize: '0.72rem', color: muted }}>
                          Le {r.date} — <span style={{ color: '#16a34a', fontWeight: 700 }}>{r.montant === 0 || r.montant === '0.00' ? '1er cours GRATUIT' : `${r.montant} DH`}</span>
                        </div>
                      </div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', flexShrink: 0 }}>
                        ...
                        {/* Bouton Contacter — visible si confirmée */}
                        {r.statut === 'confirmee' && (
                          <button
                            onClick={() => navigate('/student/messages')}
