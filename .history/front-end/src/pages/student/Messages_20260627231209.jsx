...
export default function StudentMessages() {
  ...
  // ✅ NOUVEAU : on travaille par "conversation" (1 par professeur) au lieu de "par reservation"
  ...
  useEffect(() => {
    async function fetchConversations() {
      try {
        ...
        // ✅ regroupement par enseignant : 1 seule conversation, meme avec plusieurs reservations
        const groups = {};
        confirmed.forEach(r => {
          const profId = r.creneau?.enseignant?.utilisateur_id || r.creneau?.id_enseignant;
          if (!profId) return;
          if (!groups[profId]) {
            groups[profId] = { id_enseignant: profId, prof: r.creneau?.enseignant?.user, reservations: [] };
          }
          groups[profId].reservations.push(r);
        });
  ...
  useEffect(() => {
    ...
    async function fetchMessages() {
      try {
        // ✅ on fusionne les messages de TOUTES les reservations avec ce professeur
        ...
        // ✅ marquer tout comme lu pour chaque reservation du groupe
        await Promise.all(
  ...
  async function handleSend(e) {
    ...
    setSending(true);
    try {
      // ✅ on envoie sur la reservation la plus recente du groupe
      const latestReservation = activeConv.reservations[0];
      const res = await api.post('/messages', {
        contenu: typedText,
        id_destinataire: activeConv.id_enseignant,
  ...
  const filteredConversations = conversations.filter(c =>
    `${c.prof?.prenom} ${c.prof?.nom}`.toLowerCase().includes(search.toLowerCase())
  );

  const prof = activeConv?.prof;
  const latestRes = activeConv?.reservations?.[0];
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1.5rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        <div style={{ borderBottom: `1px solid ${border}`, paddingBottom: '1.5rem' }}>
          ...
          <p style={{ fontSize: '0.8rem', color: muted, margin: 0 }}>
            Echangez avec vos professeurs apres confirmation de reservation.
          </p>
        </div>
              ...
              <div style={{ flex: 1, overflowY: 'auto' }}>
                {loading ? (
                  ...
                  filteredConversations.map(c => {
                    const isActive = activeConv?.id_enseignant === c.id_enseignant;
                    const p = c.prof;
                      ...
                      >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <span style={{ fontSize: '0.65rem', color: '#e04f00', fontWeight: 700, fontFamily: 'monospace', textTransform: 'uppercase' }}>
                            {c.reservations.length} cours reserve{c.reservations.length > 1 ? 's' : ''}
                          </span>
                        </div>
                      </button>
                    );
                ...
                <div style={{ padding: '1rem 1.25rem', borderBottom: `1px solid ${border}`, display: 'flex', alignItems: 'center', gap: '12px', background: bgCard }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '0.65rem', color: '#e04f00', fontWeight: 700, fontFamily: 'monospace', textTransform: 'uppercase' }}>
                      {latestRes?.creneau?.jour} — {latestRes?.creneau?.heureDebut?.slice(0, 5)} a {latestRes?.creneau?.heureFin?.slice(0, 5)}
                    </div>
                  </div>
                </div>
