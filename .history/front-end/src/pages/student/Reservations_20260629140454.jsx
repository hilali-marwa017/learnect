...
// FIX : Affichage conditionnel selon statut paiement
// - paiement_recu  → message orange + PAS de num ni bouton Contacter
// - confirmee      → badge vert + numéro prof + bouton Contacter
// ============================================================

import React, { useState, useEffect } from 'react';
import { Link, useNavigate, useOutletContext } from 'react-router-dom';
import { StudentNavigationActive } from './Dashboard';
import { Calendar, Trash2, Check, MessageCircle, Phone, Star, CheckCheck, AlertCircle } from 'lucide-react';
...
export default function StudentReservations() {
  ...
  function peutTerminer(reservation) {
    ...
    if (!heureFin || !dateRes) return false;
    const dateFin = new Date(dateRes + 'T' + heureFin);
    return new Date() >= dateFin;
  }

  const getHeureFinMessage = (reservation) => {
    const heureFin = reservation.creneau?.heureFin;
    if (!heureFin) return '';
    return '⏳ Disponible à partir de ' + heureFin;
  };
  ...
  async function handlePaiementConfirme() {
    ...
    try {
      ...
      // ✅ statut devient paiement_recu côté backend
  ...
  // ✅ Extraire le numéro de téléphone du prof depuis la réservation
  function getTelephoneProf(r) {
    return r.creneau?.enseignant?.user?.telephone || null;
  }
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1.5rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            {/* ✅ BANNER après paiement confirmé */}
            {confirmedInfo && (
              <div style={{
                background: confirmedInfo.statut === 'paiement_recu' ? 'rgba(168,85,247,0.06)' : 'rgba(34,197,94,0.06)',
                border: confirmedInfo.statut === 'paiement_recu' ? '1px solid rgba(168,85,247,0.25)' : '1px solid rgba(34,197,94,0.25)',
                borderRadius: '16px',
                ...
                <div>
                  <div style={{
                    fontSize: '0.6rem',
                    color: confirmedInfo.statut === 'paiement_recu' ? '#a855f7' : '#16a34a',
                    ...
                    {confirmedInfo.statut === 'paiement_recu' ? '⏳ PAIEMENT REÇU — EN ATTENTE DU PROFESSEUR' : '✓ RÉSERVATION CONFIRMÉE'}
                  </div>
                  <p style={{ fontSize: '0.82rem', fontWeight: 700, color: text, margin: '0 0 4px' }}>
                    {confirmedInfo.statut === 'paiement_recu'
                      ? 'Votre paiement a été reçu avec succès !'
                      : 'Félicitations ! Votre cours est confirmé.'}
                  </p>
                  <p style={{ fontSize: '0.75rem', color: muted, margin: 0 }}>
                    {confirmedInfo.statut === 'paiement_recu'
                      ? 'Le professeur doit valider votre réservation avant que vous puissiez le contacter.'
                      : 'Votre cours est confirmé. Vous pouvez maintenant contacter le professeur.'}
                  </p>
                </div>
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
                  {confirmedInfo.statut === 'confirmee' && confirmedInfo.whatsapp && (
                    <a href={'https://wa.me/' + confirmedInfo.whatsapp.replace(/[^0-9]/g, '')}
                  ...
                  {confirmedInfo.statut === 'confirmee' && (
                    <button
                  ...
                  <button onClick={() => setConfirmedInfo(null)}
                    style={{ padding: '9px 12px', borderRadius: '10px', background: 'transparent', border: '1px solid ' + border, color: muted, fontSize: '0.75rem', cursor: 'pointer' }}>✕</button>
                </div>
              </div>
            )}
            ...
            {loading ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {reservations.map(r => {
                  ...
                  const telephone    = getTelephoneProf(r);
                  const isPaiementRecu = r.statut === 'paiement_recu';
                  ...
                  return (
                    <div key={r.id_reservation}
                      style={{
                        background: bgCard,
                        border: '1px solid ' + (isPaiementRecu ? 'rgba(168,85,247,0.3)' : isConfirmee ? 'rgba(34,197,94,0.25)' : border),
                        borderRadius: '16px',
                        padding: '1.25rem 1.5rem',
                        display: 'flex',
                      ...
                      {/* Ligne principale : infos + badge + boutons */}
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', flexShrink: 0 }}>
                          ...
                          {/* EN ATTENTE → bouton Confirmer paiement */}
                          {r.statut === 'en_attente' && (
                            <button onClick={() => openPaiement(r)}
                          ...
                          {/* ✅ CONFIRMEE → numéro prof + bouton Contacter */}
                          {isConfirmee && telephone && (
                            <a href={'https://wa.me/' + telephone.replace(/[^0-9]/g, '')}
                              target="_blank" rel="noopener noreferrer"
                              style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '6px 12px', borderRadius: '8px', border: '1px solid rgba(37,211,102,0.3)', background: 'rgba(37,211,102,0.08)', color: '#16a34a', fontSize: '0.72rem', fontWeight: 700, textDecoration: 'none' }}>
                          ...
                          {/* ✅ TERMINER — uniquement si confirmee et heure passée */}
                          {isConfirmee && canTerminer && (
                          ...
                          {isConfirmee && !canTerminer && (
                            <span style={{ fontSize: '0.65rem', color: '#d97706', fontStyle: 'italic', padding: '6px 12px' }}>
                              {heureFinMsg}
                            </span>
                          )}
                      ...
                      {/* ✅ MESSAGE ROUGE si paiement_recu — en attente du prof */}
                      {isPaiementRecu && (
                        <div style={{
                          display: 'flex',
                          ...
                          <div>
                            <div style={{ fontSize: '0.72rem', color: '#dc2626', fontWeight: 700, marginBottom: '2px' }}>
                              Votre paiement doit être accepté par l'enseignant avant de le contacter
                            </div>
                            <div style={{ fontSize: '0.68rem', color: muted }}>
                              Le professeur recevra une notification et devra confirmer votre réservation. Le numéro et la messagerie seront disponibles après sa validation.
                            </div>
                          </div>
                        </div>
                      )}

                      {/* Formulaire avis si terminée */}
