        ...
        <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            {loading ? (
              ...
              <>
                {/* Tabs demandes avec badge nombre d'offres */}
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  {demandes.map(d => {
                    const nbOffres = d.offres?.length || 0;
                    const isActive = activeTab === d.id_demande;
                    return (
                      <button key={d.id_demande} onClick={() => { setActiveTab(d.id_demande); fetchOffres(d.id_demande); }} style={{ position: 'relative', padding: '8px 16px', borderRadius: '10px', border: '1px solid ' + (isActive ? '#e04f00' : border), background: isActive ? '#e04f00' : 'transparent', color: isActive ? '#ffffff' : muted, fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'monospace' }}>
                        {d.matiere}
                        {nbOffres > 0 && (
                      ...
                      activeDemande.offres.map((o, i) => {
                        ...
                        return (
                          <div key={o.id_offre} style={{ padding: '1rem', borderRadius: '12px', background: bgSurf, border: '1px solid ' + border, marginBottom: i < activeDemande.offres.length - 1 ? '0.75rem' : 0 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
                              <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                                ...
                                <div>
                                  ...
                                  {ensId && (
                                    <button onClick={() => navigate('/teachers/' + ensId)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: muted, fontSize: '0.65rem', fontWeight: 600, padding: 0, display: 'flex', alignItems: 'center', gap: '3px', marginTop: '2px' }}>
                            ...
                            {/* Bouton contacter après acceptation */}
                            {o.statut === 'acceptee' && (
