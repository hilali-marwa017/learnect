...
export default function StudentMessages() {
  ...
  // ID de reservation passé depuis Offres ou Reservations via navigate state
    ...
    async function fetchConversations() {
      try {
        ...
        if (confirmed.length > 0) {
          // Si un reservationId est passé, ouvrir la bonne conversation
          if (targetReservationId) {
            const target = confirmed.find(r => r.id_reservation === targetReservationId);
            setActiveRes(target || confirmed[0]);
          } else {
            setActiveRes(confirmed[0]);
          }
        }
  ...
  useEffect(() => {
    ...
    async function fetchMessages() {
      try {
        ...
        // Marquer comme lu - on ignore l'erreur si la colonne est_lu n'existe pas encore
                  ...
                  filteredReservations.map(r => {
                    ...
                    return (
                      <button
                        ...
                        style={{ width: '100%', textAlign: 'left', padding: '1rem', display: 'flex', gap: '12px', alignItems: 'flex-start', background: isActive ? activeConvBg : 'transparent', borderTop: 'none', borderRight: 'none', borderBottom: '1px solid ' + borderSub, borderLeft: isActive ? '3px solid #e04f00' : '3px solid transparent', cursor: 'pointer', transition: 'all 0.15s' }}
                      >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          {pId && (
                            <span
                              onClick={e => { e.stopPropagation(); navigate('/teachers/' + pId); }}
                              style={{ fontSize: '0.62rem', color: '#2563eb', cursor: 'pointer', textDecoration: 'underline', display: 'flex', alignItems: 'center', gap: '2px' }}
                            >
            ...
            {activeRes ? (
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid ' + border, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', background: bgCard }}>
                  ...
                  {profId && (
                    <button
                      onClick={() => navigate('/teachers/' + profId)}
