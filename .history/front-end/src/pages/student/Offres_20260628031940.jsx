...
import { Award, Check, X, Clock, CheckCircle, MessageSquare, ExternalLink } from 'lucide-react';
...
export default function StudentOffres() {
  ...
  // ✅ FILTRE : compter seulement les offres en attente avec des enseignants valides
  const getOffresValides = (offres) => {
    return (offres || []).filter(o => 
      o.enseignant && o.enseignant.user && o.enseignant.user.prenom
    );
  };
            ...
            {loading ? (
              ...
              <>
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  {demandes.map(d => {
                    const offresValides = getOffresValides(d.offres);
                    const nbOffresEnAttente = offresValides.filter(o => o.statut === 'en_attente').length;
                ...
                {activeDemande && (
                  <div style={{ background: bgCard, border: '1px solid ' + border, borderRadius: '20px', padding: '1.5rem' }}>
                    <h3 style={{ fontSize: '0.8rem', fontWeight: 700, color: text, fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 1rem', paddingBottom: '0.75rem', borderBottom: '1px solid ' + borderSub }}>
                      Propositions pour : {activeDemande.matiere || 'Demande'}
                    </h3>
                    ...
                    {!activeDemande.offres ? (
                      ...
                      (() => {
                        const offresValides = getOffresValides(activeDemande.offres);
                        if (offresValides.length === 0) {
                          return <div style={{ textAlign: 'center', padding: '2rem', color: muted, fontSize: '0.8rem' }}>Aucune offre valide pour cette demande.</div>;
                        }
                        return offresValides.map((o, i) => {
                          const ensId = o.enseignant?.utilisateur_id || o.id_enseignant;
                          return (
                            <div key={o.id_offre} style={{ padding: '1rem', borderRadius: '12px', background: bgSurf, border: '1px solid ' + border, marginBottom: i < offresValides.length - 1 ? '0.75rem' : 0 }}>
                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
                                <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                                  <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: '#e04f00', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                    <span style={{ color: '#fff', fontSize: '0.8rem', fontWeight: 700 }}>{o.enseignant?.user?.prenom?.charAt(0)}{o.enseignant?.user?.nom?.charAt(0)}</span>
                                  </div>
                                  <div>
                                    <div style={{ fontSize: '0.85rem', fontWeight: 800, color: text }}>{o.enseignant?.user?.prenom} {o.enseignant?.user?.nom}</div>
                                    <div style={{ fontSize: '0.7rem', color: '#e04f00', fontWeight: 700 }}>{o.enseignant?.titre || 'Professeur'}</div>
                                    {ensId && (
                                      <button onClick={() => navigate('/teachers/' + ensId)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#e04f00', fontSize: '0.65rem', fontWeight: 600, padding: 0, display: 'flex', alignItems: 'center', gap: '3px', marginTop: '2px', textDecoration: 'underline' }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                  <span style={{ fontSize: '1rem', fontWeight: 900, color: text }}>{o.prix} MAD/h</span>
                                  <span style={{ fontSize: '0.6rem', padding: '2px 8px', borderRadius: '20px', fontWeight: 700, fontFamily: 'monospace', background: o.statut === 'acceptee' ? 'rgba(34,197,94,0.1)' : o.statut === 'refusee' ? 'rgba(220,38,38,0.08)' : 'rgba(245,158,11,0.1)', color: o.statut === 'acceptee' ? '#16a34a' : o.statut === 'refusee' ? '#dc2626' : '#d97706' }}>
                              ...
                              {o.statut === 'acceptee' && (
                                <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end', marginTop: '8px' }}>
                                  <button onClick={() => navigate('/student/messages')} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 16px', borderRadius: '10px', background: '#e04f00', border: 'none', color: '#fff', fontSize: '0.75rem', fontWeight: 800, cursor: 'pointer', fontFamily: 'monospace' }}>
