...
export default function StudentOffres() {
  ...
  const fetchOffres = async (id_demande) => {
    try {
      // ✅ CORRIGE : URL correcte sans /demande/
      const res = await api.get('/offres/' + id_demande);
        ...
        <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            {loading ? (
              ...
              <>
                {/* onglets des demandes avec titre */}
                <div>
                  <div style={{ fontSize: '0.65rem', color: muted, fontWeight: 700, fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.75rem' }}>
                    Vos Demandes
                  </div>
                ...
                {/* liste des offres avec titre */}
                {activeDemande && (
                  <div style={{ background: bgCard, border: '1px solid ' + border, borderRadius: '20px', padding: '1.5rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', paddingBottom: '0.75rem', borderBottom: '1px solid ' + borderSub }}>
                      <h3 style={{ fontSize: '0.8rem', fontWeight: 700, color: text, fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '0.08em', margin: 0 }}>
                        Offres reçues pour : {activeDemande.matiere}
                      </h3>
                      <span style={{ fontSize: '0.6rem', color: muted, fontFamily: 'monospace' }}>
                        {(activeDemande.offres || []).length} offre{(activeDemande.offres || []).length > 1 ? 's' : ''}
                      </span>
                    </div