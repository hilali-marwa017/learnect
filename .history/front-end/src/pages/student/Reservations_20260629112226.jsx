...
export default function StudentReservations() {
  ...
  const peutTerminer = (reservation) => {
    ...
    heureFinDate.setHours(parseInt(heures), parseInt(minutes), 0);
    return new Date() >= heureFinDate;
  };
  ...
  async function handlePaiementConfirme() {
    ...
    try {
      ...
      setReservations(prev => prev.map(r => r.id_reservation === id ? { ...r, statut: 'paiement_recu' } : r));
      setConfirmedInfo({
        id_reservation: id,
  ...
  const getJourFrancais = (jour) => {
    const jours = { monday: 'Lundi', tuesday: 'Mardi', wednesday: 'Mercredi', thursday: 'Jeudi', friday: 'Vendredi', saturday: 'Samedi', sunday: 'Dimanche' };
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1.5rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            {/* ✅ BANDEAU CORRIGÉ - PAIEMENT REÇU = PAS DE CONTACTER */}
            {confirmedInfo && (() => {
              const whatsappNum = confirmedInfo.whatsapp ? confirmedInfo.whatsapp.replace(/[^0-9]/g, '') : '';
              const isPaiementRecu = confirmedInfo.statut === 'paiement_recu';
              
              return (
                <div style={{ 
                  background: isPaiementRecu ? 'rgba(168,85,247,0.06)' : 'rgba(34,197,94,0.06)',
                  border: isPaiementRecu ? '1px solid rgba(168,85,247,0.25)' : '1px solid rgba(34,197,94,0.25)',
                  borderRadius: '16px', 
                  ...
                  <div>
                    <div style={{ 
                      fontSize: '0.6rem', 
                      color: isPaiementRecu ? '#a855f7' : '#16a34a',
                      ...
                      {isPaiementRecu ? '⏳ PAIEMENT REÇU - EN ATTENTE DU PROFESSEUR' : '✓ RÉSERVATION CONFIRMÉE'}
                    </div>
                    <p style={{ fontSize: '0.82rem', fontWeight: 700, color: text, margin: '0 0 4px' }}>
                      {isPaiementRecu ? 'Votre paiement a été reçu avec succès !' : 'Félicitations ! Votre cours est confirmé.'}
                    </p>
                    <p style={{ fontSize: '0.75rem', color: muted, margin: 0 }}>
                      {isPaiementRecu 
                        ? 'Le professeur doit maintenant confirmer votre réservation. Vous serez notifié dès que ce sera fait.'
                        : 'Votre cours est confirmé. Vous pouvez contacter le professeur.'}
                    </p>
                  </div>
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    {/* ✅ BOUTON CONTACTER UNIQUEMENT SI CONFIRMÉ (pas paiement_recu) */}
                    {!isPaiementRecu && (
                      <button
                ...
                {reservations.map(r => {
                  ...
                  return (
                    <div key={r.id_reservation}
                      style={{ background: bgCard, border: '1px solid ' + (isJustConfirmed ? 'rgba(168,85,247,0.3)' : border), borderRadius: '16px', padding: '1.25rem 1.5rem', transition: 'border 0.3s' }}>
...
/* analyser les 6 composants : dashboard, message offres,profile,requests,reservations  */