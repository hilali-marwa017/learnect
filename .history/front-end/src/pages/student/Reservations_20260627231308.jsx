...
import { Calendar, Trash2, Check, MessageCircle, Phone, Star } from 'lucide-react';
...
function AvisForm({ id_reservation, id_enseignant, isDark, text, muted, border }) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setDone(true);
    } catch (e) { setErr(e.response?.data?.message || 'Erreur lors de la publication.'); }
      ...
      <div style={{ display: 'flex', gap: '2px' }}>
        {[1,2,3,4,5].map(i => (
          ...
          >
            <Star
              size={20}
              fill={(hover || note) >= i ? '#f59e0b' : 'transparent'}
              color={(hover || note) >= i ? '#f59e0b' : '#d1d5db'}
            />
          </button>
        ))}
      </div>
...
export default function StudentReservations() {
  ...
  const [paymentTarget, setPaymentTarget] = useState(null); // { id_reservation, montant, methode }
  ...
  // ✅ CORRIGE : ouvre le modal de paiement realiste au lieu d'appeler directement l'API
                ...
                <div style={{ background: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.25)', borderRadius: '16px', padding: '1.25rem 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
                  <div>
                    ...
                    <p style={{ fontSize: '0.82rem', fontWeight: 700, color: text, margin: '0 0 4px' }}>
                      Felicitations ! Votre cours est confirme.
                    </p>
                  ...
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    <button onClick={() => setConfirmedInfo(null)} style={{ padding: '9px 12px', borderRadius: '10px', background: 'transparent', border: '1px solid ' + border, color: muted, fontSize: '0.75rem', cursor: 'pointer' }}>
                      ✕
                    </button>
                  </div>
                </div>
