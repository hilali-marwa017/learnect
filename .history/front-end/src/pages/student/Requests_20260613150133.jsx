          ...
          <div className="flex-grow space-y-8">
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {requests.map((req) => (
                  <div key={req.id_demande} className="bg-surface-card border border-hairline-strong rounded-xl p-6 space-y-4 shadow-sm">
                    ...
                    <div className="pt-3 border-t border-hairline flex items-center justify-between text-[11px] text-mute">
                      <span>Budget : <strong className="text-ink">{req.budgetMin} – {req.budgetMax} MAD/h</strong></span>
                      <span className="flex items-center gap-1.5"><Sparkles className="h-3.5 w-3.5 text-accent-yellow" /><strong>{req.offres?.length || 0} offres reçues</strong></span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="text-center py-16 bg-surface-card border border-hairline rounded-xl max-w-md mx-auto space-y-4 p-8">
                ...
                <p className="text-charcoal text-xs">Aucune demande publiée pour l'instant.</p>
              </div>
            )}
          </div>
        </div>
