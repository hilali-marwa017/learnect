                  ...
                  {reservations.slice(0, 3).map(function(r) {
                    return (
                      <div key={r.id_reservation} className="p-4 bg-surface-deep/40 border border-hairline rounded-xl flex items-center justify-between gap-4 flex-col sm:flex-row">
                        ...
                        <div><span className={`px-2.5 py-1 rounded text-[10px] font-bold ${r.statut === 'confirmee' ? 'bg-accent-green/10 text-accent-green' : 'bg-accent-yellow/10 text-accent-yellow'}`}>{r.statut === 'confirmee' ? 'Confirmé' : 'En attente'}</span></div>
                      </div>
                    );
                  })}
                </div>
              ) : (
                <div className="p-8 text-center bg-surface-deep/20 rounded-xl"><p className="text-xs text-mute">Aucun cours réservé.</p></div>
              )}
            </div>
          </div>
        </div>
