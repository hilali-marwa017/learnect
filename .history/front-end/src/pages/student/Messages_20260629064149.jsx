...
export default function StudentMessages() {
  ...
  // ✅ AFFICHAGE SEULEMENT - Formater le statut avec accent
  ...
  const loadConvs = async () => {
    try {
      ...
      // ✅ Backend utilise 'confirmee' (sans accent)
      ...
      if (grouped.length > 0) {
        let target = null;
        if (targetId) {
          target = grouped.find(c => c.reservations.some(r => r.id_reservation === Number(targetId)));
        }
  ...
  useEffect(() => {
    ...
    const fetchMsgs = async () => {
      try {
        ...
        const res = await api.get('/messages/' + first.id_reservation);
        setMsgs(res.data || []);
        try {
  ...
  const toggleRes = (id) => {
    setShowRes(prev => ({ ...prev, [id]: !prev[id] }));
  };
    ...
    <div style={{ minHeight: '100vh', background: bg, paddingTop: '2rem', paddingBottom: '4rem' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1.5rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        <div style={{ borderBottom: '1px solid ' + border, paddingBottom: '1.5rem' }}>
          ...
          <h1 style={{ fontSize: '2rem', fontWeight: 900, color: colorText, margin: '0 0 0.4rem', letterSpacing: '-0.02em' }}>
            ...
            <div style={{ borderRight: '1px solid ' + border, display: 'flex', flexDirection: 'column', background: bgSurf }}>
              <div style={{ padding: '1rem', borderBottom: '1px solid ' + border, display: 'flex', alignItems: 'center', gap: '8px', background: bgCard }}>
                ...
                <input
                  ...
                  style={{ background: 'none', border: 'none', outline: 'none', color: colorText, fontSize: '0.78rem', width: '100%' }}
                />
              </div>
                        ...
                        >
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                              <div style={{ fontSize: '0.82rem', fontWeight: 700, color: isActive ? '#e04f00' : colorText, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                {fullName}
                              </div>
                        ...
                        {isActive && nb > 1 && (
                          <div style={{ padding: '0 1rem 0.75rem 1rem', background: isActive ? activeBg : 'transparent', borderBottom: '1px solid ' + borderSub }}>
                            <button
                              ...
                              style={{ display: 'flex', alignItems: 'center', gap: '4px', background: 'none', border: 'none', color: muted, fontSize: '0.65rem', cursor: 'pointer', padding: '4px 0' }}
                            >
                            ...
                            {expanded && (
                              <div style={{ marginTop: '6px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                {c.reservations.map((r, idx) => (
                                  <div key={r.id_reservation} style={{ fontSize: '0.6rem', color: muted, padding: '4px 8px', background: isDark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.03)', borderRadius: '4px', borderLeft: idx === 0 ? '2px solid #e04f00' : '2px solid transparent' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid ' + border, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', background: bgCard }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <div>
                      <div style={{ fontSize: '0.85rem', fontWeight: 800, color: colorText }}>
                      ...
                      return (
                        <div key={msg.id_message || i} style={{ display: 'flex', flexDirection: 'column', alignItems: isMe ? 'flex-end' : 'flex-start', maxWidth: '80%', alignSelf: isMe ? 'flex-end' : 'flex-start' }}>
                          <div style={{ display: 'flex', alignItems: 'flex-end', gap: '6px' }}>
                            ...
                            <div style={{ padding: '10px 14px', borderRadius: isMe ? '16px 16px 4px 16px' : '16px 16px 16px 4px', background: isMe ? '#e04f00' : msgBg, color: isMe ? '#ffffff' : colorText, fontSize: '0.82rem', lineHeight: 1.5, fontWeight: 500, border: isMe ? 'none' : '1px solid ' + border }}>
                              {msg.contenu}
                            </div>
                          </div>
                ...
                <div style={{ padding: '1rem 1.25rem', borderTop: '1px solid ' + border, display: 'flex', gap: '10px', background: bgCard }}>
                  <input
                    ...
                    style={{ flex: 1, background: inputBg, border: '1px solid ' + inputBorder, color: colorText, borderRadius: '10px', padding: '10px 14px', fontSize: '0.82rem', outline: 'none' }}
                  />
                  <button
                    onClick={handleSend}
                    disabled={sendLoad}
