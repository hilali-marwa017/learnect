...
export default function StudentReservations() {
  ...
  // ✅ UNIQUEMENT CETTE FONCTION A ÉTÉ MODIFIÉE
  function openPaiement(r) {
    const montantNum = parseFloat(r.montant);
    setPaymentTarget({
      id_reservation: r.id_reservation,
      montant: r.montant,
      // cash seulement si vraiment 0, sinon simulation (carte)
      methode: (!montantNum || montantNum === 0) ? 'cash' : 'simulation',
    });
  }
            ...
            {confirmedInfo && (() => {
              ...
              return (
                <div style={{ background: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.25)', borderRadius: '16px', padding: '1.25rem 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
                  <div>
                    <div style={{ fontSize: '0.6rem', color: '#16a34a', fontFamily: 'monospace', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '4px' }}>✓ RÉSERVATION CONFIRMÉE</div>
                    <p style={{ fontSize: '0.82rem', fontWeight: 700, color: text, margin: '0 0 4px' }}>Félicitations ! Votre cours est confirmé.</p>
                    <p style={{ fontSize: '0.75rem', color: muted, margin: 0 }}>
                      {confirmedInfo.methode === 'cash'
                        ? 'Payez ' + confirmedInfo.montant + ' DH en espèces lors du cours.'
                        : 'Paiement par carte de ' + confirmedInfo.montant + ' DH confirmé.'}
                    </p>
                  </div>
