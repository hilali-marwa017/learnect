...
// navigation sidebar etudiant avec badge offres et messages
export function StudentNavigationActive({ activeTab, isDark }) {
  ...
  // badge offres = notifs type offre non lues
  const [offresCount, setOffresCount] = useState(0);
  // badge messages = notifs type message non lues
  ...
  useEffect(() => {
    api.get('/notifications')
      .then(res => {
        ...
        setOffresCount(notifs.filter(n => n.type === 'offre'   && !n.est_lue).length);
        setMsgCount(notifs.filter(n => n.type === 'message' && !n.est_lue).length);
      })
      .catch(() => {});
  }, []);
...
export default function StudentDashboard() {
  ...
  // charger les reservations de l'etudiant
  useEffect(() => {
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1.5rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        <div style={{ background: bgSurf, border: '1px solid ' + border, borderRadius: '24px', padding: '2rem', position: 'relative', overflow: 'hidden', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <p style={{ fontSize: '0.8rem', color: muted, margin: 0 }}>Bienvenue dans votre espace. Suivez vos cours et progressez a votre rythme.</p>
          </div>
          ...
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div style={{ background: bgCard, border: '1px solid ' + border, borderRadius: '20px', padding: '1.5rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem', paddingBottom: '1rem', borderBottom: '1px solid ' + borderSub }}>
                ...
                <Link to="/student/reservations" style={{ fontSize: '0.72rem', color: '#e04f00', fontWeight: 700, textDecoration: 'none' }}>Voir toutes ({reservations.length})</Link>
              </div>
              ...
              {loading ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem', padding: '2.5rem', background: isDark ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.02)', border: '1px solid ' + borderSub, borderRadius: '16px' }}>
                  ...
                  <Link to="/teachers" style={{ background: 'rgba(224,79,0,0.08)', border: '1px solid rgba(224,79,0,0.2)', color: '#e04f00', padding: '8px 16px', borderRadius: '8px', fontSize: '0.75rem', fontWeight: 700, textDecoration: 'none' }}>Decouvrir les professeurs</Link>
                </div>
              ) : (
                  ...
                  {reservations.slice(0, 3).map((r, i) => (
                    <div key={r.id_reservation} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', padding: '1rem', borderRadius: '12px', flexWrap: 'wrap', borderBottom: i < 2 ? '1px solid ' + borderSub : 'none' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                        <div style={{ fontSize: '0.9rem', fontWeight: 800, color: text }}>{r.creneau?.enseignant?.user?.prenom} {r.creneau?.enseignant?.user?.nom}</div>
                        <div style={{ fontSize: '0.72rem', color: muted, fontFamily: 'monospace' }}>{r.creneau?.jour} — {r.creneau?.heureDebut?.slice(0,5)} a {r.creneau?.heureFin?.slice(0,5)} — Le {r.date}</div>
                      </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              ...
              <div style={{ background: bgCard, border: '1px solid ' + border, borderRadius: '16px', padding: '1.25rem' }}>
                ...
                <p style={{ fontSize: '0.75rem', color: muted, margin: 0, lineHeight: 1.6 }}>Preparez vos bilans scolaires a l'avance pour maximiser l'efficacite du cours avec votre tuteur.</p>
              </div>
            </div>
          </div>
        </div>
