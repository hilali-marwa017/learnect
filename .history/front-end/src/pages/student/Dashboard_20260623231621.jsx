...
import { Calendar, MessageSquare, Award, BookOpen, Heart, LogOut, Settings, Sparkles } from 'lucide-react'
...
export function StudentNavigationActive({ activeTab }) {
  ...
  const links = [
    ...
    { label: 'Offres reçues', path: '/student/offres', icon: Award, id: 'offres' },
    { label: 'Mes Demandes', path: '/student/requests', icon: Sparkles, id: 'requests' },
    { label: 'Mon Profil', path: '/student/profile', icon: Settings, id: 'profile' },
  ]
          ...
          <div className="flex-1 space-y-8">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              <div className="bg-surface-card border border-hairline-strong p-6 rounded-xl flex items-center gap-4">
                <div className="p-3 bg-accent-orange-glow border border-accent-orange/15 rounded-lg text-accent-orange">
              ...
              <div className="bg-surface-card border border-hairline-strong p-6 rounded-xl flex items-center gap-4">
                <div className="p-3 bg-accent-blue-glow border border-accent-blue/15 rounded-lg text-accent-blue">
              ...
              <div className="bg-surface-card border border-hairline-strong p-6 rounded-xl flex items-center gap-4">
                <div className="p-3 bg-accent-red-glow border border-accent-red/15 rounded-lg text-accent-red">
                  <Heart className="h-5 w-5 fill-accent-red" />
                </div>
                <div>
                  ...
                  {reservations.map((r) => (
                    <div key={r.id_reservation}
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-charcoal mt-0.5 font-mono">
                          Le {r.date} — {r.creneau?.jour} {r.creneau?.heureDebut}
                        </p>
                      </div>
                      <span className={`px-2.5 py-1 rounded text-[10px] font-bold font-mono uppercase border ${
                        r.statut === 'confirmee' ? 'bg-accent-green/10 border-accent-green/20 text-accent-green' :
                        r.statut === 'en_attente' ? 'bg-accent-yellow/10 border-accent-yellow/20 text-accent-yellow' :
