...
import { Calendar, Trash2, Check, MessageCircle, Phone, Star, CheckCheck, X } from 'lucide-react';
...
function AvisForm({ id_reservation, id_enseignant, isDark, text, muted, border }) {
  ...
  const [avisExistant, setAvisExistant] = useState(null); // { id_avis, note, commentaire }
  ...
  useEffect(() => {
    async function checkAvis() {
      try {
        ...
        if (!peutNoter) {
          // Deja note — chercher l'avis existant pour cette reservation
          setDejaNote(true);
          try {
  ...
  // Avis deja publie — afficher avec bouton supprimer
                ...
                {reservations.map(r => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* AvisForm — uniquement si terminee */}
