  ...
  useEffect(() => {
    async function fetchDemandes() {
      try {
        ...
        const data = (res.data || []).filter(d => d.matiere && d.matiere.trim() !== '' && d.matiere !== 'test' && d.matiere !== 'TEST');
        setDemandes(data);
              ...
              <>
                ...
                {activeDemande && (
                  <div style={{ background: bgCard, border: '1px solid ' + border, borderRadius: '20px', padding: '1.5rem' }}>
                    ...
                    ) : activeDemande.offres.filter(o => o.enseignant && o.enseignant.user && o.enseignant.user.prenom).length === 0 ? (
                      <div style={{ textAlign: 'center', padding: '2rem', color: muted, fontSize: '0.8rem' }}>Aucune offre valide pour cette demande.</div>
                    ) : (
                      activeDemande.offres.filter(o => o.enseignant && o.enseignant.user && o.enseignant.user.prenom).map((o, i) => {
