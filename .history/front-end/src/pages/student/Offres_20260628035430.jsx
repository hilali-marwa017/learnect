...
export default function StudentOffres() {
  ...
  // Trouver la reservation confirmee liee a un enseignant pour ouvrir le bon chat
  function getReservationConfirmee(enseignantId) {
    return reservations.find(r =>
      r.statut === 'confirmee' &&
      (r.creneau?.enseignant?.utilisateur_id === enseignantId || r.creneau?.id_enseignant === enseignantId)
    );
  }

  function handleContacter(offre) {
    const ensId = offre.enseignant?.utilisateur_id || offre.id_enseignant;
    const res = getReservationConfirmee(ensId);
    if (res) {
      navigate('/student/messages', { state: { reservationId: res.id_reservation } });
    } else {
                  ...
                  <div style={{ background: bgCard, border: '1px solid ' + border, borderRadius: '20px', padding: '1.5rem' }}>
                    ...
                    {!activeDemande.offres ? (
                      ...
                      activeDemande.offres.map((o, i) => {
                        ...
                        // Message de l'offre tronque proprement
                        ...
                        return (
                          <div key={o.id_offre} style={{ padding: '1.25rem', borderRadius: '14px', background: bgSurf, border: '1px solid ' + border, marginBottom: i < activeDemande.offres.length - 1 ? '0.75rem' : 0 }}>
                            ...
                            {/* Message de proposition — affiché seulement si non vide */}
                            {msgOffre && (
