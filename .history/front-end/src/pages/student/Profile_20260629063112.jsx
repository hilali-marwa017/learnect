...
export default function StudentProfile() {
  ...
  useEffect(() => {
    async function fetchProfile() {
      try {
        ...
        if (d.photo) {
          setPhotoPreview(getPhotoUrl(d.photo));
  ...
  async function handleSubmit(e) {
    ...
    try {
      // 1. Envoyer les données du profil
      ...
      if (photoFile) {
        ...
        const res = await api.post('/etudiant/photo', fd, { 
          headers: { 'Content-Type': 'multipart/form-data' } 
        });
        
        // ✅ FORCER LE RAFFRAÎCHISSEMENT DE LA PHOTO
        if (res.data?.photo) {
          const newPhoto = res.data.photo.startsWith('http') 
            ? res.data.photo 
            : 'http://localhost:8000/storage/' + res.data.photo;
          setPhotoPreview(newPhoto + '?v=' + Date.now());
          setPhotoKey(Date.now());
        }
        setPhotoFile(null);
      }
          ...
          <div style={{ flex: 1, minWidth: 0, background: bgCard, border: `1px solid ${border}`, borderRadius: '20px', padding: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1rem', background: bgInner, borderRadius: '12px', border: `1px solid ${borderSub}`, marginBottom: '1.5rem' }}>
              ...
              <div>
                  ...
                  <div style={{ fontSize: '0.65rem', color: '#e04f00', fontStyle: 'italic', marginTop: '4px' }}>
                    📸 Cliquez sur l'icône pour changer la photo
                  </div>
                  {photoFile && (
                    <div style={{ fontSize: '0.65rem', color: '#16a34a', marginTop: '2px', fontWeight: 600 }}>
                      ✅ Photo sélectionnée - sera sauvegardée avec le profil
                    </div>
                  )}
                </div>
            </div>
