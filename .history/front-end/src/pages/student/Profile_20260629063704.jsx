...
export default function StudentProfile() {
  const { user, updateUser } = useAuth(); // ✅ Récupérer updateUser
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (photoFile) {
        ...
        // ✅ IMPORTANT : Mettre à jour le contexte Auth
        if (res.data?.photo) {
          // Mettre à jour le user dans le contexte avec la nouvelle photo
          if (updateUser) {
          ...
          setPhotoPreview(photoUrl + '?v=' + Date.now());
        }
        setPhotoFile(null);
      } else if (updateUser) {
        // ✅ Si pas de photo, mettre à jour les autres infos
        updateUser({ ...form });
      }
