...
function AvisForm({ id_reservation, id_enseignant, isDark, text, muted, border }) {
  ...
  if (done) return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.72rem', color: '#16a34a', fontWeight: 700, marginTop: '8px' }}>
      <Star size={13} fill="#16a34a" color="#16a34a" /> Avis publié avec succès
    </div>
  );

  async function handleSubmit() {
    if (note === 0)              { setErr('Veuillez choisir une note.'); return; }
    if (commentaire.length < 10) { setErr('Commentaire trop court (min 10 caractères).'); return; }
                ...
                {reservations.map(r => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Formulaire avis — visible uniquement si terminée */}
