...
export default function StudentMessages() {
  ...
  // conversations groupées par enseignant
  const [grouped, setGrouped] = useState([]);
  // conversation active = { enseignantId, enseignantUser, reservations[] }
  ...
  // Charger et grouper les conversations par enseignant
  useEffect(() => {
    async function fetchConversations() {
      try {
        ...
        // Grouper par utilisateur_id de l'enseignant
        const map = {};
        data.forEach(r => {
          const ensId = r.creneau?.enseignant?.utilisateur_id;
          if (!ensId) return;
          if (!map[ensId]) {
            map[ensId] = {
              enseignantId: ensId,
              enseignantUser: r.creneau?.enseignant?.user,
              reservations: [],
            };
          }
          map[ensId].reservations.push(r);
        });
  ...
  async function handleSend(e) {
    ...
    setSending(true);
    try {
      // Envoyer sur la dernière réservation confirmée du groupe
      const lastReservation = activeGroup.reservations[activeGroup.reservations.length - 1];
      const res = await api.post('/messages', {
        contenu: typedText,
        id_destinataire: activeGroup.enseignantId,
  ...
  const filteredGroups = grouped.filter(g => {
    const ens = g.enseignantUser;
    return `${ens?.prenom || ''} ${ens?.nom || ''}`.toLowerCase().includes(search.toLowerCase());
  });

  const enseignantDisplay = activeGroup?.enseignantUser;
              ...
              <div style={{ flex: 1, overflowY: 'auto' }}>
                {loading ? (
                  ...
                  filteredGroups.map(g => {
                    const isActive = activeGroup?.enseignantId === g.enseignantId;
                    const ens = g.enseignantUser;
                    return (
                      <button
                        key={g.enseignantId}
                ...
                <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid ' + border, display: 'flex', alignItems: 'center', gap: '12px', background: bgCard }}>
                  <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: '#e04f00', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <span style={{ color: '#fff', fontSize: '0.8rem', fontWeight: 700 }}>
                      {enseignantDisplay?.prenom?.charAt(0)}{enseignantDisplay?.nom?.charAt(0)}
                    </span>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.85rem', fontWeight: 800, color: text }}>
                      {enseignantDisplay?.prenom} {enseignantDisplay?.nom}
                    </div>
