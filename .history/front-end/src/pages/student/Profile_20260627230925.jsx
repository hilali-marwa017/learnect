...
import { Save, User, Phone, Mail, GraduationCap, MapPin, X, Camera } from 'lucide-react';
...
export default function StudentProfile() {
  ...
  useEffect(() => {
    async function fetchProfile() {
      try {
        ...
        if (d.photo) {
          setPhotoPreview(d.photo.startsWith('http') ? d.photo : 'http://localhost:8000/storage/' + d.photo);
        }
      } catch (e) { console.error(e); }
    }
    fetchProfile();
  ...
  // ✅ NOUVEAU : upload immediat de la photo
  async function handlePhotoUpload() {
    ...
    try {
      ...
      await api.post('/etudiant/photo', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
      setSaveSuccess(true);
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1.5rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 0, background: bgCard, border: `1px solid ${border}`, borderRadius: '20px', padding: '1.5rem' }}>
            ...
            {/* ✅ NOUVEAU : bloc photo de profil */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1rem', background: bgInner, borderRadius: '12px', border: `1px solid ${borderSub}`, marginBottom: '1.5rem' }}>
              <div style={{ position: 'relative', flexShrink: 0 }}>
                <div style={{ width: '72px', height: '72px', borderRadius: '50%', background: 'rgba(224,79,0,0.1)', border: '2px solid rgba(224,79,0,0.3)', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {photoPreview
                    ...
                    : <span style={{ fontSize: '1.4rem', fontWeight: 900, color: '#e04f00' }}>{form.prenom?.charAt(0)}{form.nom?.charAt(0)}</span>
                  }
                </div>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: '0.9rem', fontWeight: 800, color: text }}>{form.prenom} {form.nom}</div>
                <div style={{ fontSize: '0.75rem', color: muted, marginTop: '2px' }}>{user?.email}</div>
              </div>
              {photoFile && (
                <button onClick={handlePhotoUpload} disabled={uploadingPhoto} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 16px', borderRadius: '8px', background: uploadingPhoto ? '#e5e7eb' : '#e04f00', color: uploadingPhoto ? muted : '#fff', border: 'none', fontSize: '0.72rem', fontWeight: 700, cursor: uploadingPhoto ? 'not-allowed' : 'pointer', fontFamily: 'monospace', flexShrink: 0 }}>
                  <Save size={13} /> {uploadingPhoto ? 'Envoi...' : 'Sauvegarder la photo'}
                </button>
              )}
            </div>
