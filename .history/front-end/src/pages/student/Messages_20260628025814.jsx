...
import { Send, Search, CheckCheck, MessageSquare, Trash2, ExternalLink } from 'lucide-react';
...
export default function StudentMessages() {
  ...
  useEffect(() => {
    async function fetchConversations() {
      try {
        ...
        if (confirmed.length > 0) setActiveRes(confirmed[0]);
  ...
  useEffect(() => {
    ...
    async function fetchMessages() {
      try {
        ...
        try { await api.put('/messages/' + activeRes.id_reservation + '/lu'); } catch (e) {}
      } catch (e) { console.error(e); }
    }
    fetchMessages();
  }, [activeRes]);
  ...
  async function handleSend(e) {
    ...
    setSending(true);
    try {
      const destinataire = activeRes.creneau?.enseignant?.utilisateur_id || activeRes.creneau?.id_enseignant;
  ...
  const filteredReservations = reservations.filter(r => {
    const nom = (r.creneau?.enseignant?.user?.prenom || '') + ' ' + (r.creneau?.enseignant?.user?.nom || '');
    return nom.toLowerCase().includes(search.toLowerCase());
  });

  const prof  = activeRes?.creneau?.enseignant?.user;
  const profId = activeRes?.creneau?.enseignant?.utilisateur_id || activeRes?.creneau?.id_enseignant;
    ...
    <div style={{ minHeight: '100vh', background: bg, paddingTop: '2rem', paddingBottom: '4rem' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1.5rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        <div style={{ borderBottom: '1px solid ' + border, paddingBottom: '1.5rem' }}>
          ...
          <h1 style={{ fontSize: '2rem', fontWeight: 900, color: text, margin: '0 0 0.4rem', letterSpacing: '-0.02em' }}>Messagerie Securisee</h1>
          <p style={{ fontSize: '0.8rem', color: muted, margin: 0 }}>Echangez avec vos professeurs apres confirmation de reservation.</p>
        </div>
          ...
          <div style={{ flex: 1, minWidth: 0, background: bgCard, border: '1px solid ' + border, borderRadius: '20px', overflow: 'hidden', display: 'grid', gridTemplateColumns: '280px 1fr', minHeight: '500px' }}>
            ...
            <div style={{ borderRight: '1px solid ' + border, display: 'flex', flexDirection: 'column', background: bgSurf }}>
              <div style={{ padding: '1rem', borderBottom: '1px solid ' + border, display: 'flex', alignItems: 'center', gap: '8px', background: bgCard }}>
                ...
                <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher un professeur..." style={{ background: 'none', border: 'none', outline: 'none', color: text, fontSize: '0.78rem', width: '100%' }} />
              </div>
              ...
              <div style={{ flex: 1, overflowY: 'auto' }}>
                {loading ? (
                  ...
                  <div style={{ padding: '2rem', textAlign: 'center', color: muted, fontSize: '0.75rem' }}>Aucune conversation disponible</div>
                ) : (
                  filteredReservations.map(r => {
                    const isActive = activeRes?.id_reservation === r.id_reservation;
                    const p = r.creneau?.enseignant?.user;
                    const pId = r.creneau?.enseignant?.utilisateur_id || r.creneau?.id_enseignant;
                    return (
                      <button key={r.id_reservation} onClick={() => setActiveRes(r)} style={{ width: '100%', textAlign: 'left', padding: '1rem', display: 'flex', gap: '12px', alignItems: 'flex-start', background: isActive ? activeConvBg : 'transparent', borderTop: 'none', borderRight: 'none', borderBottom: '1px solid ' + borderSub, borderLeft: isActive ? '3px solid #e04f00' : '3px solid transparent', cursor: 'pointer', transition: 'all 0.15s' }}>
                        <div style={{ width: '38px', height: '38px', borderRadius: '50%', background: '#e04f00', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <span style={{ color: '#fff', fontSize: '0.85rem', fontWeight: 700 }}>{p?.prenom?.charAt(0)}{p?.nom?.charAt(0)}</span>
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: '0.82rem', fontWeight: 700, color: text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginBottom: '3px' }}>{p?.prenom} {p?.nom}</div>
                          <div style={{ fontSize: '0.65rem', color: '#e04f00', fontWeight: 700, fontFamily: 'monospace', textTransform: 'uppercase', marginBottom: '2px' }}>
                            {r.creneau?.jour} {r.creneau?.heureDebut?.slice(0, 5)} a {r.creneau?.heureFin?.slice(0, 5)}
                          </div>
                          {pId && (
                            <span onClick={e => { e.stopPropagation(); navigate('/teachers/' + pId); }} style={{ fontSize: '0.62rem', color: '#2563eb', cursor: 'pointer', textDecoration: 'underline', display: 'flex', alignItems: 'center', gap: '2px' }}>
            ...
            {activeRes ? (
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid ' + border, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', background: bgCard }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: '#e04f00', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <span style={{ color: '#fff', fontSize: '0.8rem', fontWeight: 700 }}>{prof?.prenom?.charAt(0)}{prof?.nom?.charAt(0)}</span>
                    </div>
                    <div>
                      <div style={{ fontSize: '0.85rem', fontWeight: 800, color: text }}>{prof?.prenom} {prof?.nom}</div>
                      <div style={{ fontSize: '0.65rem', color: '#e04f00', fontWeight: 700, fontFamily: 'monospace', textTransform: 'uppercase' }}>
                        {activeRes.creneau?.jour} — {activeRes.creneau?.heureDebut?.slice(0, 5)} a {activeRes.creneau?.heureFin?.slice(0, 5)}
                      </div>
                    </div>
                  </div>
                  {profId && (
                    <button onClick={() => navigate('/teachers/' + profId)} style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '6px 12px', borderRadius: '8px', border: '1px solid ' + border, background: 'transparent', color: muted, fontSize: '0.7rem', fontWeight: 600, cursor: 'pointer' }}>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '12px', maxHeight: '380px', background: bgSurf }}>
                  {messages.length === 0 ? (
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, gap: '1rem', padding: '2rem' }}>
                      ...
                      <p style={{ fontSize: '0.8rem', color: muted, margin: 0 }}>Aucun message pour le moment. Envoyez le premier !</p>
                    </div>
                  ) : (
                    messages.map((msg, i) => {
                ...
                <div style={{ padding: '1rem 1.25rem', borderTop: '1px solid ' + border, display: 'flex', gap: '10px', background: bgCard }}>
                  <input type="text" value={typedText} onChange={e => setTypedText(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) handleSend(e); }} placeholder="Tapez votre message..." style={{ flex: 1, background: inputBg, border: '1px solid ' + inputBorder, color: text, borderRadius: '10px', padding: '10px 14px', fontSize: '0.82rem', outline: 'none' }} />
                  <button onClick={handleSend} disabled={sending} style={{ background: sending ? (isDark ? 'rgba(255,255,255,0.05)' : '#e5e7eb') : '#111827', color: sending ? muted : '#ffffff', border: 'none', borderRadius: '10px', padding: '10px 18px', cursor: sending ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <Send size={16} />
                  </button>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, gap: '1rem', color: muted }}>
                ...
                <p style={{ fontSize: '0.85rem', margin: 0 }}>Selectionnez une conversation</p>
              </div>
            )}
          </div>
        </div>
