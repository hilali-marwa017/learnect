...
export default function StudentMessages() {
  ...
  const filteredReservations = reservations.filter(r =>
    `${r.creneau?.enseignant?.user?.prenom} ${r.creneau?.enseignant?.user?.nom}`.toLowerCase().includes(search.toLowerCase())
  );
          ...
          <div style={{ flex: 1, minWidth: 0, background: bgCard, border: `1px solid ${border}`, borderRadius: '20px', overflow: 'hidden', display: 'grid', gridTemplateColumns: '280px 1fr', minHeight: '500px' }}>
              ...
              <div style={{ flex: 1, overflowY: 'auto' }}>
                {loading ? (
                  ...
                  filteredReservations.map(r => {
                    ...
                    return (
                      <button key={r.id_reservation} onClick={() => setActiveRes(r)} style={{ width: '100%', textAlign: 'left', padding: '1rem', display: 'flex', gap: '12px', alignItems: 'center', background: isActive ? activeConvBg : 'transparent', borderLeft: isActive ? '3px solid #e04f00' : '3px solid transparent', border: 'none', borderBottom: `1px solid ${borderSub}`, cursor: 'pointer', transition: 'all 0.15s' }}>
            ...
            {activeRes ? (
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <div style={{ padding: '1rem 1.25rem', borderBottom: `1px solid ${border}`, display: 'flex', alignItems: 'center', gap: '12px', background: bgCard }}>
                  <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: '#e04f00', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <span style={{ color: '#fff', fontSize: '0.8rem', fontWeight: 700 }}>
                      {activeRes.creneau?.enseignant?.user?.prenom?.charAt(0)}{activeRes.creneau?.enseignant?.user?.nom?.charAt(0)}
                    </span>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.85rem', fontWeight: 800, color: text }}>
                      {activeRes.creneau?.enseignant?.user?.prenom} {activeRes.creneau?.enseignant?.user?.nom}
                    </div>
                ...
                <form onSubmit={handleSend} style={{ padding: '1rem 1.25rem', borderTop: `1px solid ${border}`, display: 'flex', gap: '10px', background: bgCard }}>
                  <input required type="text" value={typedText} onChange={e => setTypedText(e.target.value)} placeholder="Tapez votre message..." style={{ flex: 1, background: inputBg, border: `1px solid ${inputBorder}`, color: text, borderRadius: '10px', padding: '10px 14px', fontSize: '0.82rem', outline: 'none' }} />
                  <button type="submit" disabled={sending} style={{ background: sending ? (isDark ? 'rgba(255,255,255,0.05)' : '#e5e7eb') : '#111827', color: sending ? muted : '#ffffff', border: 'none', borderRadius: '10px', padding: '10px 18px', cursor: sending ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <Send size={16} />
                  </button>
                </form>
              </div>
