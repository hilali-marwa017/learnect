...
export function StudentNavigationActive({ activeTab, isDark }) {
  ...
  const [reservationsConfirmeesCount, setReservationsConfirmeesCount] = useState(0);
  ...
  useEffect(() => {
    api.get('/reservations')
      .then(res => {
        ...
        const confirmeesNonVues = reservations.filter(r => r.statut === 'confirmee').length;
        setReservationsConfirmeesCount(confirmeesNonVues);
        const hasAccess = reservations.some(r => r.statut === 'confirmee');
        setHasMessagingAccess(hasAccess);
      })
      .catch(() => {
        setReservationsConfirmeesCount(0);
  ...
  const links = [
    ...
    { label: 'Réservations',     path: '/student/reservations', icon: Calendar,      id: 'reservations', badge: reservationsConfirmeesCount },
    { label: 'Messagerie',       path: '/student/messages',     icon: MessageSquare, id: 'messages',     badge: msgCount, disabled: !hasMessagingAccess },
    ...
    <div style={{ background: bg, border: '1px solid ' + border, borderRadius: '16px', padding: '1rem', width: '220px', flexShrink: 0 }}>
      ...
      <nav style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
        {links.map(link => {
          ...
          return isDisabled ? (
            <div key={link.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderRadius: '10px', fontSize: '0.82rem', fontWeight: 500, opacity: 0.4, cursor: 'not-allowed', color: text }}>
              <Icon size={16} style={{ flexShrink: 0 }} />
              <span style={{ flex: 1 }}>{link.label}</span>
              <span style={{ fontSize: '0.55rem', color: muted, fontFamily: 'monospace' }}>🔒</span>
            </div>
          ) : (
