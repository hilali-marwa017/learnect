...
function AvisForm({ id_reservation, id_enseignant, isDark, text, muted, border }) {
  ...
  // Verifier si un avis existe deja pour cette reservation
  useEffect(() => {
    async function checkAvis() {
      try {
        ...
        if (!peutNoter) {
          // Plus aucune reservation a noter pour cet enseignant = deja note
          setDejaNote(true);
        } else if (idResNoter !== id_reservation) {
          // Une autre reservation peut etre notee, pas celle-ci
          setDejaNote(true);
        }
      } catch (e) {
        // En cas d erreur on laisse le formulaire visible
      } finally {
        setChecking(false);
      }
    }
    checkAvis();
  }, [id_reservation, id_enseignant]);
  ...
  if (dejaNote) return (
    <div style={{ marginTop: '12px', padding: '10px 14px', background: isDark ? 'rgba(34,197,94,0.06)' : 'rgba(34,197,94,0.05)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: '10px', display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.72rem', color: '#16a34a', fontWeight: 700 }}>
      <Star size={13} fill="#16a34a" color="#16a34a" /> Vous avez deja laisse un avis pour ce cours.
    </div>
  );

  if (done) return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.72rem', color: '#16a34a', fontWeight: 700, marginTop: '8px' }}>
      <Star size={13} fill="#16a34a" color="#16a34a" /> Avis publie avec succes !
    </div>
  );
  ...
  async function handleSubmit() {
    ...
    try {
      await api.post('/avis', { note, commentaire, id_enseignant, id_reservation });
      setDone(true);
    } catch (e) {
                ...
                {reservations.map(r => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Formulaire avis — seulement si terminee */}
