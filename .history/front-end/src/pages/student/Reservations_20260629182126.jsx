...
// ✅ CORRIGÉ : Importer depuis student/Dashboard.jsx
import { StudentNavigationActive } from '../student/Dashboard';
...
export default function StudentMessages() {
      ...
      try {
        ...
        if (res.data.length > 0) {
          if (preselectedReservationId) {
            const matched = res.data.find(r => r.id_reservation === preselectedReservationId);
            if (matched) {
  ...
  useEffect(() => {
    ...
    async function fetchMessages() {
      try {
        // ✅ URL correcte avec /conversations/
        const responses = await Promise.all(
          [activeConv].map(r => api.get(`/messages/conversations/${r.id_reservation}`))
        );
        ...
        await Promise.all(
          [activeConv].map(r => api.put(`/messages/${r.id_reservation}/lu`).catch(() => {}))
        );
      } catch (e) {
  ...
  async function handleSend(e) {
    ...
    try {
      const res = await api.post('/messages', {
        contenu: typedText,
        id_destinataire: activeConv.creneau?.enseignant?.utilisateur_id || activeConv.id_enseignant,
        id_reservation: activeConv.id_reservation,
      });
  ...
  const filteredConversations = conversations.filter(c => {
    const enseignant = c.creneau?.enseignant?.user;
    return `${enseignant?.prenom || ''} ${enseignant?.nom || ''}`.toLowerCase().includes(search.toLowerCase());
  });

  const enseignantDisplay = activeConv?.creneau?.enseignant?.user;
              ...
              <div style={{ flex: 1, overflowY: 'auto' }}>
                {loading ? (
                  ...
                  filteredConversations.map(c => {
                    const isActive = activeConv?.id_reservation === c.id_reservation;
                    const enseignant = c.creneau?.enseignant?.user;
                    return (
                      <button key={c.id_reservation} onClick={() => setActiveConv(c)} style={{ width: '100%', textAlign: 'left', padding: '1rem', display: 'flex', gap: '12px', alignItems: 'center', background: isActive ? activeConvBg : 'transparent', borderLeft: isActive ? '3px solid #e04f00' : '3px solid transparent', border: 'none', borderBottom: '1px solid ' + borderSub, cursor: 'pointer', transition: 'all 0.15s' }}>
                        <div style={{ width: '38px', height: '38px', borderRadius: '50%', background: '#e04f00', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <span style={{ color: '#fff', fontSize: '0.85rem', fontWeight: 700 }}>{enseignant?.prenom?.charAt(0)}{enseignant?.nom?.charAt(0)}</span>
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: '0.82rem', fontWeight: 700, color: text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginBottom: '2px' }}>{enseignant?.prenom} {enseignant?.nom}</div>
                          <span style={{ fontSize: '0.65rem', color: '#e04f00', fontWeight: 700, fontFamily: 'monospace', textTransform: 'uppercase' }}>{c.creneau?.jour} — {c.creneau?.heureDebut?.slice(0,5)}</span>
                        </div>
                      </button>
                    );
                  })
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid ' + border, display: 'flex', alignItems: 'center', gap: '12px', background: bgCard }}>
                  <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: '#e04f00', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <span style={{ color: '#fff', fontSize: '0.8rem', fontWeight: 700 }}>{enseignantDisplay?.prenom?.charAt(0)}{enseignantDisplay?.nom?.charAt(0)}</span>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.85rem', fontWeight: 800, color: text }}>{enseignantDisplay?.prenom} {enseignantDisplay?.nom}</div>
                    <div style={{ fontSize: '0.65rem', color: '#e04f00', fontWeight: 700, fontFamily: 'monospace', textTransform: 'uppercase' }}>
                      {activeConv.creneau?.jour} — {activeConv.creneau?.heureDebut?.slice(0, 5)} à {activeConv.creneau?.heureFin?.slice(0, 5)}
                    </div>
                  </div>
                </div>
