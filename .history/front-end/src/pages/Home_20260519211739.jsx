...
function Home() {
  ...
  return (
    <>
                ...
                <div style={{
                  ...
                  {villes.map(v => (
                    <div key={v}
                      ...
                      style={{ padding: '12px 20px', cursor: 'pointer', fontSize: '0.93rem', fontWeight: 500 }}
          ...
          <div style={{
            ...
            {categories.map((c, i) => (
              <Link key={i} to={`/teachers?matiere=${c.label}`}
                ...
                <span style={{ fontSize: '0.75rem', fontWeight: 600, color: '#1a1a1a' }}>{c.label}</span>
              </Link>
            ))}
          </div>
        </div>
        ...
        <div className="wrap" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 24, textAlign: 'center' }}>
          {stats.map((s, i) => (
            <div key={i} className="fu" style={{ animationDelay: `${i * 0.08}s`, opacity: 0 }}>
              <div style={{ fontSize: '2.2rem', fontWeight: 900, color: '#4CAF7D', letterSpacing: -1 }}>{s.n}</div>
              <div style={{ color: '#717171', fontSize: '0.88rem', marginTop: 4, fontWeight: 500 }}>{s.l}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* ── COMMENT CA MARCHE ── */}
      <section style={{ padding: '72px 0', background: '#FFF8F0' }}>
        <div className="wrap">
