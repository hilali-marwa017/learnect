...
export function TeacherNavigationActive({ activeTab, isDark }) {
  ...
  const links = [
    ...
    { label: 'Messages',        path: '/teacher/messages',     icon: MessageSquare, id: 'messages'     },
        ...
        {links.map(link => {
          ...
          return (
            <Link
              key={link.id}
              to={link.path}
              style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderRadius: '10px', fontSize: '0.82rem', fontWeight: isActive ? 700 : 500, textDecoration: 'none', transition: 'all 0.15s', background: isActive ? '#e04f00' : 'transparent', color: isActive ? '#ffffff' : text }}
            >
...
export default function TeacherDashboard() {
  ...
  async function handleAction(id, action) {
    try {
      ...
      // Note: le prof ne confirme pas — c'est l'étudiant qui confirme le paiement
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1.5rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        <div style={{ background: bgSurf, border: `1px solid ${border}`, borderRadius: '24px', padding: '2rem', position: 'relative', overflow: 'hidden', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <h1 style={{ fontSize: '1.8rem', fontWeight: 900, color: text, margin: '0 0 0.4rem', letterSpacing: '-0.02em' }}>
              Espace Professeur : {user?.prenom} {user?.nom}
            </h1>
            <p style={{ fontSize: '0.8rem', color: muted, margin: 0 }}>Administrez vos demandes de cours et suivez vos réservations.</p>
          </div>
          ...
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div style={{ background: bgCard, border: `1px solid ${border}`, borderRadius: '20px', padding: '1.5rem' }}>
              <h3 style={{ fontSize: '0.7rem', fontWeight: 700, color: text, fontFamily: 'monospace', letterSpacing: '0.1em', textTransform: 'uppercase', margin: '0 0 1.25rem', paddingBottom: '1rem', borderBottom: `1px solid ${borderSub}` }}>
                Réservations en attente de paiement étudiant
              </h3>
              ...
              {loading ? (
                    ...
                    <div key={r.id_reservation} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', padding: '1rem', borderRadius: '12px', flexWrap: 'wrap', borderBottom: i < pending.length - 1 ? `1px solid ${borderSub}` : 'none' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                        ...
                        <div style={{ fontSize: '0.75rem', color: muted, fontFamily: 'monospace' }}>
                          Le {r.date} —{' '}
                          <span style={{ color: r.montant == 0 ? '#16a34a' : text, fontWeight: 700 }}>
                            {r.montant == 0 ? '1er cours GRATUIT' : `${r.montant} DH`}
                          </span>
                        </div>
                      </div>
                      <div style={{ display: 'flex', gap: '8px', flexShrink: 0 }}>
                        {/* Info : le prof ne confirme pas, c'est l'étudiant */}
                        ...
                        <button
                          ...
                          style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '36px', height: '36px', borderRadius: '10px', border: '1px solid rgba(239,68,68,0.3)', background: 'rgba(239,68,68,0.08)', color: '#dc2626', cursor: 'pointer' }}
                        >
                          <X size={14} />
                        </button>
                      </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem', padding: '2.5rem', background: isDark ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.02)', border: `1px solid ${borderSub}`, borderRadius: '16px' }}>
                  ...
                  <p style={{ fontSize: '0.8rem', color: muted, fontWeight: 600, margin: 0 }}>Aucune réservation en attente.</p>
                </div>
              )}
            </div>
            ...
            <div style={{ background: bgCard, border: `1px solid ${border}`, borderRadius: '20px', padding: '1.25rem 1.5rem', display: 'flex', alignItems: 'flex-start', gap: '1rem' }}>
              ...
              <div>
                <div style={{ fontSize: '0.8rem', fontWeight: 700, color: text, marginBottom: '4px' }}>Consigne — Première heure de cours</div>
                <p style={{ fontSize: '0.75rem', color: muted, margin: 0, lineHeight: 1.6 }}>
                  Le premier entretien avec l'élève est toujours offert sur Learnect. Déterminez ensemble ses objectifs et mettez en place un programme sur-mesure.
                </p>
              </div>
            </div>
