...
export default function TeacherMessages() {
      ...
      try {
        ...
        if (res.data.length > 0) {
          if (preselectedReservationId) {
            ...
            setActiveConv(matched || res.data[0]);
          } else {
  ...
  useEffect(() => {
    ...
    async function fetchMessages() {
      try {
        const res = await api.get(`/messages/conversations/${activeConv.id_reservation}`);
        const msgs = (res.data || []).sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
        setMessages(msgs);
        await api.put(`/messages/${activeConv.id_reservation}/lu`).catch(() => {});
      } catch (e) {
  ...
  const filteredConversations = conversations.filter(c => {
    const etudiant = c.etudiant;
    return `${etudiant?.prenom || ''} ${etudiant?.nom || ''}`.toLowerCase().includes(search.toLowerCase());
  });

  const etudiantDisplay = activeConv?.etudiant;
                    ...
                    return (
                      <button key={c.id_reservation} onClick={() => setActiveConv(c)} style={{ width: '100%', textAlign: 'left', padding: '1rem', display: 'flex', gap: '12px', alignItems: 'center', background: isActive ? activeConvBg : 'transparent', borderLeft: isActive ? '3px solid #e04f00' : '3px solid transparent', border: 'none', borderBottom: '1px solid ' + borderSub, cursor: 'pointer', transition: 'all 0.15s' }}>
                        <div style={{ width: '38px', height: '38px', borderRadius: '50%', background: '#e04f00', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <span style={{ color: '#fff', fontSize: '0.85rem', fontWeight: 700 }}>{etudiant?.prenom?.charAt(0)}{etudiant?.nom?.charAt(0)}</span>
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: '0.82rem', fontWeight: 700, color: text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginBottom: '2px' }}>{etudiant?.prenom} {etudiant?.nom}</div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid ' + border, display: 'flex', alignItems: 'center', gap: '12px', background: bgCard }}>
                  <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: '#e04f00', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <span style={{ color: '#fff', fontSize: '0.8rem', fontWeight: 700 }}>{etudiantDisplay?.prenom?.charAt(0)}{etudiantDisplay?.nom?.charAt(0)}</span>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.85rem', fontWeight: 800, color: text }}>{etudiantDisplay?.prenom} {etudiantDisplay?.nom}</div>
