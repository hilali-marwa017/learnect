...
export default function TeacherAvailability() {
  ...
  const inputStyle = { width: '100%', background: inputBg, border: `1px solid ${inputBorder}`, color: inputColor, borderRadius: '10px', padding: '10px 12px', fontSize: '0.8rem', outline: 'none', cursor: 'pointer' };
  ...
  // Ouvrir mode modification
  function handleEdit(creneau) {
    setEditId(creneau.id_creneau);
    setEditJour(creneau.jour);
    setEditDebut(creneau.heureDebut?.slice(0,5));
    setEditFin(creneau.heureFin?.slice(0,5));
    setError('');
  }
  ...
  async function handleSaveEdit(id) {
    ...
    try {
      const res = await api.put(`/creneaux/${id}`, { jour: editJour, heureDebut: editDebut, heureFin: editFin });
    ...
    <div style={{ minHeight: '100vh', background: bg, paddingTop: '2rem', paddingBottom: '4rem' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1.5rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        <div style={{ borderBottom: `1px solid ${border}`, paddingBottom: '1.5rem' }}>
          ...
          <h1 style={{ fontSize: '2rem', fontWeight: 900, color: text, margin: '0 0 0.4rem', letterSpacing: '-0.02em' }}>Disponibilités & Calendrier</h1>
        ...
        <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              ...
              <form onSubmit={handleAdd} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr auto', gap: '1rem', alignItems: 'end' }}>
                <div>
                  ...
                  <select value={newJour} onChange={e => setNewJour(e.target.value)} style={{ ...inputStyle, background: isDark ? '#2a2a2c' : '#f8f9fc' }}>
                    {JOURS.map(j => <option key={j} value={j} style={{ background: isDark ? '#1a1a1c' : '#ffffff' }}>{JOURS_LABELS[j]}</option>)}
                  </select>
                </div>
                <div>
                  ...
                  <select value={newDebut} onChange={e => setNewDebut(e.target.value)} style={{ ...inputStyle, background: isDark ? '#2a2a2c' : '#f8f9fc' }}>
                    {TIME_SLOTS.map(t => <option key={t} value={t} style={{ background: isDark ? '#1a1a1c' : '#ffffff' }}>{t}</option>)}
                  </select>
                </div>
                <div>
            ...
            <div style={{ background: bgCard, border: `1px solid ${border}`, borderRadius: '20px', padding: '1.5rem' }}>
              ...
              {loading ? (
                <div style={{ textAlign: 'center', padding: '2rem', color: muted, fontSize: '0.8rem' }}>Chargement des créneaux...</div>
              ) : (
                <div>
                  {JOURS.map((jour, i) => {
                    ...
                    return (
                      <div key={jour} style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem', padding: '14px 0', borderBottom: i < JOURS.length - 1 ? `1px solid ${borderSub}` : 'none', flexWrap: 'wrap' }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', flex: 1 }}>
                          {slots.length === 0 ? (
                            <span style={{ fontSize: '0.72rem', color: isDark ? 'rgba(107,114,128,0.5)' : '#d1d5db', fontStyle: 'italic', fontFamily: 'monospace' }}>Aucun créneau</span>
                          ) : (
                            slots.map(c => (
                              <div key={c.id_creneau}>
                                ...
                                {editId === c.id_creneau ? (
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: isDark ? 'rgba(255,255,255,0.05)' : '#f1f5f9', border: `1px solid ${border}`, borderRadius: '10px', padding: '6px 10px' }}>
                                    <select value={editDebut} onChange={e => setEditDebut(e.target.value)} style={{ background: isDark ? '#2a2a2c' : '#ffffff', border: `1px solid ${inputBorder}`, color: text, borderRadius: '6px', padding: '4px 6px', fontSize: '0.75rem', outline: 'none' }}>
                                      {TIME_SLOTS.map(t => <option key={t} value={t}>{t}</option>)}
                                    </select>
                                    <span style={{ color: muted, fontSize: '0.75rem' }}>→</span>
                                    <select value={editFin} onChange={e => setEditFin(e.target.value)} style={{ background: isDark ? '#2a2a2c' : '#ffffff', border: `1px solid ${inputBorder}`, color: text, borderRadius: '6px', padding: '4px 6px', fontSize: '0.75rem', outline: 'none' }}>
                                      {TIME_SLOTS.map(t => <option key={t} value={t}>{t}</option>)}
                                    </select>
                                    {/* Bouton sauvegarder */}
              ...
              <div style={{ marginTop: '1rem', paddingTop: '1rem', borderTop: `1px solid ${borderSub}` }}>
                <p style={{ fontSize: '0.7rem', color: muted, fontFamily: 'monospace', margin: 0, display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <Calendar size={12} color="#e04f00" /> Cliquez sur un créneau pour le rendre disponible / indisponible. Les créneaux <span style={{ color: '#e04f00', fontWeight: 700 }}>orange</span> sont actifs et réservables.
                </p>
              </div>
            </div>
