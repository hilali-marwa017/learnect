...
export default function TeacherProfile() {
  ...
  // ✅ NOUVEAU : refresh user apres upload photo
  ...
  async function handleSubmit(e) {
    ...
    setError('');
    try {
      // mettre a jour le profil enseignant + user
      const payload = {
        ...form,
        ...userForm
      };
      const res = await api.put('/enseignant/profil', payload);
      ...
      if (photoFile) {
        ...
        const photoRes = await api.post('/enseignant/photo', fd, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
        // ✅ mettre a jour l'utilisateur apres upload photo
        if (setUser && photoRes.data.user) {
  ...
  async function handlePhotoUpload() {
    ...
    try {
      const fd = new FormData();
      fd.append('photo', photoFile);
      const res = await api.post('/enseignant/photo', fd, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
      if (setUser && res.data.user) {
        setUser(res.data.user);
      ...
      if (photo) {
        setPhotoPreview(photo.startsWith('http') ? photo : 'http://localhost:8000/storage/' + photo);
      }
      setSaveSuccess(true);
      setTimeout(() => setSaveSuccess(false), 2500);
    } catch (err) {
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1rem', background: bgInner, borderRadius: '12px', border: '1px solid ' + borderSub }}>
                ...
                {photoFile && (
                  <button
                    ...
                    disabled={loading}
                    style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 16px', borderRadius: '8px', background: loading ? '#e5e7eb' : '#e04f00', color: loading ? muted : '#fff', border: 'none', fontSize: '0.72rem', fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'monospace', flexShrink: 0 }}
                  >
                    <Save size={13} /> {loading ? 'Envoi...' : 'Sauvegarder la photo'}
                  </button>
                )}
              </div>
