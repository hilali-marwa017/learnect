...
// fonction pure: initiales
const getInitials = (prenom, nom) => {
  return (prenom?.charAt(0) || '') + (nom?.charAt(0) || '');
};

// fonction pure: formater le telephone
  ...
  return (
    <div>
      ...
      <select value={value} onChange={onChange} style={baseStyle}>
        ...
        {options.map(opt => (
          <option key={opt.id || opt} value={opt.nom || opt} style={{ background: isDark ? '#1a1a1c' : '#ffffff' }}>
            {opt.nom || opt}
          </option>
        ))}
      </select>
...
function FormCheckbox({ label, name, checked, onChange }) {
  ...
  return (
    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '0.82rem', color: text, fontWeight: 500 }}>
      <input type="checkbox" name={name} checked={checked} onChange={onChange} style={{ accentColor: '#e04f00', width: '16px', height: '16px' }} />
      {label}
    </label>
  );
}
