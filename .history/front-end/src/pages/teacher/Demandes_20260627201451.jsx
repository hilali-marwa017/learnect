...
export default function TeacherDemandes() {
  ...
  useEffect(() => {
    async function fetchAll() {
      try {
        const [demRes, offRes] = await Promise.all([
  ...
  async function handleEnvoyer(id_demande) {
    if (!prix || !message.trim()) { setError('Veuillez renseigner un prix et un message.'); return; }
    ...
    try {
      ...
      setFormOpenId(null);
      setPrix(''); setMessage('');
    } catch (e) { setError(e.response?.data?.message || "Erreur lors de l'envoi."); }
    ...
    <div style={{ minHeight: '100vh', background: bg, paddingTop: '2rem', paddingBottom: '4rem' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1.5rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        <div style={{ borderBottom: `1px solid ${border}`, paddingBottom: '1.5rem' }}>
          ...
          <h1 style={{ fontSize: '2rem', fontWeight: 900, color: text, margin: '0 0 0.4rem', letterSpacing: '-0.02em' }}>Demandes des Etudiants</h1>
          <p style={{ fontSize: '0.8rem', color: muted, margin: 0 }}>Parcourez les demandes actives et envoyez vos propositions personnalisees.</p>
        </div>
                  ...
                  <div key={d.id_demande} style={{ background: bgCard, border: `1px solid ${border}`, borderRadius: '16px', padding: '1.25rem 1.5rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
                      <div>
                        ...
                        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', marginBottom: '6px' }}>
                          <span style={{ fontSize: '0.72rem', color: muted, display: 'flex', alignItems: 'center', gap: '4px' }}><MapPin size={12} color="#e04f00" /> {d.ville}</span>
                          <span style={{ fontSize: '0.72rem', color: muted, display: 'flex', alignItems: 'center', gap: '4px' }}><Wallet size={12} color="#e04f00" /> Budget max : {d.budgetMax} MAD/h</span>
                        </div>
                      ...
                      {postule ? (
                        <span style={{ fontSize: '0.65rem', padding: '6px 14px', borderRadius: '20px', fontWeight: 700, fontFamily: 'monospace', background: 'rgba(34,197,94,0.1)', color: '#16a34a', display: 'flex', alignItems: 'center', gap: '4px', flexShrink: 0 }}>
                          <CheckCircle size={12} /> OFFRE ENVOYEE
                        </span>
                      ) : (
                        <button
                          onClick={() => { setFormOpenId(isFormOpen ? null : d.id_demande); setError(''); }}
                          style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 16px', borderRadius: '10px', background: isFormOpen ? 'transparent' : '#e04f00', border: isFormOpen ? `1px solid ${border}` : 'none', color: isFormOpen ? muted : '#fff', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'monospace', flexShrink: 0 }}
                        >
                    ...
                    {isFormOpen && !postule && (
                      <div style={{ marginTop: '1rem', padding: '1rem', background: bgSurf, border: `1px solid ${border}`, borderRadius: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        ...
                        <input
                          type="number" min="0" value={prix} onChange={e => setPrix(e.target.value)}
                          placeholder="Votre prix propose (MAD/h)"
                          style={{ background: inputBg, border: `1px solid ${inputBorder}`, color: text, borderRadius: '8px', padding: '8px 12px', fontSize: '0.78rem', outline: 'none' }}
                        />
                        <textarea
                          value={message} onChange={e => setMessage(e.target.value)}
                          placeholder="Presentez-vous et votre methode pedagogique..."
                          rows={2}
                          style={{ background: inputBg, border: `1px solid ${inputBorder}`, color: text, borderRadius: '8px', padding: '8px 12px', fontSize: '0.78rem', outline: 'none', resize: 'vertical' }}
                        />
                        <button
                          onClick={() => handleEnvoyer(d.id_demande)}
                          disabled={sending}
                          style={{ alignSelf: 'flex-end', padding: '7px 16px', borderRadius: '8px', background: sending ? '#e5e7eb' : '#e04f00', color: sending ? muted : '#fff', border: 'none', fontSize: '0.72rem', fontWeight: 800, cursor: sending ? 'not-allowed' : 'pointer', fontFamily: 'monospace' }}
                        >
