...
export default function TeacherMessages() {
  ...
  useEffect(() => {
    async function fetchConversations() {
      try {
        ...
        (res.data || []).forEach(creneau => {
          (creneau.reservations || []).forEach(r => {
            if (r.statut === 'confirmee') {
              // ✅ FIX : r.etudiant est directement un User (pas .user imbriqué)
              all.push({ ...r, creneau, etudiant_display: r.etudiant || {} });
            }
          });
        });
        ...
        const groups = {};
        all.forEach(r => {
          const studId = r.id_utilisateur;
          if (!groups[studId]) {
            groups[studId] = { id_utilisateur: studId, etudiant: r.etudiant_display, reservations: [] };
          }
          groups[studId].reservations.push(r);
        });

        const list = Object.values(groups).map(g => ({
          ...g,
          reservations: g.reservations.sort((a, b) => new Date(b.created_at) - new Date(a.created_at)),
        }));

        setConversations(list);
        if (list.length > 0) setActiveConv(list[0]);
      } catch (e) {
        console.error(e);
      } finally {
        setLoading(false);
  ...
  useEffect(() => {
    ...
    async function fetchMessages() {
      try {
        const responses = await Promise.all(activeConv.reservations.map(r => api.get(`/messages/${r.id_reservation}`)));
        const merged = responses.flatMap(res => res.data || []);
        merged.sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
        setMessages(merged);

        await Promise.all(activeConv.reservations.map(r => api.put(`/messages/${r.id_reservation}/lu`).catch(() => {})));
      } catch (e) {
        console.error(e);
      }
    }
  ...
  async function handleSend(e) {
    ...
    setSending(true);
    try {
      const latestReservation = activeConv.reservations[0];
      const res = await api.post('/messages', {
        contenu: typedText,
        id_destinataire: activeConv.id_utilisateur,
        id_reservation: latestReservation.id_reservation,
      });
  ...
  const studDisplay = activeConv?.etudiant;
          ...
          <div style={{ flex: 1, minWidth: 0, background: bgCard, border: '1px solid ' + border, borderRadius: '20px', overflow: 'hidden', display: 'grid', gridTemplateColumns: '280px 1fr', minHeight: '500px' }}>
            ...
            <div style={{ borderRight: '1px solid ' + border, display: 'flex', flexDirection: 'column', background: bgSurf }}>
              <div style={{ padding: '1rem', borderBottom: '1px solid ' + border, display: 'flex', alignItems: 'center', gap: '8px', background: bgCard }}>
                ...
                <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher un étudiant..." style={{ background: 'none', border: 'none', outline: 'none', color: text, fontSize: '0.78rem', width: '100%' }} />
              </div>
              ...
              <div style={{ flex: 1, overflowY: 'auto' }}>
                {loading ? (
                  ...
                  filteredConversations.map(c => {
                    const isActive = activeConv?.id_utilisateur === c.id_utilisateur;
                    const d = c.etudiant;
                    return (
                      <button key={c.id_utilisateur} onClick={() => setActiveConv(c)} style={{ width: '100%', textAlign: 'left', padding: '1rem', display: 'flex', gap: '12px', alignItems: 'center', background: isActive ? activeConvBg : 'transparent', borderLeft: isActive ? '3px solid #e04f00' : '3px solid transparent', border: 'none', borderBottom: '1px solid ' + borderSub, cursor: 'pointer', transition: 'all 0.15s' }}>
                        <div style={{ width: '38px', height: '38px', borderRadius: '50%', background: '#e04f00', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <span style={{ color: '#fff', fontSize: '0.85rem', fontWeight: 700 }}>{d?.prenom?.charAt(0)}{d?.nom?.charAt(0)}</span>
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: '0.82rem', fontWeight: 700, color: text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginBottom: '2px' }}>{d?.prenom} {d?.nom}</div>
                          <span style={{ fontSize: '0.65rem', color: '#e04f00', fontWeight: 700, fontFamily: 'monospace', textTransform: 'uppercase' }}>{c.reservations.length} cours réservé{c.reservations.length > 1 ? 's' : ''}</span>
                        </div>
                      </button>
                    );
                  })
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid ' + border, display: 'flex', alignItems: 'center', gap: '12px', background: bgCard }}>
                  <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: '#e04f00', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <span style={{ color: '#fff', fontSize: '0.8rem', fontWeight: 700 }}>{studDisplay?.prenom?.charAt(0)}{studDisplay?.nom?.charAt(0)}</span>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.85rem', fontWeight: 800, color: text }}>{studDisplay?.prenom} {studDisplay?.nom}</div>
                    <div style={{ fontSize: '0.65rem', color: '#e04f00', fontWeight: 700, fontFamily: 'monospace', textTransform: 'uppercase' }}>
                      {activeConv.reservations[0]?.creneau?.jour} — {activeConv.reservations[0]?.creneau?.heureDebut?.slice(0, 5)} à {activeConv.reservations[0]?.creneau?.heureFin?.slice(0, 5)}
                    </div>
                  </div>
                </div>
