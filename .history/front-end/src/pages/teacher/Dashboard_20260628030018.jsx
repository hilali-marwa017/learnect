...
import { Calendar, DollarSign, Clock, Settings, Check, X, ShieldAlert, Star, MessageSquare, Sparkles } from 'lucide-react';
  ...
  useEffect(() => {
    async function fetchMsgCount() {
      try {
        ...
        setMsgCount(res.data?.non_lus || 0);
      } catch (e) { setMsgCount(0); }
    }
    fetchMsgCount();
  }, []);

  useEffect(() => {
    async function fetchDemandesCount() {
      try {
...
export default function TeacherDashboard() {
  ...
  function statutBadge(statut) {
    ...
    if (statut === 'confirmee')  return { bg: 'rgba(34,197,94,0.1)',  color: '#16a34a', label: 'CONFIRME' };
    if (statut === 'annulee')    return { bg: 'rgba(220,38,38,0.08)', color: '#dc2626', label: 'ANNULE' };
    if (statut === 'terminee')   return { bg: 'rgba(59,130,246,0.08)',color: '#2563eb', label: 'TERMINE' };
  ...
  const allVisible = [...pending, ...confirmed].slice(0, 10);
          ...
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div style={{ background: bgCard, border: '1px solid ' + border, borderRadius: '20px', padding: '1.5rem' }}>
              <h3 style={{ fontSize: '0.7rem', fontWeight: 700, color: text, fontFamily: 'monospace', letterSpacing: '0.1em', textTransform: 'uppercase', margin: '0 0 1.25rem', paddingBottom: '1rem', borderBottom: '1px solid ' + borderSub }}>
                Reservations actives ({allVisible.length})
              </h3>
              ...
              {loading ? (
                    ...
                    return (
                      <div key={r.id_reservation} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', padding: '1rem', borderRadius: '12px', flexWrap: 'wrap', borderBottom: i < allVisible.length - 1 ? '1px solid ' + borderSub : 'none' }}>
                        ...
                        <div style={{ display: 'flex', gap: '8px', flexShrink: 0, alignItems: 'center' }}>
                          <span style={{ fontSize: '0.65rem', padding: '4px 10px', borderRadius: '20px', fontWeight: 700, fontFamily: 'monospace', background: s.bg, color: s.color }}>
                            {s.label}
                          </span>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem', padding: '2.5rem', background: isDark ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.02)', border: '1px solid ' + borderSub, borderRadius: '16px' }}>
                  ...
                  <p style={{ fontSize: '0.8rem', color: muted, fontWeight: 600, margin: 0 }}>Aucune reservation active actuellement.</p>
                </div>
              )}
            </div>
