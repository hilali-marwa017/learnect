...
function FormTextarea({ label, name, value, onChange, placeholder, rows = 3 }) {
  ...
  return (
    <div>
      <label style={labelStyle}>{label}</label>
      <textarea name={name} value={value || ''} onChange={onChange} placeholder={placeholder} rows={rows} style={baseStyle} required />
    </div>
  );
}
...
export default function TeacherProfile() {
  ...
  // ✅ AJOUT distance_max dans le state form
    ...
    api.get('/enseignants/' + user.utilisateur_id)
      .then(res => {
        ...
        setForm({
          ...
          distance_max:        e.distance_max         || '', // ✅ AJOUT
        });
        setUserForm({
          nom:       e.user?.nom       || '',
          ...
          <div style={{ flex: 1, minWidth: 0, background: bgCard, border: '1px solid ' + border, borderRadius: '20px', padding: '1.5rem' }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              <div>
                ...
                {/* ✅ AJOUT : Distance max — visible uniquement si cours_domicile coché */}
                {form.cours_domicile && (
                  <div style={{ marginTop: '1rem', maxWidth: '300px' }}>
                    ...
                    <p style={{ fontSize: '0.7rem', color: muted, marginTop: '4px', marginBottom: 0 }}>
