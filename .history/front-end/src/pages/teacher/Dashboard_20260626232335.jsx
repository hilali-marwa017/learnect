...
// Navigation sidebar enseignant avec badge messages non lus
export function TeacherNavigationActive({ activeTab, isDark }) {
  ...
  // compteur messages non lus depuis les notifications
  const [msgCount, setMsgCount] = useState(0);

  useEffect(() => {
    // recuperer le nombre de notifications non lues de type message
    api.get('/notifications')
      .then(res => {
        const nonLus = (res.data || []).filter(n => n.type === 'message' && !n.est_lue).length;
        setMsgCount(nonLus);
      })
      .catch(() => setMsgCount(0));
  }, []);
        ...
        {links.map(link => {
          ...
          return (
            <Link key={link.id} to={link.path} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderRadius: '10px', fontSize: '0.82rem', fontWeight: isActive ? 700 : 500, textDecoration: 'none', transition: 'all 0.15s', background: isActive ? '#e04f00' : 'transparent', color: isActive ? '#ffffff' : text }}>
              ...
              {/* badge visible seulement si msgCount > 0 */}
              {link.badge > 0 && (
...
// Dashboard enseignant principal
export default function TeacherDashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        // charger dashboard enseignant
        const dashRes = await api.get('/enseignant/dashboard');
        setStats(dashRes.data.stats);
        // charger reservations enseignant
  ...
  // annuler une reservation cote prof
        ...
        <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
              ...
              {metricCard(c => <Star size={20} color={c} />, 'rgba(251,191,36,0.1)', '#d97706', 'Note du profil', (stats?.note_moyenne ?? 0) + ' / 5')}
            </div>
                  ...
                  {pending.map((r, i) => (
                    <div key={r.id_reservation} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', padding: '1rem', borderRadius: '12px', flexWrap: 'wrap', borderBottom: i < pending.length - 1 ? '1px solid ' + borderSub : 'none' }}>
                      ...
                      <div style={{ display: 'flex', gap: '8px', flexShrink: 0 }}>
                        ...
                        <button onClick={() => handleAnnuler(r.id_reservation)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '36px', height: '36px', borderRadius: '10px', border: '1px solid rgba(239,68,68,0.3)', background: 'rgba(239,68,68,0.08)', color: '#dc2626', cursor: 'pointer' }}>
                          <X size={14} />
                        </button>
                      </div>
                    </div>
            ...
            <div style={{ background: bgCard, border: '1px solid ' + border, borderRadius: '20px', padding: '1.25rem 1.5rem', display: 'flex', alignItems: 'flex-start', gap: '1rem' }}>
              ...
              <div>
                ...
                <p style={{ fontSize: '0.75rem', color: muted, margin: 0, lineHeight: 1.6 }}>Le premier entretien avec l'eleve est toujours offert sur Learnect. Determinez ensemble ses objectifs, evaluez ses points faibles et mettez en place un programme sur-mesure pour stimuler sa reussite.</p>
              </div>
            </div>
