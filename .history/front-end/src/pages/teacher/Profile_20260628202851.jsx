...
function FormInput({ label, name, value, onChange, placeholder, type = 'text', required = false, disabled = false, icon: Icon, style = {} }) {
...
export default function TeacherProfile() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ CORRIGE : on fusionne immediatement les champs renvoyes par
      // /enseignant/profil (nom, prenom, telephone, ville a jour) dans le
      // contexte d'auth, pour que la Navbar et le reste de l'app se mettent
      // a jour sans attendre la photo ni un F5
      ...
      if (photoFile) {
        ...
        if (photoRes?.user) {
          // ✅ CORRIGE : updateUser fusionne + persiste dans localStorage +
          // met a jour le contexte React partage, donc la Navbar recoit la
          // nouvelle photo immediatement, sans F5
          ...
          <div style={{ flex: 1, minWidth: 0, background: bgCard, border: '1px solid ' + border, borderRadius: '20px', padding: '1.5rem' }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <FormInput label="Prenom" name="prenom" value={userForm.prenom} onChange={handleUserFormChange} required />
                <FormInput label="Nom de famille" name="nom" value={userForm.nom} onChange={handleUserFormChange} required />
                <FormInput label="Adresse email" value={user?.email || ''} disabled icon={Mail} />
                <FormInput label="Telephone (Maroc)" name="telephone" value={userForm.telephone} onChange={handleUserFormChange} placeholder="06 XX XX XX XX" icon={Phone} required />
                <FormInput label="Titre de l'annonce" name="titre" value={form.titre} onChange={handleFormChange} placeholder="Ex: Professeur agree de SVT" required />
                <FormInput label="Tarif Horaire (MAD / heure)" name="tarifHeure" value={form.tarifHeure} onChange={handleFormChange} placeholder="150" type="number" required style={{ fontFamily: 'monospace' }} />
                <FormSelect label="Ville principale" name="ville" value={userForm.ville} onChange={handleUserFormChange} options={villes} placeholder="Selectionner une ville" />
                <FormInput label="Langues d'enseignement" name="langues" value={form.langues} onChange={handleFormChange} placeholder="Ex: Francais, Arabe, Anglais" />
              </div>
              <div>
              ...
              <FormTextarea label="Description du cours" name="description_cours" value={form.description_cours} onChange={handleFormChange} placeholder="Decrivez votre methode pedagogique..." rows={3} />
              <FormTextarea label="Description biographique publique" name="description_profil" value={form.description_profil} onChange={handleFormChange} placeholder="Experiences, diplomes, parcours academique..." rows={3} />
