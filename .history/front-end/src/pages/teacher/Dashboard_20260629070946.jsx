      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1.5rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        <div style={{ background: bgSurf, border: '1px solid ' + border, borderRadius: '24px', padding: '2rem', position: 'relative', overflow: 'hidden', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', gap: '8px', alignItems: 'flex-end' }}>
            <div style={{ background: 'rgba(34,197,94,0.15)', border: '1px solid rgba(34,197,94,0.3)', borderRadius: '999px', padding: '6px 16px', fontSize: '0.7rem', fontWeight: 700, color: '#4ade80', fontFamily: 'monospace', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <Check size={12} /> COMPTE VÉRIFIÉ
            </div>
            {pendingPaiement.length > 0 && (
              <div style={{ background: 'rgba(168,85,247,0.15)', border: '1px solid rgba(168,85,247,0.3)', borderRadius: '999px', padding: '6px 16px', fontSize: '0.7rem', fontWeight: 700, color: '#c084fc', fontFamily: 'monospace', display: 'flex', alignItems: 'center', gap: '6px' }}>
        ...
        <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            {pendingPaiement.length > 0 && (
              <div style={{ background: 'rgba(168,85,247,0.1)', border: '1px solid rgba(168,85,247,0.25)', borderRadius: '14px', padding: '1rem 1.25rem', display: 'flex', alignItems: 'center', gap: '12px' }}>
                      ...
                      return (
                        <div key={r.id_reservation} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', padding: '1rem', borderRadius: '12px', flexWrap: 'wrap', borderBottom: i < arr.length - 1 ? '1px solid ' + borderSub : 'none', background: r.statut === 'paiement_recu' ? (isDark ? 'rgba(168,85,247,0.08)' : 'rgba(168,85,247,0.03)') : 'transparent' }}>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                            <span style={{ fontSize: '0.65rem', fontWeight: 700, color: '#60a5fa', background: 'rgba(59,130,246,0.15)', border: '1px solid rgba(59,130,246,0.3)', borderRadius: '6px', padding: '2px 8px', fontFamily: 'monospace', textTransform: 'uppercase', width: 'fit-content' }}>
                            ...
                            <div style={{ fontSize: '0.75rem', color: muted, fontFamily: 'monospace' }}>
                              Le {r.date} —{' '}
                              <span style={{ color: !montantNum ? '#4ade80' : text, fontWeight: 700 }}>
                          ...
                          <div style={{ display: 'flex', gap: '8px', flexShrink: 0, alignItems: 'center', flexWrap: 'wrap' }}>
                            ...
                            {r.statut === 'paiement_recu' && (
                              <>
                                <button onClick={() => handleAccepter(r.id_reservation)} disabled={!!isLoading} style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '7px 14px', borderRadius: '8px', border: '1px solid rgba(34,197,94,0.4)', background: 'rgba(34,197,94,0.15)', color: '#4ade80', fontSize: '0.72rem', fontWeight: 800, cursor: isLoading ? 'not-allowed' : 'pointer', fontFamily: 'monospace' }}>
                                  <CheckCircle2 size={13} /> {actionLoading === r.id_reservation + '_accepter' ? 'En cours...' : 'Accepter'}
                                </button>
                                <button onClick={() => handleRefuser(r.id_reservation)} disabled={!!isLoading} style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '7px 14px', borderRadius: '8px', border: '1px solid rgba(220,38,38,0.3)', background: 'rgba(220,38,38,0.15)', color: '#f87171', fontSize: '0.72rem', fontWeight: 800, cursor: isLoading ? 'not-allowed' : 'pointer', fontFamily: 'monospace' }}>
                      ...
                      return (
                        <div key={p.id_paiement} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', padding: '1rem', flexWrap: 'wrap', borderBottom: i < paiements.length - 1 ? '1px solid ' + borderSub : 'none' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                            <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: 'rgba(224,79,0,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
