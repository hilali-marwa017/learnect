...
import { CheckCircle, XCircle, Trash2, Loader, FileText, Image, Phone, ShieldOff } from 'lucide-react';
...
function DocLink({ doc }) {
    ...
    return (
        <a href={url} target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 14px', borderRadius: '8px', border: '1px solid var(--color-border)', background: 'var(--color-bg)', textDecoration: 'none', color: '#e04f00', fontSize: '0.72rem', fontWeight: 600 }}>
...
// correction : badge dedie pour chaque statut possible (actif / en_attente / bloque / refuse)
function StatutBadge({ statut }) {
    const config = {
        actif:      { label: 'ACTIF',      bg: 'rgba(16,185,129,0.1)', color: '#10b981' },
        en_attente: { label: 'EN ATTENTE', bg: 'rgba(245,158,11,0.1)', color: '#f59e0b' },
        bloque:     { label: 'BLOQUÉ',     bg: 'rgba(220,38,38,0.1)',  color: '#dc2626' },
        refuse:     { label: 'REFUSÉ',     bg: 'rgba(107,114,128,0.12)', color: '#6b7280' },
    };
    const c = config[statut] || config.bloque;
    return (
        <span style={{ fontSize: '0.65rem', padding: '3px 8px', borderRadius: '20px', fontWeight: 700, background: c.bg, color: c.color }}>
            {c.label}
        </span>
    );
}
...
export function UsersTab({ users, setUsers }) {
    ...
    async function bloquer(userId, statut) {
        ...
        const message = isActif ? 'Voulez-vous vraiment bloquer cet utilisateur ?' : 'Voulez-vous vraiment débloquer cet utilisateur ?';
        if (!window.confirm(message)) return;
        setLoadingId(userId);
        try {
            const action = isActif ? 'bloquer' : 'debloquer';
            await api.put(`/admin/users/${userId}/${action}`);
            setUsers(prev => prev.map(u => {
                if (u.utilisateur_id === userId) {
                    return { ...u, statut: isActif ? 'bloque' : 'actif' };
                }
                return u;
            }));
        } catch (e) {
            // correction : afficher le vrai message renvoye par le backend
            // (ex: tentative de debloquer un compte 'refuse')
            ...
            <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    ...
                    <tbody>
                        {users.length === 0 ? (
                            <tr>
                                <td colSpan={5} style={{ textAlign: 'center', padding: '2rem', color: 'var(--color-muted)' }}>Aucun utilisateur</td>
                            </tr>
                        ) : users.map((u, i) => {
                            // correction : le toggle bloquer/debloquer ne s'affiche
                            // que pour les statuts actif/bloque. Un compte refuse
                            // ou en_attente ne doit pas pouvoir etre "debloque"
                            // directement (il doit passer par Verifications).
                            const canToggleBlock = u.statut === 'actif' || u.statut === 'bloque';
                            ...
                            return (
                                <tr key={u.utilisateur_id ?? i} style={{ borderBottom: '1px solid var(--color-border)' }}>
                                    <td style={{ padding: '12px', fontSize: '0.78rem', color: 'var(--color-text)' }}>
                                        ...
                                        {u.telephone && (
                                            <div style={{ fontSize: '0.65rem', color: 'var(--color-muted)', display: 'flex', alignItems: 'center', gap: '4px', marginTop: '2px' }}>
                                                <Phone size={11} />
                                                {u.telephone}
                                            </div>
                                        )}
                                    ...
                                    <td style={{ padding: '12px', textAlign: 'right' }}>
                                        <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end' }}>
                                            {u.role !== 'admin' && canToggleBlock && (
                                                <button onClick={() => bloquer(u.utilisateur_id, u.statut)} disabled={loadingId === u.utilisateur_id} style={{ padding: '5px 12px', borderRadius: '7px', border: `1px solid ${u.statut === 'actif' ? 'rgba(220,38,38,0.25)' : 'rgba(16,185,129,0.25)'}`, background: u.statut === 'actif' ? 'rgba(220,38,38,0.06)' : 'rgba(16,185,129,0.06)', color: u.statut === 'actif' ? '#dc2626' : '#10b981', fontSize: '0.7rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px', opacity: loadingId === u.utilisateur_id ? 0.6 : 1 }}>
                                                    {loadingId === u.utilisateur_id
                                                        ? <Loader size={13} style={{ animation: 'spin 1s linear infinite' }} />
                                                        : u.statut === 'actif'
                                                            ? <><XCircle size={13} /> Bloquer</>
                                                            : <><CheckCircle size={13} /> Débloquer</>
                                                    }
                                                </button>
                                            )}
                                            {/* correction : pour un compte refuse, on affiche une info au lieu d'un bouton actif */}
                                            {u.role !== 'admin' && u.statut === 'refuse' && (
                                                <span style={{ padding: '5px 10px', borderRadius: '7px', fontSize: '0.65rem', fontWeight: 600, color: 'var(--color-muted)', display: 'flex', alignItems: 'center', gap: '5px' }}>
                                                    <ShieldOff size={13} /> Via Vérifications
                                                </span>
                                            )}
                                            {/* correction : on ne peut plus supprimer un compte admin (et donc pas son propre compte) */}
                                            {u.role !== 'admin' && (
                                                <button onClick={() => supprimerUser(u.utilisateur_id)} disabled={loadingId === u.utilisateur_id} style={{ padding: '5px 8px', borderRadius: '7px', border: '1px solid rgba(220,38,38,0.2)', background: 'rgba(220,38,38,0.06)', color: '#dc2626', cursor: 'pointer', display: 'flex', alignItems: 'center', opacity: loadingId === u.utilisateur_id ? 0.6 : 1 }}>
                                                    <Trash2 size={13} />
                                                </button>
                                            )}
                                        </div>
    ...
    return (
        <>
            <div style={{ background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: '16px', padding: '1.5rem' }}>
                ...
                <div style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--color-text)', marginBottom: '1.25rem' }}>Diplômes et Dossiers ({enseignantsAttente.length})</div>
                    ...
                    <div key={e.utilisateur_id} style={{ background: 'var(--color-bg)', border: '1px solid var(--color-border)', borderRadius: '12px', padding: '1.25rem', marginBottom: '1rem' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
                            <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                                ...
                                <div>
                                    ...
                                    {e.user?.telephone && (
                                        <div style={{ fontSize: '0.7rem', color: 'var(--color-muted)', display: 'flex', alignItems: 'center', gap: '4px', marginTop: '2px' }}>
                                            <Phone size={11} />
                                            {e.user?.telephone}
                                        </div>
                                    )}
                                    {e.titre && (
                                        <div style={{ fontSize: '0.68rem', color: '#e04f00', fontWeight: 600, marginTop: '2px' }}>{e.titre}</div>
                                    )}
                                </div>
                            </div>
                            <div style={{ display: 'flex', gap: '8px' }}>
                                ...
                                <button onClick={() => valider(e.utilisateur_id)} disabled={loadingId === e.utilisateur_id} style={{ padding: '8px 16px', borderRadius: '8px', border: 'none', background: '#e04f00', color: '#fff', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', opacity: loadingId === e.utilisateur_id ? 0.6 : 1 }}>
                        ...
                        {e.documents && e.documents.length > 0 && (
                            <div style={{ marginTop: '1rem', background: 'var(--color-card)', borderRadius: '10px', padding: '1rem', border: '1px solid var(--color-border)' }}>
                                <div style={{ fontSize: '0.6rem', color: 'var(--color-muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '10px' }}>Documents soumis</div>
                        ...
                        {e.description_profil && (
                            <div style={{ marginTop: '10px', fontSize: '0.72rem', color: 'var(--color-muted)', fontStyle: 'italic', padding: '10px', background: 'var(--color-card)', borderRadius: '8px', border: '1px solid var(--color-border)', borderLeft: '3px solid #e04f00' }}>
                                "{e.description_profil}"
                            </div>
                        )}
                    </div>
            ...
            {refusModal && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999, padding: '1rem' }}>
                    <div style={{ background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: '16px', padding: '1.5rem', width: '100%', maxWidth: '460px' }}>
                        ...
                        <textarea value={refusRaison} onChange={ev => setRefusRaison(ev.target.value)} placeholder="Le justificatif du diplôme n'est pas lisible." rows={4} style={{ width: '100%', background: 'var(--color-bg)', border: '1px solid var(--color-border)', borderRadius: '10px', padding: '12px', color: 'var(--color-text)', fontSize: '0.8rem', outline: 'none', resize: 'vertical', boxSizing: 'border-box', fontFamily: 'inherit' }} />
                        <div style={{ display: 'flex', gap: '8px', marginTop: '1rem', justifyContent: 'flex-end' }}>
                            <button onClick={() => setRefusModal(null)} style={{ padding: '8px 16px', borderRadius: '8px', border: '1px solid var(--color-border)', background: 'transparent', color: 'var(--color-muted)', fontSize: '0.78rem', cursor: 'pointer', fontWeight: 600 }}>
                                Annuler
                            </button>
                            <button onClick={() => refuser(refusModal)} disabled={!!loadingId} style={{ padding: '8px 16px', borderRadius: '8px', border: 'none', background: '#dc2626', color: '#fff', fontSize: '0.78rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', opacity: loadingId ? 0.6 : 1 }}>
