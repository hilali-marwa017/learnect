...
function ManageUsers({ users, onRefresh }) {
  ...
  const getRoleBadge = (role) => {
    if (role === 'admin') return { bg: '#f3e8ff', color: '#7e22ce', icon: 'bi-shield' };
    if (role === 'enseignant') return { bg: '#dbeafe', color: '#1d4ed8', icon: 'bi-briefcase' };
    return { bg: '#dcfce7', color: '#166534', icon: 'bi-mortarboard' };
  };
                ...
                <tr key={u.utilisateur_id} style={{ borderBottom: '1px solid #e2e8f0' }}>
                  ...
                  <td className="py-3">
                    <span className="badge px-3 py-2 rounded-pill" style={{ background: roleStyle.bg, color: roleStyle.color, fontSize: '11px', fontWeight: 'bold' }}>
                      <i className={`bi ${roleStyle.icon} me-1`}></i> {u.role === 'enseignant' ? 'Enseignant' : u.role === 'admin' ? 'Admin' : 'Étudiant'}
                    </span>
                  </td>
                    ...
                    <div className="d-flex gap-2">
                      {u.statut === 'actif' ? (
                        <button 
                          ...
                          style={{ background: '#fee2e2', color: '#dc2626', border: 'none', fontSize: '11px', fontWeight: 'bold' }}
                        >
                        ...
                        <button 
                          ...
                          disabled={isLoading} 
                          className="btn btn-sm px-3 py-1 rounded-pill"
                          style={{ background: '#dcfce7', color: '#16a34a', border: 'none', fontSize: '11px', fontWeight: 'bold' }}
                        >
                      ...
                      <button 
                        ...
                        disabled={isLoading} 
                        className="btn btn-sm px-3 py-1 rounded-pill"
                        style={{ background: '#f1f5f9', color: '#64748b', border: 'none', fontSize: '11px', fontWeight: 'bold' }}
                      >
