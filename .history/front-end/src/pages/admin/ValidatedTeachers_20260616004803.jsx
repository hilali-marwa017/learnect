...
export function ReservationsTab({ reservations }) {
  return (
    <div style={{ background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: '16px', padding: '1.5rem' }}>
      ...
      <div style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--color-text)', marginBottom: '1.25rem' }}>Réservations ({reservations.length})</div>
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>{['Étudiant', 'Enseignant', 'Montant', 'Statut', 'Date'].map(h => (
          ...
          <tbody>
            {reservations.length === 0 ? (
              <tr><td colSpan={5} style={{ textAlign: 'center', padding: '2rem', color: 'var(--color-muted)' }}>Aucune réservation</td></tr>
              ...
              <tr key={r.id_reservation ?? i} style={{ borderBottom: '1px solid var(--color-border)' }}>
                ...
                <td style={{ padding: '12px', fontSize: '0.78rem', color: 'var(--color-muted)' }}>{r.creneau?.enseignant?.user?.prenom} {r.creneau?.enseignant?.user?.nom}</td>
                ...
                <td style={{ padding: '12px', fontSize: '0.78rem', color: 'var(--color-muted)' }}>{new Date(r.created_at).toLocaleDateString('fr-FR')}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
export function PaiementsTab({ paiements }) {
  return (
    <div style={{ background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: '16px', padding: '1.5rem' }}>
      ...
      <div style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--color-text)', marginBottom: '1.25rem' }}>Paiements ({paiements.length})</div>
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>{['Montant', 'Commission (10%)', 'Statut', 'Date'].map(h => (
              <th key={h} style={{ textAlign: 'left', padding: '10px 12px', fontSize: '0.6rem', color: 'var(--color-muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', borderBottom: '1px solid var(--color-border)' }}>{h}</th>
            ))}</tr>
          </thead>
          <tbody>
            {paiements.length === 0 ? (
              <tr><td colSpan={4} style={{ textAlign: 'center', padding: '2rem', color: 'var(--color-muted)' }}>Aucun paiement</td></tr>
            ) : paiements.map((p, i) => (
              <tr key={p.id ?? i} style={{ borderBottom: '1px solid var(--color-border)' }}>
                <td style={{ padding: '12px', fontSize: '0.78rem', color: '#10b981', fontWeight: 700 }}>{p.montant} DH</td>
                <td style={{ padding: '12px', fontSize: '0.78rem', color: '#e04f00', fontWeight: 600 }}>{p.comission || 0} DH</td>
                <td style={{ padding: '12px' }}>
                ...
                <td style={{ padding: '12px', fontSize: '0.78rem', color: 'var(--color-muted)' }}>{new Date(p.created_at).toLocaleDateString('fr-FR')}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
export function DemandesTab({ demandes }) {
  return (
    <div style={{ background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: '16px', padding: '1.5rem' }}>
      <div style={{ fontSize: '0.6rem', color: '#e04f00', fontWeight: 700, letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '4px' }}>DEMANDES ÉTUDIANTS</div>
      <div style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--color-text)', marginBottom: '1.25rem' }}>Demandes ({demandes.length})</div>
      {demandes.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--color-muted)' }}>Aucune demande</div>
      ) : demandes.map((d, i) => (
        <div key={d.id ?? i} style={{ background: 'var(--color-bg)', border: '1px solid var(--color-border)', borderRadius: '10px', padding: '1rem', marginBottom: '0.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
            <div>
              <div style={{ fontWeight: 600, color: 'var(--color-text)', fontSize: '0.82rem' }}>{d.etudiant?.prenom} {d.etudiant?.nom}</div>
              <div style={{ fontSize: '0.68rem', color: 'var(--color-muted)' }}>{d.etudiant?.email}</div>
              <div style={{ fontSize: '0.73rem', color: 'var(--color-muted)', marginTop: '6px', maxWidth: '500px' }}>{d.message?.substring(0, 100)}...</div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ fontSize: '0.65rem', color: 'var(--color-muted)' }}>{new Date(d.created_at).toLocaleDateString('fr-FR')}</span>
            </div>
          </div>
        </div>
      ))}
