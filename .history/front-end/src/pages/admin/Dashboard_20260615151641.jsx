...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (donutRef.current) {
      donutInstance.current = new Chart(donutRef.current, {
        type: 'doughnut',
        data: { labels: ['Élèves', 'Tuteurs'], datasets: [{ data: [stats.totalEtudiants, stats.totalEnseignants], backgroundColor: [orange, '#1e293b'], borderWidth: 0, hoverOffset: 6 }] },
    ...
    if (barRef.current) {
      barInstance.current = new Chart(barRef.current, {
        ...
        options: {
          ...
          scales: {
            x: { grid: { color: 'rgba(128,128,128,0.1)' }, ticks: { color: muted, font: { size: 11 } }, border: { display: false } },
  ...
  return (
    <div style={{ minHeight: '100vh', background: bg, paddingTop: '50px' }}>
