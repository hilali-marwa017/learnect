        ...
        <table className="w-100" style={{ borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {users.map(u => {
              ...
              const isLoading = loadingId === u.utilisateur_id;
              return (
                <tr key={u.utilisateur_id} style={{ borderBottom: '1px solid #e2e8f0' }}>
                  ...
                  <td className="py-3" style={{ fontSize: '13px' }}>
                    <i className="bi bi-geo-alt me-1" style={{ color: '#94a3b8' }}></i> {u.ville || 'Non renseignée'}
                  </td>
                  <td className="py-3">
                    {isActif ? (
                    ...
                    <div className="d-flex gap-2">
                      {isActif ? (
                        <button 
                          ...
                          style={{ background: '#fee2e2', color: '#dc2626', border: 'none', fontSize: '11px', fontWeight: 'bold', transition: 'none', minWidth: '70px' }}
                        >
                          {isLoading ? '...' : <><i className="bi bi-lock me-1"></i> Bloquer</>}
                        </button>
                      ) : (
                        <button 
                          ...
                          disabled={isLoading}
                          className="btn btn-sm px-3 py-1 rounded-pill"
                          style={{ background: '#dcfce7', color: '#16a34a', border: 'none', fontSize: '11px', fontWeight: 'bold', transition: 'none', minWidth: '70px' }}
                        >
                          {isLoading ? '...' : <><i className="bi bi-unlock me-1"></i> Débloquer</>}
                        </button>
                      )}
                      <button 
        ...
        <div className="modal show d-block" style={{ 
          ...
          <div className="bg-white rounded-3 p-4" style={{ width: '400px', transition: 'none', transform: 'none' }}>
            ...
            <div className="d-flex gap-2 justify-content-end">
              ...
              <button className="btn btn-danger px-4" onClick={() => handleSupprimer(deleteId)} disabled={loadingId === deleteId} style={{ transition: 'none' }}>
                {loadingId === deleteId ? 'Confirmer' : 'Confirmer'}
              </button>
            </div>
          </div>
        </div>
