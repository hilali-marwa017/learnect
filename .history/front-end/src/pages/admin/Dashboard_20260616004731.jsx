...
export default function AdminDashboard() {
  ...
  async function loadAll() {
    ...
    try {
      const res = await api.get('/admin/enseignants/attente');
      if (Array.isArray(res.data)) setEnseignantsAttente(res.data);
    } catch(e) { console.error(e); }

    try {
      const res = await api.get('/admin/signalements');
      if (Array.isArray(res.data)) setSignalements(res.data);
    } catch(e) { console.error(e); }

    try {
      const res = await api.get('/admin/demandes');
      if (Array.isArray(res.data)) setDemandes(res.data);
    } catch(e) { console.error(e); }
    ...
    try {
      ...
      if (Array.isArray(res.data)) setPaiements(res.data);
    } catch(e) { console.error(e); }
  ...
  useEffect(() => {
    ...
    const tickColor = isDark ? '#a1a4a5' : '#718096';
    ...
    if (donutRef.current) {
      donutInstance.current = new Chart(donutRef.current, {
        ...
        data: {
          ...
          datasets: [{ data: [stats.totalEtudiants, stats.totalEnseignants], backgroundColor: ['#e04f00', '#1e293b'], borderWidth: 0, hoverOffset: 6 }]
        },
      ...
      barInstance.current = new Chart(barRef.current, {
        ...
        options: {
          ...
          scales: {
            ...
            y: { grid: { display: false }, ticks: { color: tickColor, font: { size: 12 } }, border: { display: false } }
          }
        }
      });
    }
    ...
    <div className="min-h-screen pt-20" style={{ background: 'var(--color-bg)' }}>
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '2rem 1.5rem', display: 'flex', gap: '1.5rem' }}>
        ...
        <aside style={{ width: '240px', flexShrink: 0, position: 'sticky', top: '90px', alignSelf: 'flex-start' }}>
          <div style={{ background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 1px 8px rgba(0,0,0,0.06)' }}>
            <div style={{ padding: '1.25rem 1rem', borderBottom: '1px solid var(--color-border)' }}>
              ...
              <div style={{ fontWeight: 800, color: 'var(--color-text)', fontSize: '0.95rem' }}>Console Admin</div>
              <div style={{ fontSize: '0.7rem', color: 'var(--color-muted)', marginTop: '2px' }}>{user?.prenom} {user?.nom}</div>
            </div>
              ...
              {navItems.map(item => {
                ...
                return (
                  <button key={item.id} onClick={() => setActiveTab(item.id)}
                    style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderRadius: '10px', border: 'none', cursor: 'pointer', background: isActive ? '#e04f00' : 'transparent', color: isActive ? '#fff' : 'var(--color-muted)', fontSize: '0.8rem', fontWeight: isActive ? 700 : 500, marginBottom: '2px', textAlign: 'left', transition: 'all 0.15s' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: '1rem' }}>
                {[
                  ...
                  <div key={i} style={{ background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: '14px', padding: '1.1rem 1rem', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
                    <div style={{ fontSize: '0.6rem', color: 'var(--color-muted)', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 700, marginBottom: '8px' }}>{kpi.label}</div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
                <div style={{ background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: '16px', padding: '1.5rem' }}>
                  <div style={{ fontSize: '0.82rem', fontWeight: 700, color: 'var(--color-text)', marginBottom: '4px' }}>Répartition des utilisateurs</div>
                  <div style={{ fontSize: '0.7rem', color: 'var(--color-muted)', marginBottom: '1rem' }}>Élèves vs Tuteurs</div>
                  <div style={{ display: 'flex', gap: '1rem', marginBottom: '1rem' }}>
                    <span style={{ fontSize: '0.7rem', color: 'var(--color-muted)', display: 'flex', alignItems: 'center', gap: '6px' }}><span style={{ width: '10px', height: '10px', background: '#e04f00', borderRadius: '3px', display: 'inline-block' }} /> Élèves ({stats.totalEtudiants})</span>
                    <span style={{ fontSize: '0.7rem', color: 'var(--color-muted)', display: 'flex', alignItems: 'center', gap: '6px' }}><span style={{ width: '10px', height: '10px', background: '#1e293b', borderRadius: '3px', display: 'inline-block' }} /> Tuteurs ({stats.totalEnseignants})</span>
                  </div>
                  <div style={{ height: '200px', display: 'flex', justifyContent: 'center' }}><canvas ref={donutRef} /></div>
                </div>
                <div style={{ background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: '16px', padding: '1.5rem' }}>
                  <div style={{ fontSize: '0.82rem', fontWeight: 700, color: 'var(--color-text)', marginBottom: '4px' }}>Activité de la plateforme</div>
                  <div style={{ fontSize: '0.7rem', color: 'var(--color-muted)', marginBottom: '1rem' }}>Vue d'ensemble par catégorie</div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
                <div style={{ background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: '16px', padding: '1.5rem' }}>
                  <div style={{ fontSize: '0.82rem', fontWeight: 700, color: 'var(--color-text)', marginBottom: '1rem' }}>Informations générales</div>
                  {[
                    ...
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid var(--color-border)' }}>
                      <span style={{ fontSize: '0.78rem', color: 'var(--color-muted)' }}>{row.label}</span>
                      <span style={{ fontSize: '0.82rem', fontWeight: 700, color: 'var(--color-text)' }}>{row.value}</span>
                    </div>
                  ))}
                </div>
