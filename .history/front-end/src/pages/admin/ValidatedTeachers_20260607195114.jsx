...
function ValidatedTeachers({ pendingTeachers, onValidate, onRefresh }) {
  const [refuseId, setRefuseId] = useState(null);
  const [refuseRaison, setRefuseRaison] = useState('');

  const handleRefuser = async () => {
    if (refuseId && refuseRaison) {
      await fetch(`http://localhost:8000/api/admin/enseignants/${refuseId}/refuser`, {
        method: 'POST',
  ...
  return (
    <div className="bg-white p-6 rounded-3xl border">
      ...
      <p className="text-xs text-slate-500 mt-1">Examinez minutieusement les documents de diplômes téléversés.</p>
          ...
          {pendingTeachers.map(t => (
            <div key={t.utilisateur_id} className="p-5 bg-slate-50 rounded-2xl border">
              ...
              <div className="flex gap-3 mt-4">
                <button onClick={() => onValidate(t.utilisateur_id)} className="bg-blue-600 text-white text-xs font-bold px-4 py-2 rounded-xl">
                  <i className="bi bi-shield-check me-1"></i>Approuver et Valider
                </button>
                <button onClick={() => setRefuseId(t.utilisateur_id)} className="border border-slate-300 text-rose-600 text-xs font-bold px-4 py-2 rounded-xl">
                  Demander correction
                </button>
              </div>
            </div>
