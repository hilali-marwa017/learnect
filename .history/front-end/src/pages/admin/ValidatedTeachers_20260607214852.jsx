...
function ValidatedTeachers({ pendingTeachers, onRefresh }) {
  ...
  const handleValider = async (id) => {
    ...
    setErreur('');
    try {
      const res = await fetch(`http://localhost:8000/api/admin/enseignants/${id}/valider`, {
        method: 'PUT',
  ...
  const handleRefuser = async () => {
    if (refuseId && refuseRaison) {
      ...
      setErreur('');
      try {
        const res = await fetch(`http://localhost:8000/api/admin/enseignants/${refuseId}/refuser`, {
          method: 'PUT',
          headers: { 'Authorization': `Bearer ${getToken()}`, 'Content-Type': 'application/json' },
      ...
      <div className="card-body p-4">
        <div className="d-flex align-items-center gap-3 mb-4 pb-2 border-bottom">
          ...
          <div>
            ...
            <p className="text-muted small mb-0">Examinez minutieusement les documents avant autorisation</p>
          </div>
        </div>
              ...
              <div className="border rounded-3 p-3 bg-white">
                <div className="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-3">
                  <div>
                    ...
                    <p className="text-muted small mb-0"><i className="bi bi-envelope me-1"></i> {t.user?.email} | <i className="bi bi-geo-alt me-1"></i> {t.user?.ville}</p>
                  </div>
                ...
                <div className="mt-3 d-flex gap-2 justify-content-end">
                  <button onClick={() => handleValider(t.utilisateur_id)} disabled={isLoading} className="btn btn-success btn-sm px-4">
                    {isLoading ? <><span className="spinner-border spinner-border-sm me-1"></span> Chargement...</> : <><i className="bi bi-shield-check me-1"></i> Approuver et Valider</>}
                  </button>
                  <button onClick={() => setRefuseId(t.utilisateur_id)} disabled={isLoading} className="btn btn-outline-danger btn-sm px-4">
                    <i className="bi bi-x-lg me-1"></i> Demander correction
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="modal show d-block" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div className="bg-white rounded-3 p-4" style={{ width: '450px' }}>
            ...
            <div className="d-flex gap-2 justify-content-end">
              <button className="btn btn-light px-4" onClick={() => setRefuseId(null)} disabled={isLoading}>Annuler</button>
              <button className="btn btn-danger px-4" onClick={handleRefuser} disabled={isLoading}>{isLoading ? 'Envoi...' : 'Confirmer le Rejet'}</button>
            </div>
          </div>
        </div>
      )}
