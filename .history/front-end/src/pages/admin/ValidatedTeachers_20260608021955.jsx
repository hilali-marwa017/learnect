...
function ValidatedTeachers({ pendingTeachers, onRefresh }) {
  ...
  // confirmValider = { id, nom }

  async function executerValider() {
    const { id } = confirmValider;
    setLoadingId(id);
          ...
          <div key={teacher.utilisateur_id} className="border rounded-3 p-3 mb-3" style={{ transition: 'none', transform: 'none' }}>
            ...
            <div className="d-flex gap-2 justify-content-end">
              {loadingId === teacher.utilisateur_id ? (
                ...
                <>
                  <button
                    onClick={() => setConfirmValider({ id: teacher.utilisateur_id, nom: `${teacher.user?.prenom} ${teacher.user?.nom}` })}
      ...
      {confirmValider && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div className="bg-white rounded-3 p-4" style={{ width: '400px' }}>
            ...
            <div className="d-flex gap-2 justify-content-end">
              <button className="btn btn-light px-4" onClick={() => setConfirmValider(null)} style={{ transition: 'none' }}>
                Annuler
              </button>
      ...
      {refuseId && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div className="bg-white rounded-3 p-4" style={{ width: '450px' }}>
            ...
            />
            <div className="d-flex gap-2 justify-content-end">
              <button className="btn btn-light px-4" onClick={() => { setRefuseId(null); setRaisonRefus(''); }} style={{ transition: 'none' }}>
                Annuler
              </button>
