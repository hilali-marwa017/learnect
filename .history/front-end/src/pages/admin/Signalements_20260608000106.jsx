...
function Signalements({ signalements, onRefresh }) {
  ...
  if (pending.length === 0) {
    return <div className="card border-0 shadow-sm p-4 text-center" style={{ transition: 'none', transform: 'none' }}><i className="bi bi-shield-check fs-1 text-success"></i><p className="mt-2 text-muted">Aucun signalement en attente</p></div>;
  }
    ...
    <div className="card border-0 shadow-sm" style={{ transition: 'none', transform: 'none' }}>
      <div className="card-body p-4">
        <div className="d-flex align-items-center gap-3 mb-4 pb-2 border-bottom">
          <div className="bg-danger bg-opacity-10 p-3 rounded-3" style={{ transition: 'none', transform: 'none' }}><i className="bi bi-flag fs-4 text-danger"></i></div>
        ...
        {pending.map(s => (
          <div key={s.id_signalement} className="border rounded-3 p-3 mb-3" style={{ transition: 'none', transform: 'none' }}>
            ...
            <div className="bg-light rounded-3 p-3 mb-2" style={{ transition: 'none', transform: 'none' }}><div className="small text-muted mb-1"><i className="bi bi-chat-quote me-1"></i> Avis signalé :</div><p className="mb-0 fst-italic">"{s.avis?.commentaire}"</p></div>
            <div className="bg-warning bg-opacity-10 rounded-3 p-3 mb-3" style={{ transition: 'none', transform: 'none' }}><div className="small text-muted mb-1"><i className="bi bi-info-circle me-1"></i> Motif :</div><p className="mb-0 small fw-medium">{s.motif}</p></div>
            <div className="d-flex gap-2 justify-content-end">
              <button onClick={() => handleTraiter(s.id_signalement)} disabled={isLoading} className="btn btn-primary btn-sm px-4" style={{ transition: 'none' }}><i className="bi bi-check-lg me-1"></i> Rejeter</button>
              <button onClick={() => handleSupprimerAvis(s.id_avis, s.id_signalement)} disabled={isLoading} className="btn btn-danger btn-sm px-4" style={{ transition: 'none' }}><i className="bi bi-trash me-1"></i> Supprimer</button>
            </div>
          </div>
        ))}
      </div>
