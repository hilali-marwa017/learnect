      ...
      <div className="card-body p-4">
        <div className="d-flex align-items-center gap-3 mb-4 pb-2 border-bottom">
          ...
          <div>
            ...
            <p className="text-muted small mb-0">{pendingTeachers.length} dossiers en attente</p>
          </div>
        </div>
        ...
        {pendingTeachers.length === 0 ? (
          <div className="text-center py-5">
            ...
            <p className="text-muted mb-0">Aucun enseignant en attente de vérification</p>
          </div>
        ) : (
                  ...
                  <div className="d-flex flex-wrap justify-content-between align-items-start gap-3">
                    <div className="d-flex gap-3">
                      <div className="bg-primary bg-opacity-10 rounded-circle d-flex align-items-center justify-content-center" style={{ width: '48px', height: '48px' }}>
                        <span className="fw-bold fs-5 text-primary">{t.user?.prenom?.[0]}</span>
                      </div>
                      <div>
                    ...
                    <div className="row g-2">
                      <div className="col-md-12">
                        <div className="bg-light rounded-2 p-2">
                          <span className="small text-muted"><i className="bi bi-file-pdf me-1"></i> Diplôme :</span>
                          <span className="small fw-medium ms-2">{t.diplome}</span>
                        </div>
                      </div>
                      <div className="col-md-6">
                        <div className="bg-light rounded-2 p-2">
                          <span className="small text-muted"><i className="bi bi-card-image me-1"></i> CIN Recto :</span>
                          <span className="small ms-2">{t.cin_recto || 'non fourni'}</span>
                        </div>
                      </div>
                      <div className="col-md-6">
                        <div className="bg-light rounded-2 p-2">
                          <span className="small text-muted"><i className="bi bi-card-image me-1"></i> CIN Verso :</span>
                          <span className="small ms-2">{t.cin_verso || 'non fourni'}</span>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div className="mt-3 p-2 bg-primary bg-opacity-5 rounded-2">
                    <i className="bi bi-quote text-primary me-1"></i>
                    <span className="small text-secondary">{t.description_profil?.substring(0, 150)}...</span>
                  </div>

                  <div className="mt-3 d-flex gap-2 justify-content-end">
                    <button onClick={() => handleValider(t.utilisateur_id)} className="btn btn-success btn-sm px-4">
                      <i className="bi bi-check-lg me-1"></i> Valider
                    </button>
                    <button onClick={() => setRefuseId(t.utilisateur_id)} className="btn btn-outline-danger btn-sm px-4">
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content rounded-4 border-0 shadow">
              <div className="modal-header border-0 pb-0">
                ...
                <button type="button" className="btn-close" onClick={() => setRefuseId(null)}></button>
              </div>
              <div className="modal-body">
                <p className="small text-muted mb-3">Précisez à l'enseignant pourquoi sa validation a été refusée.</p>
                <textarea 
              ...
              <div className="modal-footer border-0 pt-0">
                <button className="btn btn-light px-4" onClick={() => setRefuseId(null)}>Annuler</button>
                <button className="btn btn-danger px-4" onClick={handleRefuser}>Confirmer le rejet</button>
              </div>
            </div>
          </div>
        </div>
