...
function AdminDashboard() {
  ...
  // Créer le graphique quand les stats sont chargées
  useEffect(function() {
    if (onglet === 'stats' && graphRef.current && (stats.totalEtudiants > 0 || stats.totalEnseignants > 0)) {
      if (chartInstance.current) {
        chartInstance.current.destroy();
        chartInstance.current = null;
      }
      
      const total = stats.totalEtudiants + stats.totalEnseignants;
      const pctEtudiants = total > 0 ? Math.round((stats.totalEtudiants / total) * 100) : 0;
      const pctEnseignants = total > 0 ? Math.round((stats.totalEnseignants / total) * 100) : 0;
      
      chartInstance.current = new Chart(graphRef.current, {
        type: 'doughnut',
        data: {
          labels: [`Élèves (${pctEtudiants}%)`, `Tuteurs (${pctEnseignants}%)`],
          datasets: [{
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: {
              ...
              callbacks: {
                label: function(ctx) {
                  const valeur = ctx.raw;
                  const pourcentage = total > 0 ? Math.round((valeur / total) * 100) : 0;
                  return ctx.label.split('(')[0] + ': ' + valeur + ' (' + pourcentage + '%)';
                }
              }
            }
          },
        ...
        plugins: [{
          ...
          afterDraw: function(chart) {
            ...
            const totalUsers = data.datasets[0].data.reduce(function(a, b) { return a + b; }, 0);
            ...
            ctx.fillText(totalUsers, cx, cy - 10);
  ...
  }, [onglet, stats.totalEtudiants, stats.totalEnseignants]);
            ...
            return (
              <button
                ...
                style={{ 
                  ...
                  padding: '12px 4px', 
                  fontSize: '13px', 
                  fontWeight: onglet === tab ? 'bold' : 'normal', 
                  color: onglet === tab ? '#0d6efd' : '#6c757d', 
                  borderBottom: onglet === tab ? '2px solid #0d6efd' : 'none', 
                  cursor: 'pointer', 
                  transition: 'none',
                  minWidth: '90px'
                }}
