    ...
    <div className="container-fluid p-4" style={{ background: '#f5f7fa', minHeight: '100vh' }}>
      ...
      <div className="mb-4">
        <div className="d-flex gap-4" style={{ flexWrap: 'wrap' }}>
          <span onClick={() => setOnglet('stats')} style={{ fontSize: '14px', cursor: 'pointer', color: onglet === 'stats' ? '#000' : '#6c757d', transition: 'none' }}>Statistiques</span>
          <span onClick={() => setOnglet('teachers')} style={{ fontSize: '14px', cursor: 'pointer', color: onglet === 'teachers' ? '#000' : '#6c757d', transition: 'none' }}>Vérification Enseignants ({enseignantsAttente.length})</span>
          <span onClick={() => setOnglet('reports')} style={{ fontSize: '14px', cursor: 'pointer', color: onglet === 'reports' ? '#000' : '#6c757d', transition: 'none' }}>Signalements ({signalements.length})</span>
          <span onClick={() => setOnglet('demandes')} style={{ fontSize: '14px', cursor: 'pointer', color: onglet === 'demandes' ? '#000' : '#6c757d', transition: 'none' }}>Demandes ({demandes.length})</span>
          <span onClick={() => setOnglet('reservations')} style={{ fontSize: '14px', cursor: 'pointer', color: onglet === 'reservations' ? '#000' : '#6c757d', transition: 'none' }}>Réservations ({reservations.length})</span>
          <span onClick={() => setOnglet('paiements')} style={{ fontSize: '14px', cursor: 'pointer', color: onglet === 'paiements' ? '#000' : '#6c757d', transition: 'none' }}>Paiements ({paiements.length})</span>
          <span onClick={() => setOnglet('users')} style={{ fontSize: '14px', cursor: 'pointer', color: onglet === 'users' ? '#000' : '#6c757d', transition: 'none' }}>Utilisateurs ({users.length})</span>
        </div>
      </div>
