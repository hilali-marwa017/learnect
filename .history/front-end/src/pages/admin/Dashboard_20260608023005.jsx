...
function AdminDashboard() {
  ...
  async function chargerDonnees() {
    setChargement(true);
    try {
      // 1. Charger stats + users en priorité (affichage immédiat)
      ...
      if (resTeachers.data) {
        setEnseignantsAttente(Array.isArray(resTeachers.data) ? resTeachers.data : resTeachers.data.data || []);
      }
      if (resReports.data) {
        setSignalements(Array.isArray(resReports.data) ? resReports.data : resReports.data.data || []);
      }
      if (resDemandes.data) {
        setDemandes(Array.isArray(resDemandes.data) ? resDemandes.data : resDemandes.data.data || []);
      }
      if (resReservations.data) {
        setReservations(Array.isArray(resReservations.data) ? resReservations.data : resReservations.data.data || []);
      }
      if (resPaiements.data) {
        setPaiements(Array.isArray(resPaiements.data) ? resPaiements.data : resPaiements.data.data || []);
      }
  ...
  async function rafraichirDonnees() { 
    // Rafraîchissement complet sans re-afficher le spinner
    try {
      const [resStats, resUsers] = await Promise.all([
        api.get('/admin/stats'),
        api.get('/admin/users')
      ...
      if (resTeachers.data) setEnseignantsAttente(Array.isArray(resTeachers.data) ? resTeachers.data : resTeachers.data.data || []);
      if (resReports.data) setSignalements(Array.isArray(resReports.data) ? resReports.data : resReports.data.data || []);
      if (resDemandes.data) setDemandes(Array.isArray(resDemandes.data) ? resDemandes.data : resDemandes.data.data || []);
      if (resReservations.data) setReservations(Array.isArray(resReservations.data) ? resReservations.data : resReservations.data.data || []);
      if (resPaiements.data) setPaiements(Array.isArray(resPaiements.data) ? resPaiements.data : resPaiements.data.data || []);
    } catch(err) {
      console.error(err);
    }
  }
