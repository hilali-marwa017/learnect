...
function Signalements({ signalements, onRefresh }) {
  ...
  const handleTraiter = async (id) => {
    setIsLoading(true);
    try {
      const res = await fetch(`http://localhost:8000/api/admin/signalements/${id}/traiter`, {
        method: 'PUT',
  ...
  const handleSupprimerAvis = async (idAvis, idSignalement) => {
    setIsLoading(true);
    try {
      await fetch(`http://localhost:8000/api/admin/avis/${idAvis}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${getToken()}` } });
      await fetch(`http://localhost:8000/api/admin/signalements/${idSignalement}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${getToken()}` } });
      if (onRefresh) onRefresh();
