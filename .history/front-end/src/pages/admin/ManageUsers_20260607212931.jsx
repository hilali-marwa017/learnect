      ...
      <div style={{ padding: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '24px', paddingBottom: '12px', borderBottom: '1px solid #e2e8f0' }}>
          ...
          <div>
            ...
            <p style={{ color: '#6c757d', fontSize: '12px', marginBottom: '0' }}>{users.length} utilisateurs inscrits</p>
          </div>
        </div>
            ...
            <tbody>
              {users.map(u => (
                <tr key={u.utilisateur_id} style={{ borderBottom: '1px solid #e2e8f0' }}>
                  ...
                  <td style={{ padding: '12px' }}>
                    <span style={{ ...getRoleStyle(u.role), padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 'bold', display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                      <i className={getRoleIcon(u.role)}></i> {u.role}
                    </span>
                  </td>
                  ...
                  <td style={{ padding: '12px' }}>
                    {u.statut === 'actif' ? (
                      <button onClick={() => handleBloquer(u.utilisateur_id)} style={{ background: 'white', color: '#dc3545', border: '1px solid #dc3545', padding: '4px 12px', borderRadius: '6px', fontSize: '11px', fontWeight: 'bold' }}>
                        <i className="bi bi-lock" style={{ marginRight: '4px' }}></i> Bloquer
                      </button>
                    ) : (
                      <button onClick={() => handleDebloquer(u.utilisateur_id)} style={{ background: 'white', color: '#198754', border: '1px solid #198754', padding: '4px 12px', borderRadius: '6px', fontSize: '11px', fontWeight: 'bold' }}>
