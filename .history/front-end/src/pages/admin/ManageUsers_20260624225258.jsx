...
function DocLink({ doc }) {
    ...
    return (
        ...
        >
            ...
            <span style={{
                fontSize: '0.55rem',
                background: isPdf ? 'rgba(59,130,246,0.1)' : 'rgba(16,185,129,0.1)',
                color: isPdf ? '#3b82f6' : '#10b981',
                padding: '1px 5px',
                borderRadius: '4px',
                fontWeight: 700,
                            ...
                            <tr key={u.utilisateur_id ?? i} style={{ borderBottom: '1px solid var(--color-border)' }}>
                                ...
                                <td style={{ padding: '12px' }}>
                                    <span style={{
                                        ...
                                        borderRadius: '20px',
                                        fontWeight: 700,
                                        background: u.role === 'admin' ? 'rgba(224,79,0,0.1)' : u.role === 'enseignant' ? 'rgba(59,130,246,0.1)' : 'rgba(16,185,129,0.1)',
                                        color: u.role === 'admin' ? '#e04f00' : u.role === 'enseignant' ? '#3b82f6' : '#10b981',
                                    }}>
                                ...
                                <td style={{ padding: '12px' }}>
                                    <span style={{
                                        ...
                                        borderRadius: '20px',
                                        fontWeight: 700,
                                        background: u.statut === 'actif' ? 'rgba(16,185,129,0.1)' : u.statut === 'en_attente' ? 'rgba(245,158,11,0.1)' : 'rgba(220,38,38,0.1)',
                                        color: u.statut === 'actif' ? '#10b981' : u.statut === 'en_attente' ? '#f59e0b' : '#dc2626',
                                    }}>
                                ...
                                <td style={{ padding: '12px', textAlign: 'right' }}>
                                    <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end' }}>
                                        {u.role !== 'admin' && (
                                            <button
                                                onClick={() => bloquer(u.utilisateur_id, u.statut)}
                                                disabled={loadingId === u.utilisateur_id}
                                                style={{
                                                    padding: '5px 12px',
                                                    borderRadius: '7px',
                                                    border: `1px solid ${u.statut === 'actif' ? 'rgba(220,38,38,0.25)' : 'rgba(16,185,129,0.25)'}`,
                                                    background: u.statut === 'actif' ? 'rgba(220,38,38,0.06)' : 'rgba(16,185,129,0.06)',
                                                    color: u.statut === 'actif' ? '#dc2626' : '#10b981',
                                                    fontSize: '0.7rem',
                                                    fontWeight: 700,
                                                    cursor: 'pointer',
                                                    display: 'flex',
                                                    alignItems: 'center',
                                                    gap: '5px',
                                                    opacity: loadingId === u.utilisateur_id ? 0.6 : 1,
                                                }}
                                            >
                                        ...
                                        <button
                                            onClick={() => supprimerUser(u.utilisateur_id)}
                                            disabled={loadingId === u.utilisateur_id}
                                            style={{
                                                padding: '5px 8px',
                                                borderRadius: '7px',
                    ...
                    <div key={e.utilisateur_id} style={{ background: 'var(--color-bg)', border: '1px solid var(--color-border)', borderRadius: '12px', padding: '1.25rem', marginBottom: '1rem' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
                            ...
                            <div style={{ display: 'flex', gap: '8px' }}>
                                <button
                                    onClick={() => { setRefusModal(e.utilisateur_id); setRefusRaison(''); }}
                                ...
                                <button
                                    onClick={() => valider(e.utilisateur_id)}
                                    disabled={loadingId === e.utilisateur_id}
            ...
            {refusModal && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999, padding: '1rem' }}>
                    <div style={{ background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: '16px', padding: '1.5rem', width: '100%', maxWidth: '460px' }}>
                        ...
                        <textarea
                            value={refusRaison}
                            onChange={ev => setRefusRaison(ev.target.value)}
                        ...
                        <div style={{ display: 'flex', gap: '8px', marginTop: '1rem', justifyContent: 'flex-end' }}>
                            <button
                                ...
                                style={{ padding: '8px 16px', borderRadius: '8px', border: '1px solid var(--color-border)', background: 'transparent', color: 'var(--color-muted)', fontSize: '0.78rem', cursor: 'pointer', fontWeight: 600 }}
                            >
                                Annuler
                            </button>
                            <button
                                onClick={() => refuser(refusModal)}
                                disabled={!!loadingId}
