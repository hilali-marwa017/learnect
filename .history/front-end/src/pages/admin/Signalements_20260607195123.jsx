function Signalements({ signalements, onTraiter, onSupprimerAvis }) {
    ...
    <div className="bg-white p-6 rounded-3xl border">
      ...
      {pending.length === 0 ? (
        <div className="p-8 text-center bg-slate-50 rounded-2xl mt-4">
          <i className="bi bi-check-circle fs-1 text-green-300"></i>
        ...
        <div className="space-y-4 mt-4">
          {pending.map(s => (
            <div key={s.id_signalement} className="p-5 bg-slate-50 rounded-2xl border">
              <span className="inline-flex items-center gap-1.5 text-[10px] bg-rose-50 text-rose-700 px-2.5 py-1 rounded-full">
                <i className="bi bi-exclamation-triangle"></i>Signalé
              </span>
              ...
              <div className="flex gap-3 mt-4">
                <button onClick={() => onTraiter(s.id_signalement)} className="bg-blue-600 text-white text-xs px-3 py-2 rounded-xl">
                  Rejeter Signalement
                </button>
                <button onClick={() => onSupprimerAvis(s.id_avis, s.id_signalement)} className="bg-rose-50 text-rose-600 text-xs px-3 py-2 rounded-xl">
                  Supprimer l'avis
                </button>
              </div>
            </div>
