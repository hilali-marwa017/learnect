...
export default function AdminDashboard() {
  ...
  async function loadAll() {
    setLoading(true);
    try { const r = await api.get('/admin/stats'); const d = r.data; setStats({ totalEtudiants: d.total_etudiants||0, totalEnseignants: d.total_enseignants||0, totalReservations: d.total_reservations||0, totalAvis: d.total_avis||0, revenusTotal: d.revenus_total||0 }); } catch(e) {}
              ...
              <div className="admin-charts-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
                <div style={{ background: card, border: `1px solid ${border}`, borderRadius: '16px', padding: '1.5rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '1rem', marginBottom: '1rem' }}>
                    <span style={{ fontSize: '0.7rem', color: muted, display: 'flex', alignItems: 'center', gap: '6px' }}><span style={{ width: '10px', height: '10px', background: '#e04f00', borderRadius: '3px', display: 'inline-block' }} /> Élèves ({stats.totalEtudiants})</span>
                    <span style={{ fontSize: '0.7rem', color: muted, display: 'flex', alignItems: 'center', gap: '6px' }}><span style={{ width: '10px', height: '10px', background: isDark ? '#334155' : '#1e293b', borderRadius: '3px', display: 'inline-block' }} /> Tuteurs ({stats.totalEnseignants})</span>
                  </div>
