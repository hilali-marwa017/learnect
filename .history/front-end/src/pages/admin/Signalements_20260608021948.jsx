...
function Signalements({ signalements, onRefresh }) {
  ...
  // confirmAction = { type: 'traiter' | 'supprimer', idSignalement, idAvis? }

  async function executerAction() {
    const { type, idSignalement, idAvis } = confirmAction;
  ...
  const signalementsEnAttente = (signalements || []).filter(s => s.statut === 'en_attente');
          ...
          <div key={s.id_signalement} className="border rounded-3 p-3 mb-3" style={{ transition: 'none', transform: 'none' }}>
            ...
            <div className="d-flex gap-2 justify-content-end">
              {loadingId === s.id_signalement ? (
                ...
                <>
                  <button
                    onClick={() => setConfirmAction({ type: 'traiter', idSignalement: s.id_signalement })}
                  ...
                  <button
                    onClick={() => setConfirmAction({ type: 'supprimer', idSignalement: s.id_signalement, idAvis: s.id_avis })}
      ...
      {confirmAction && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div className="bg-white rounded-3 p-4" style={{ width: '400px' }}>
            ...
            <div className="d-flex gap-2 justify-content-end">
              <button className="btn btn-light px-4" onClick={() => setConfirmAction(null)} style={{ transition: 'none' }}>
                Annuler
              </button>
              <button
