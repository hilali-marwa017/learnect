        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {users.length === 0 ? (
              ...
              <tr key={u.utilisateur_id ?? i} style={{ borderBottom: `1px solid ${border}` }}>
...
export function VerificationsTab({ enseignantsAttente, setEnseignantsAttente }) {
  ...
  async function refuser(id) {
    ...
    try {
      ...
      setEnseignantsAttente(prev => prev.filter(e => e.utilisateur_id !== id));
      setRefusModal(null); setRefusRaison('');
    } catch(e) { alert('Erreur'); }
    finally { setLoadingId(null); }
  }
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    <button onClick={() => valider(e.utilisateur_id)} disabled={loadingId === e.utilisateur_id}
                      style={{ padding: '8px 16px', borderRadius: '8px', border: 'none', background: orange, color: '#fff', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', opacity: loadingId === e.utilisateur_id ? 0.6 : 1 }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999, padding: '1rem' }}>
          <div style={{ background: card, border: `1px solid ${border}`, borderRadius: '16px', padding: '1.5rem', width: '100%', maxWidth: '460px', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px' }}>
              ...
              <div style={{ fontSize: '1rem', fontWeight: 800, color: text }}>Motif du rejet</div>
            </div>
            <div style={{ fontSize: '0.72rem', color: muted, marginBottom: '1rem' }}>Précisez pourquoi la validation a été refusée.</div>
            <textarea value={refusRaison} onChange={e => setRefusRaison(e.target.value)}
              placeholder="Le justificatif du diplôme n'est pas lisible." rows={4}
            ...
            <div style={{ display: 'flex', gap: '8px', marginTop: '1rem', justifyContent: 'flex-end' }}>
              ...
              <button onClick={() => refuser(refusModal)} disabled={!!loadingId}
                style={{ padding: '8px 16px', borderRadius: '8px', border: 'none', background: '#dc2626', color: '#fff', fontSize: '0.78rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}>
