...
export function UsersTab({ users, setUsers }) {
  ...
  async function bloquer(userId, statut) {
    ...
    const msg = isActif ? 'Voulez-vous vraiment bloquer cet utilisateur ?' : 'Voulez-vous vraiment débloquer cet utilisateur ?';
    if (!window.confirm(msg)) return;
    const action = isActif ? 'bloquer' : 'debloquer';
    setLoadingId(userId);
    try {
      await api.put(`/admin/users/${userId}/${action}`);
      setUsers(prev => prev.map(u => u.id === userId ? { ...u, statut: isActif ? 'bloque' : 'actif' } : u));
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {users.length === 0 ? (
              ...
              <tr key={u.id ?? u.utilisateur_id ?? i} style={{ borderBottom: `1px solid ${border}` }}>
                ...
                <td style={{ padding: '12px', textAlign: 'right' }}>
                  <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end' }}>
                    {u.role !== 'admin' && (
                      <button
                        onClick={() => bloquer(u.id, u.statut)}
                        disabled={loadingId === u.id}
                        style={{ padding: '5px 12px', borderRadius: '7px', border: `1px solid ${u.statut === 'actif' ? 'rgba(220,38,38,0.25)' : 'rgba(16,185,129,0.25)'}`, background: u.statut === 'actif' ? 'rgba(220,38,38,0.06)' : 'rgba(16,185,129,0.06)', color: u.statut === 'actif' ? '#dc2626' : '#10b981', fontSize: '0.7rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px', opacity: loadingId === u.id ? 0.6 : 1 }}>
                        {loadingId === u.id ? <Loader size={13} style={{ animation: 'spin 1s linear infinite' }} /> : u.statut === 'actif' ? <><XCircle size={13} /> Bloquer</> : <><CheckCircle size={13} /> Débloquer</>}
                      </button>
                    )}
                    <button
                      onClick={() => supprimerUser(u.id)}
                      disabled={loadingId === u.id}
                      style={{ padding: '5px 8px', borderRadius: '7px', border: '1px solid rgba(220,38,38,0.2)', background: 'rgba(220,38,38,0.06)', color: '#dc2626', cursor: 'pointer', display: 'flex', alignItems: 'center', opacity: loadingId === u.id ? 0.6 : 1 }}>
                      <Trash2 size={13} />
                    </button>
                  </div>
                </td>
    ...
    <>
              ...
              <div key={e.utilisateur_id} style={{ background: bg, border: `1px solid ${border}`, borderRadius: '12px', padding: '1.25rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    <button onClick={() => valider(e.utilisateur_id)} disabled={loadingId === e.utilisateur_id} style={{ padding: '8px 16px', borderRadius: '8px', border: 'none', background: orange, color: '#fff', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', opacity: loadingId === e.utilisateur_id ? 0.6 : 1 }}>
                      {loadingId === e.utilisateur_id ? <Loader size={14} style={{ animation: 'spin 1s linear infinite' }} /> : <CheckCircle size={14} />} Approuver et Valider
                    </button>
                  </div>
                </div>
      ...
      {refusModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999, padding: '1rem' }}>
          <div style={{ background: card, border: `1px solid ${border}`, borderRadius: '16px', padding: '1.5rem', width: '100%', maxWidth: '460px', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px' }}>
              <XCircle size={20} color="#dc2626" />
              <div style={{ fontSize: '1rem', fontWeight: 800, color: text }}>Motif du rejet du document</div>
            </div>
            <div style={{ fontSize: '0.72rem', color: muted, marginBottom: '1rem' }}>Précisez à l'enseignant pourquoi sa validation a été refusée.</div>
            <textarea
              value={refusRaison}
              onChange={e => setRefusRaison(e.target.value)}
              placeholder="Le justificatif du diplôme n'est pas lisible ou non agréé au Maroc."
              rows={4}
              style={{ width: '100%', background: bg, border: `1px solid ${border}`, borderRadius: '10px', padding: '12px', color: text, fontSize: '0.8rem', outline: 'none', resize: 'vertical', boxSizing: 'border-box', fontFamily: 'inherit' }}
            />
            <div style={{ display: 'flex', gap: '8px', marginTop: '1rem', justifyContent: 'flex-end' }}>
              <button onClick={() => setRefusModal(null)} style={{ padding: '8px 16px', borderRadius: '8px', border: `1px solid ${border}`, background: 'transparent', color: muted, fontSize: '0.78rem', cursor: 'pointer', fontWeight: 600 }}>Annuler</button>
              <button onClick={() => refuser(refusModal)} disabled={loadingId !== null} style={{ padding: '8px 16px', borderRadius: '8px', border: 'none', background: '#dc2626', color: '#fff', fontSize: '0.78rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}>
...
export default function AdminManageUsers() { return null; }