import { useState } from 'react';
import PropTypes from 'prop-types';

/**
 * Composant de validation des enseignants
 * @param {Array} pendingTeachers - Liste des enseignants en attente
 * @param {Function} onRefresh - Callback pour rafraîchir les données
 */
const API_URL = 'http://localhost:8000/api';

function ValidatedTeachers({ pendingTeachers = [], onRefresh = null }) {
  const [refuseId, setRefuseId] = useState(null);
  const [refuseRaison, setRefuseRaison] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [erreur, setErreur] = useState('');

  const getToken = () => localStorage.getItem('token');
  
  const getHeaders = () => ({
    'Authorization': `Bearer ${getToken()}`,
    'Content-Type': 'application/json'
  });

  const handleValider = async (id) => {
    setIsLoading(true);
    setErreur('');
    
    try {
      const response = await fetch(`${API_URL}/admin/enseignants/${id}/valider`, {
        method: 'PUT',
        headers: getHeaders()
      });
      if (!response.ok) throw new Error('Erreur lors de la validation');
      if (onRefresh) onRefresh();
    } catch (err) { 
      setErreur(err.message); 
    } finally { 
      setIsLoading(false); 
    }
  };

  const handleRefuser = async () => {
    if (!refuseId || !refuseRaison.trim()) return;

    setIsLoading(true);
    setErreur('');

    try {
      const response = await fetch(`${API_URL}/admin/enseignants/${refuseId}/refuser`, {
        method: 'PUT',
        headers: getHeaders(),
        body: JSON.stringify({ raison: refuseRaison })
      });
      if (!response.ok) throw new Error('Erreur lors du refus');
      
      setRefuseId(null);
      setRefuseRaison('');
      if (onRefresh) onRefresh();
    } catch (err) { 
      setErreur(err.message); 
    } finally { 
      setIsLoading(false); 
    }
  };

  if (!pendingTeachers || pendingTeachers.length === 0) {
    return (
      <div className="card border-0 shadow-sm p-4 text-center">
        <i className="bi bi-check-circle fs-1 text-success"></i>
        <p className="mt-2 text-muted">Aucun enseignant en attente</p>
      </div>
    );
  }

  return (
    <div className="card border-0 shadow-sm">
      <div className="card-body p-4">
        <div className="d-flex align-items-center gap-3 mb-4 pb-2 border-bottom">
          <div className="bg-primary bg-opacity-10 p-3 rounded-3">
            <i className="bi bi-file-text fs-4 text-primary"></i>
          </div>
          <div>
            <h3 className="h5 fw-bold mb-0">Vérification des enseignants</h3>
            <p className="text-muted small mb-0">{pendingTeachers.length} enseignant(s) en attente</p>
          </div>
        </div>

        {erreur && <div className="alert alert-danger py-2 mb-3">{erreur}</div>}

        {pendingTeachers.map(teacher => (
          <div key={teacher.utilisateur_id} className="border rounded-3 p-3 mb-3">
            <div className="d-flex justify-content-between align-items-start mb-3">
              <div>
                <h5 className="fw-bold mb-1">{teacher.user?.prenom} {teacher.user?.nom}</h5>
                <p className="text-muted small mb-0">
                  <i className="bi bi-envelope me-1"></i> {teacher.user?.email}
                </p>
              </div>
              <div className="bg-light rounded-3 p-2 text-center">
                <div className="small text-muted">Tarif</div>
                <div className="fw-bold text-primary">{teacher.tarifHeure} DH/h</div>
              </div>
            </div>

            <div className="bg-light rounded-3 p-3 mb-3">
              <p className="fw-bold mb-2 small">Documents fournis :</p>
              <p className="mb-1 small">📄 Diplôme : {teacher.diplome || 'Non fourni'}</p>
              <p className="mb-0 small">🆔 CIN Recto : {teacher.cin_recto || 'Non fourni'}</p>
            </div>

            <div className="d-flex gap-2 justify-content-end">
              <button 
                onClick={() => handleValider(teacher.utilisateur_id)} 
                disabled={isLoading} 
                className="btn btn-success btn-sm px-4"
              >
                <i className="bi bi-check-lg me-1"></i> Valider
              </button>
              <button 
                onClick={() => setRefuseId(teacher.utilisateur_id)} 
                disabled={isLoading} 
                className="btn btn-danger btn-sm px-4"
              >
                <i className="bi bi-x-lg me-1"></i> Refuser
              </button>
            </div>
          </div>
        ))}
      </div>

      {/* Modal de refus */}
      {refuseId && (
        <div className="modal show d-block" style={{ 
          position: 'fixed', top: 0, left: 0, right: 0, bottom: 0,
          background: 'rgba(0,0,0,0.5)', display: 'flex',
          alignItems: 'center', justifyContent: 'center', zIndex: 1000
        }}>
          <div className="bg-white rounded-3 p-4" style={{ width: '450px' }}>
            <h5 className="fw-bold mb-3">Motif du refus</h5>
            <textarea 
              className="form-control mb-3" 
              rows="4"
              placeholder="Expliquez pourquoi la validation est refusée..."
              value={refuseRaison} 
              onChange={e => setRefuseRaison(e.target.value)} 
              disabled={isLoading}
            />
            <div className="d-flex gap-2 justify-content-end">
              <button className="btn btn-light" onClick={() => setRefuseId(null)} disabled={isLoading}>
                Annuler
              </button>
              <button className="btn btn-danger" onClick={handleRefuser} disabled={isLoading}>
                {isLoading ? 'Envoi...' : 'Confirmer'}
              </button>
            </div>
          </div>
        </div>
      )}
    </div>
  );
}

ValidatedTeachers.propTypes = {
  pendingTeachers: PropTypes.array,
  onRefresh: PropTypes.func
};

export default ValidatedTeachers;