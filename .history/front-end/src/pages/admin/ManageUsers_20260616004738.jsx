              ...
              <div style={{ marginTop: '1rem', background: 'var(--color-card)', borderRadius: '10px', padding: '1rem', border: '1px solid var(--color-border)' }}>
                {e.documents.map((doc, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                    <span style={{ fontSize: '0.6rem', background: 'rgba(59,130,246,0.1)', color: '#3b82f6', padding: '2px 8px', borderRadius: '4px', fontWeight: 700, textTransform: 'uppercase' }}>{doc.type_document}</span>
                    <a href={`http://localhost:8000/storage/${doc.chemin}`} target="_blank" rel="noreferrer" style={{ fontSize: '0.75rem', color: '#e04f00', textDecoration: 'none' }}>{doc.chemin?.split('/').pop()}</a>
                  </div>
                ))}
              </div>
            )}
