...
const MOROCCAN_CITIES = ['Casablanca', 'Rabat', 'Tanger', 'Marrakech', 'Fès', 'Agadir', 'Oujda', 'Meknès', 'Kénitra', 'Tétouan', 'Salé', 'Mohammedia', 'Nador']
...
export default function Register() {
  ...
  const validateStep1 = () => {
    ...
    if (!email.includes('@')) { setErrorMsg('Adresse email invalide.'); return false }
    if (password.length < 8) { setErrorMsg('Le mot de passe doit contenir au moins 8 caractères.'); return false }
    if (password !== passwordConfirm) { setErrorMsg('Les mots de passe ne correspondent pas.'); return false }
    return true
  }
  ...
  const handleSubmitEtudiant = async (e) => {
    ...
    setLoading(true)
    try {
      const formData = buildFormData('etudiant')
  ...
  const handleSubmitEnseignant = async (e) => {
    e.preventDefault()
    setErrorMsg('')
    if (!cinRecto || !cinVerso || !diplome) { setErrorMsg('Veuillez fournir le CIN recto/verso et le diplôme PDF.'); return }
    setLoading(true)
    try {
      const formData = buildFormData('enseignant')
    ...
    return (
      <div className="min-h-screen py-24 bg-canvas flex flex-col items-center justify-center px-6">
        <div className="max-w-3xl w-full text-center space-y-4 mb-12">
          <span className="text-xs uppercase tracking-[0.2em] font-caption text-accent-orange font-bold animate-pulse block">COMMENCEZ DÈS AUJOURD'HUI</span>
          <h1 className="font-display-lg text-4xl md:text-5xl text-ink leading-none font-bold">Rejoignez Learnect<span className="text-accent-orange font-bold">.ma</span></h1>
          <p className="text-charcoal text-sm max-w-xl mx-auto">Sélectionnez votre profil d'inscription pour démarrer votre parcours pédagogique au Maroc.</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl w-full">
          <div onClick={() => { setSelectedRole('etudiant'); setStep(1); setErrorMsg('') }}
            ...
            <div className="space-y-6">
              <div className="p-4 bg-accent-blue-glow rounded-xl w-fit border border-accent-blue/10 text-accent-blue"><BookOpen className="h-6 w-6" /></div>
              <div className="space-y-2">
                <h3 className="font-heading-md text-xl text-ink font-extrabold group-hover:text-accent-blue transition-colors">Je suis un Élève / Étudiant</h3>
                <p className="text-charcoal text-xs leading-relaxed">Je souhaite trouver un professeur qualifié pour remonter mes notes et réussir mes examens.</p>
              </div>
              <ul className="space-y-2 text-mute text-xs font-semibold">
                <li>✓ Annuaire d'enseignants accrédités</li>
                <li>✓ Première heure de diagnostic offerte</li>
            ...
            <button className="mt-8 bg-surface-deep/50 text-ink border border-hairline hover:bg-accent-blue hover:text-white hover:border-accent-blue p-3.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2">
              <span>M'inscrire comme Étudiant</span><ArrowRight className="h-4 w-4" />
            </button>
          </div>
          <div onClick={() => { setSelectedRole('enseignant'); setStep(1); setErrorMsg('') }}
            ...
            <div className="space-y-6">
              <div className="p-4 bg-accent-orange-glow rounded-xl w-fit border border-accent-orange/10 text-accent-orange"><GraduationCap className="h-6 w-6" /></div>
              <div className="space-y-2">
                <h3 className="font-heading-md text-xl text-ink font-extrabold group-hover:text-accent-orange transition-colors">Je suis un Enseignant / Tuteur</h3>
                <p className="text-charcoal text-xs leading-relaxed">Je souhaite donner des cours de soutien scolaire et gérer mes réservations.</p>
              </div>
              <ul className="space-y-2 text-mute text-xs font-semibold">
                <li>✓ Visibilité auprès de milliers de familles</li>
                <li>✓ Zéro commission d'engagement</li>
                <li>✓ Panel d'administration des gains</li>
              </ul>
            </div>
            <button className="mt-8 bg-surface-deep/50 text-ink border border-hairline hover:bg-accent-orange hover:text-white hover:border-accent-orange p-3.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2">
              <span>M'inscrire comme Tuteur</span><ArrowRight className="h-4 w-4" />
            </button>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen py-24 bg-canvas flex flex-col items-center justify-center px-6">
      <div className="w-full max-w-2xl bg-surface-card border border-hairline-strong rounded-2xl shadow-2xl overflow-hidden">
        <div className="p-6 border-b border-hairline flex justify-between items-center bg-surface-elevated/40">
          <div>
            <h3 className="font-heading-md text-lg text-ink font-bold">{selectedRole === 'enseignant' ? 'Inscription Professeur' : 'Inscription Étudiant'}</h3>
            <p className="text-[10px] text-mute font-caption uppercase tracking-wider font-bold">{selectedRole === 'enseignant' ? `Étape ${step}/2` : 'Étape 1/1'}</p>
          </div>
          <button onClick={() => setSelectedRole(null)} className="p-1.5 rounded-full hover:bg-surface-elevated text-charcoal hover:text-ink transition-colors cursor-pointer"><X className="h-5 w-5" /></button>
        </div>

        {errorMsg && <div className="bg-accent-red/10 border-b border-accent-red/25 text-accent-red text-center py-3 text-xs font-semibold px-4">⚠️ {errorMsg}</div>}
        {success && <div className="bg-accent-green/10 border-b border-accent-green/25 text-accent-green text-center py-3 text-xs font-semibold px-4">✓ Inscription réussie ! Redirection...</div>}
        ...
        <div className="p-6 space-y-5">
            ...
            <div className="space-y-4 animate-in fade-in duration-200">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <div className="relative"><User className="absolute left-3 top-3 h-4 w-4 text-mute" />
                    <input type="text" required placeholder="Ex: Amine" value={prenom} onChange={(e) => setPrenom(e.target.value)}
                      className="w-full bg-surface-deep/30 border border-hairline-strong text-ink rounded-lg pl-9 p-2.5 text-xs focus:ring-1 focus:ring-accent-orange outline-none" /></div>
                </div>
                <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <div className="relative"><Mail className="absolute left-3 top-3 h-4 w-4 text-mute" />
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative"><Phone className="absolute left-3 top-3 h-4 w-4 text-mute" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <div className="relative"><Lock className="absolute left-3 top-3 h-4 w-4 text-mute" />
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative"><MapPin className="absolute left-3 top-3.5 h-4 w-4 text-mute" />
          ...
          {step === 2 && selectedRole === 'enseignant' && (
            <div className="space-y-4 animate-in fade-in duration-200">
              <p className="text-xs text-charcoal leading-relaxed">Pour valider votre compte enseignant, veuillez fournir votre CIN recto/verso et votre diplôme en PDF.</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  { label: 'CIN (Recto) *', state: cinRecto, setter: setCinRecto, accept: 'image/*' },
                  { label: 'CIN (Verso) *', state: cinVerso, setter: setCinVerso, accept: 'image/*' },
                ].map(({ label, state, setter, accept }) => (
                  <div key={label} className="space-y-2 p-3.5 bg-surface-deep/30 border border-hairline-strong rounded-xl">
                    <span className="text-[11px] uppercase font-caption font-bold text-ink block">{label}</span>
                    <label className="relative border border-dashed border-hairline-strong rounded-lg h-24 flex flex-col items-center justify-center text-center p-3 hover:border-accent-orange/40 transition-colors cursor-pointer">
                      <input type="file" accept={accept} onChange={(e) => setter(e.target.files[0])} className="absolute inset-0 opacity-0 cursor-pointer" />
                      {state ? <span className="text-xs font-semibold text-accent-green flex items-center gap-1"><CheckCircle className="h-4 w-4" />{state.name}</span>
                        : <><Upload className="h-4 w-4 text-mute mb-1" /><span className="text-[10px] text-ink font-semibold">Télécharger</span></>}
                    </label>
                  </div>
                ))}
              </div>
              <div className="space-y-2 p-3.5 bg-surface-deep/30 border border-hairline-strong rounded-xl">
                ...
                <label className="relative border border-dashed border-hairline-strong rounded-lg p-5 flex flex-col items-center justify-center text-center hover:border-accent-orange/40 transition-colors cursor-pointer">
                  ...
                  {diplome ? <span className="text-xs font-semibold text-accent-green flex items-center gap-1"><CheckCircle className="h-4 w-4" />{diplome.name}</span>
              ...
              <div className="pt-4 border-t border-hairline flex justify-between">
                <button onClick={() => { setStep(1); setErrorMsg('') }} className="px-4 py-2.5 rounded-xl border border-hairline text-xs font-bold text-ink hover:bg-surface-elevated transition-colors cursor-pointer">← Retour</button>
