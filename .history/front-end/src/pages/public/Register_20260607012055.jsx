...
// Utilise ces chemins (car Register.jsx est dans pages/public/)
import MapComponent from '../../components/MapComponent';
import { getCityFromCoordinates, getCoordinatesFromCity, DEFAULT_POSITION } from '../../services/GeocodingService';vice';
...
function Register() {
  ...
  const [form, setForm] = useState({
    ...
    distance_max: '', langues: '', latitude: '', longitude: ''
  });
  ...
  // Mise à jour de la position quand la ville change
  const handleCityChange = async (e) => {
    const cityName = e.target.value;
    setForm({ ...form, ville: cityName });
    
    if (cityName && role === 'enseignant') {
      setIsGeocoding(true);
  ...
  // Mise à jour de la ville quand la carte est cliquée
  const handlePositionChange = async (newPosition) => {
    setPosition(newPosition);
    setForm(prev => ({ ...prev, latitude: newPosition[0], longitude: newPosition[1] }));
    
    setIsGeocoding(true);
    const cityName = await getCityFromCoordinates(newPosition[0], newPosition[1]);
    if (cityName) {
      setForm(prev => ({ ...prev, ville: cityName }));
    }
    setIsGeocoding(false);
  };
      ...
      <div className="card" style={{ maxWidth: '550px', width: '100%', borderRadius: '16px' }}>
        <div className="card-body p-4">
          ...
          <div className="d-flex justify-content-between align-items-center mb-3">
            <Link to="/" className="btn btn-link text-muted p-0 text-decoration-none">
              <i className="bi bi-x-lg fs-4"></i>
            </Link>
            {role === 'enseignant' && (
              <span className="small text-muted">Étape {step}/2</span>
            )}
          </div>
            ...
            {step === 1 && (
              <>
                ...
                <div className="mb-3">
                  ...
                  {isGeocoding && <small className="text-muted">Recherche en cours...</small>}
                </div>

                {/* Carte Leaflet pour enseignant */}
                {role === 'enseignant' && (
                  <div className="mb-3">
                    <label className="form-label small fw-semibold text-secondary">POSITION SUR LA CARTE</label>
                    <MapComponent 
                      position={position} 
                      onPositionChange={handlePositionChange} 
                      height="250px" 
                    />
                    <small className="text-muted">Cliquez sur la carte ou déplacez le marqueur</small>
                  </div>
                )}
            ...
            {role === 'enseignant' && step === 2 && (
              <>
                ...
                <div className="d-flex gap-3">
                  <button type="button" className="btn btn-outline-secondary flex-grow-1" onClick={() => { setStep(1); setError(''); window.scrollTo(0, 0); }}>
                    ← Retour
                  </button>
