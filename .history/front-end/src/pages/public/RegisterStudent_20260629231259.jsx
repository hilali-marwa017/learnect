...
import { User, Mail, Lock, Phone, MapPin, GraduationCap, Wallet, X, UserPlus, Eye, EyeOff, Camera } from 'lucide-react';
import api from '../../api/axios';
import { useAuth } from '../../context/AuthContext';

// Liste des villes du Maroc avec accents
const CITIES = ['Casablanca','Rabat','Marrakech','Tanger','Agadir','Fès','Meknès','Oujda','Kénitra','Tétouan','Salé','Mohammédia','Nador'];

// Liste des niveaux scolaires avec accents
...
export default function RegisterStudent() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const resultat = await registerMultipart(fd);
      if (resultat.success) {
        navigate('/student');
      } else {
        setError(resultat.message || 'Une erreur est survenue, veuillez réessayer.');
      }
    } catch(err){
      ...
      <div style={{ width: '100%', maxWidth: 720 }}>
        <div style={{ background: bgCard, border: '1px solid ' + bdr, borderRadius: 18, boxShadow: '0 4px 24px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
          ...
          <form onSubmit={handleSubmit} style={{ padding: '2rem' }}>
            ...
            {error && (
              <div style={{ background: 'rgba(226,75,74,0.06)', border: '1px solid #e24b4a33', borderRadius: 10, padding: '12px 14px', color: '#e24b4a', fontSize: '0.8rem', marginBottom: '1.5rem' }}>
                {error}
              </div>
            )}
            ...
            {step === 1 && (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
                ...
                <div style={{ gridColumn: '1 / -1', display: 'flex', alignItems: 'center', gap: '1rem', padding: '1rem', background: bgSurf, border: '1px solid ' + bdr, borderRadius: 12 }}>
                  <div style={{ position: 'relative', flexShrink: 0 }}>
                    <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: orange + '15', border: '2px solid ' + orange + '40', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      {photoPreview
                        ? <img src={photoPreview} alt="Aperçu" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                        : <User size={26} color={orange} />
                      }
                    </div>
                    <label style={{ position: 'absolute', bottom: '-2px', right: '-2px', width: '22px', height: '22px', borderRadius: '50%', background: orange, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                      <Camera size={11} color="#fff" />
                      <input type="file" accept="image/jpg,image/jpeg,image/png" onChange={handlePhotoChange} style={{ display: 'none' }} />
                    </label>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.78rem', fontWeight: 700, color: ink }}>Photo de profil</div>
                  </div>
                </div>

                <Field label="Prénom" icon={User} name="prenom" value={form.prenom} onChange={handleChange} error={errors.prenom} isDark={isDark} />
                ...
                <Field label="Téléphone portable" icon={Phone} name="telephone" value={form.telephone} onChange={handleChange} error={errors.telephone} isDark={isDark} />
