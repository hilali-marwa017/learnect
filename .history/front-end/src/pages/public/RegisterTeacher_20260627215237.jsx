...
import { GraduationCap, User, Mail, Lock, Phone, MapPin, BookOpen, Wallet, FileText, ArrowRight, CheckCircle, File, Shield, Eye, EyeOff, X } from 'lucide-react';
...
export default function RegisterTeacher() {
  ...
  const inputStyle = { width: '100%', padding: '14px 14px 14px 42px', background: bgInput, border: '1px solid ' + border, borderRadius: '10px', color: text, fontSize: '0.85rem', outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit' };
  const labelStyle = { display: 'block', fontSize: '0.7rem', fontWeight: 700, color: iconColor, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '6px' };
  ...
  // success = true → enseignant soumis, en attente de validation
  ...
  const [form, setForm] = useState({ nom: '', prenom: '', email: '', telephone: '', ville: '', password: '', password_confirmation: '', titre: '', tarifHeure: '', langues: '', description_profil: '', description_cours: '' });
  const [checkboxes, setCheckboxes] = useState({ cours_enligne: false, cours_domicile: false, cours_deplacement: false, certifie: false });
  ...
  function handleCheckbox(e) { setCheckboxes({ ...checkboxes, [e.target.name]: e.target.checked }); }
  function handleFile(e) { setFiles({ ...files, [e.target.name]: e.target.files[0] }); }
  function handleCitySelect(city) { setForm({ ...form, ville: city }); setShowCityDrop(false); }
  ...
  function validateStep1() {
    ...
    if (form.password.length < 8) { setError('Mot de passe minimum 8 caracteres.'); return false; }
  ...
  function handleNext() { if (validateStep1()) { setStep(2); window.scrollTo(0, 0); } }
  function handleBack() { setStep(1); window.scrollTo(0, 0); }

  // soumission finale avec registerMultipart du contexte
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (result.success) {
        // toujours afficher l'ecran d'attente — ne jamais rediriger vers /teacher
        setSuccess(true);
      } else {
  ...
  // ecran de succes — en attente de validation admin
  if (success) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: bgPage, padding: '2rem' }}>
        <div style={{ background: bgPage, border: '1px solid ' + border, borderRadius: '24px', padding: '2.5rem', textAlign: 'center', maxWidth: '460px', width: '100%' }}>
          <div style={{ width: '72px', height: '72px', background: 'rgba(245,158,11,0.1)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1.5rem', border: '2px solid rgba(245,158,11,0.3)' }}>
            <CheckCircle size={36} color="#d97706" />
          </div>
          <div style={{ fontSize: '0.65rem', color: '#d97706', fontWeight: 700, fontFamily: 'monospace', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '0.75rem' }}>
            DOSSIER SOUMIS AVEC SUCCES
          </div>
          <h2 style={{ fontSize: '1.4rem', fontWeight: 900, marginBottom: '0.75rem', color: text }}>Candidature en cours de verification</h2>
          <p style={{ color: muted, marginBottom: '0.75rem', fontSize: '0.85rem', lineHeight: 1.6 }}>
            Notre equipe va examiner votre dossier (CIN + diplome) sous <strong style={{ color: text }}>24h ouvrables</strong>.
          </p>
          <p style={{ color: muted, marginBottom: '2rem', fontSize: '0.82rem', lineHeight: 1.6 }}>
            Une fois valide par l'administrateur, vous pourrez vous connecter et acceder a votre espace enseignant.
          </p>
          <div style={{ background: isDark ? 'rgba(245,158,11,0.06)' : 'rgba(245,158,11,0.04)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: '12px', padding: '1rem', marginBottom: '1.5rem' }}>
            <p style={{ fontSize: '0.75rem', color: muted, margin: 0, lineHeight: 1.6 }}>
              En attendant la validation, la connexion a votre compte sera bloquee. Vous recevrez une notification une fois le dossier traite.
            </p>
          </div>
          <Link to="/login" style={{ display: 'inline-block', background: orange, color: '#fff', padding: '12px 28px', borderRadius: '40px', textDecoration: 'none', fontWeight: 700, fontSize: '0.85rem', fontFamily: 'monospace' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                  <div>
                    ...
                    <div style={{ position: 'relative' }}>
                      ...
                      <input style={inputStyle} type="text" name="prenom" value={form.prenom} onChange={handleChange} required />
                    </div>
                  </div>
                  <div>
                    ...
                    <div style={{ position: 'relative' }}>
                      <User size={16} style={iconStyle} />
                      <input style={inputStyle} type="text" name="nom" value={form.nom} onChange={handleChange} required />
                    </div>
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                  <div>
                    ...
                    <div style={{ position: 'relative' }}>
                      ...
                      <input style={inputStyle} type="tel" name="telephone" value={form.telephone} onChange={handleChange} required />
                    </div>
                  </div>
                  <div>
