...
import { Mail, Lock, Eye, EyeOff, GraduationCap, ArrowRight, X, AlertTriangle, Clock } from 'lucide-react';
...
export default function Login() {
  ...
  const [refusInfo, setRefusInfo]   = useState(null);  // { raison } si refuse
  const [enAttente, setEnAttente]   = useState(false); // true si en_attente
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (resultat.success) {
        if (resultat.role === 'admin')      navigate('/admin',   { replace: true });
        else if (resultat.role === 'enseignant') navigate('/teacher', { replace: true });
        else                                navigate('/student', { replace: true });
      } else {
        // cas refuse — afficher la raison
        if (resultat.statut === 'refuse') {
          setRefusInfo({ raison: resultat.raison });
        }
        // cas en_attente
        ...
        // autre erreur
          ...
          setError(resultat.message || 'Email ou mot de passe incorrect');
        }
      }
    } catch (err) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: bgPage, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      <div style={{
        width: '100%', maxWidth: '900px', background: bgCard,
        border: `1px solid ${borderColor}`, borderRadius: '20px', overflow: 'hidden',
        ...
        {/* PANNEAU GAUCHE */}
        <div style={{
          background: bgLeft, padding: '3rem 2.5rem',
          display: 'flex', flexDirection: 'column', justifyContent: 'flex-start',
          gap: '2rem', borderRight: `1px solid ${leftBorder}`
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <div style={{ width: '40px', height: '40px', background: '#e04f00', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        {/* PANNEAU DROIT */}
        <div style={{ display: 'flex', flexDirection: 'column', background: bgRight }}>
          ...
          <div style={{ padding: '0 2.5rem 3rem 2.5rem', display: 'flex', flexDirection: 'column', justifyContent: 'center', flex: 1 }}>
            ...
            {/* erreur classique */}
            {error && (
              <div style={{ background: 'rgba(220,38,38,0.08)', border: '1px solid rgba(220,38,38,0.25)', color: '#dc2626', fontSize: '0.82rem', padding: '10px 14px', borderRadius: '8px', marginBottom: '1.2rem' }}>
                {error}
              </div>
            )}
            ...
            {/* en attente */}
            {enAttente && (
              <div style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: '10px', padding: '14px 16px', marginBottom: '1.2rem', display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                <Clock size={18} color="#d97706" style={{ flexShrink: 0, marginTop: '1px' }} />
                <div>
                  <div style={{ fontWeight: 700, fontSize: '0.82rem', color: '#d97706', marginBottom: '4px' }}>
                    Dossier en cours d'examen
                  </div>
                  <div style={{ fontSize: '0.78rem', color: textRightMuted, lineHeight: 1.5 }}>
                    Notre équipe examine votre candidature. Vous serez notifié une fois la décision prise.
                  </div>
                </div>
              </div>
            )}
            ...
            {/* refuse avec raison */}
            {refusInfo && (
              <div style={{ background: 'rgba(220,38,38,0.06)', border: '1px solid rgba(220,38,38,0.25)', borderRadius: '10px', padding: '14px 16px', marginBottom: '1.2rem', display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '0.78rem', color: textRightMuted, lineHeight: 1.5, marginBottom: '8px' }}>
                    <strong style={{ color: textRight }}>Raison :</strong> {refusInfo.raison}
                  </div>
                  <Link to="/register/teacher" style={{ fontSize: '0.75rem', color: orange, fontWeight: 700, textDecoration: 'none' }}>
            ...
            <div style={{ marginBottom: '1.8rem' }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <button
                  ...
                  style={{ position: 'absolute', right: '14px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: showPassword ? orange : textRightMuted, padding: 0, display: 'flex', alignItems: 'center' }}
                >
            ...
            <button
              ...
              style={{
                ...
                borderRadius: '10px', padding: '14px', fontWeight: 700, fontSize: '0.82rem',
