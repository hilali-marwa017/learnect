...
import {
  ...
  Clock, Gift, X, MessageCircle
} from 'lucide-react';

// Helper : construit l'URL avatar sans doubler le domaine
function buildAvatarUrl(photo, prenom, nom) {
  if (!photo) {
    return `https://ui-avatars.com/api/?name=${prenom}+${nom}&background=e04f00&color=fff&size=128`;
  }
  // Photo externe (randomuser, gravatar, etc.)
  if (photo.startsWith('http://') || photo.startsWith('https://')) {
    return photo;
  }
  // Photo uploadée dans storage Laravel
...
export default function TeacherProfile() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Garder tous les créneaux disponibles
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', background: bg, display: 'flex', alignItems: 'center', justifyContent: 'center', color: muted, fontFamily: 'monospace', fontSize: '0.82rem' }}>
      Chargement...
    </div>
  );
  ...
  const avatarUrl = buildAvatarUrl(enseignant.user?.photo, enseignant.user?.prenom, enseignant.user?.nom);
    ...
    <div style={{ minHeight: '100vh', background: bg, paddingTop: '5rem', paddingBottom: '4rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div style={{ background: bgCard, border: `1px solid ${border}`, borderRadius: '20px', padding: '2rem', display: 'flex', gap: '1.5rem', alignItems: 'flex-start' }}>
              <img
                ...
                onError={e => { e.target.src = `https://ui-avatars.com/api/?name=${enseignant.user?.prenom}+${enseignant.user?.nom}&background=e04f00&color=fff&size=128`; }}
                style={{ width: '100px', height: '100px', borderRadius: '16px', objectFit: 'cover', border: `1px solid ${border}`, flexShrink: 0 }}
              />
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap', marginBottom: '6px' }}>
                  <h1 style={{ fontSize: '1.6rem', fontWeight: 900, color: text, margin: 0, letterSpacing: '-0.02em' }}>
                ...
                <p style={{ fontSize: '0.82rem', color: '#e04f00', fontWeight: 700, margin: '0 0 12px' }}>
                  {enseignant.titre}
                </p>
          ...
          <div style={{ position: 'sticky', top: '5.5rem' }}>
            <div style={{ background: bgCard, border: `1px solid ${border}`, borderRadius: '20px', padding: '1.75rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', paddingBottom: '1.25rem', borderBottom: `1px solid ${borderS}` }}>
                ...
                <span style={{ fontSize: '0.6rem', fontWeight: 700, fontFamily: 'monospace', color: '#16a34a', background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: '8px', padding: '4px 10px' }}>
                  1ER COURS OFFERT
                </span>
              </div>
              ...
              {/* ── SUCCÈS ── */}
              {bookingDone ? (
                <div style={{ textAlign: 'center', padding: '1.5rem', background: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: '16px' }}>
                  <div style={{ width: '56px', height: '56px', borderRadius: '50%', background: 'rgba(34,197,94,0.1)', border: '2px solid rgba(34,197,94,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
                    <CheckCircle size={28} color="#16a34a" />
                  </div>
                  <div style={{ fontSize: '0.6rem', color: '#16a34a', fontFamily: 'monospace', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '8px' }}>
                    RÉSERVATION CONFIRMÉE
                  </div>
                  <h3 style={{ fontSize: '1rem', fontWeight: 900, color: text, margin: '0 0 8px', letterSpacing: '-0.01em' }}>
                    Félicitations ! 🎉
                  </h3>
                  <p style={{ fontSize: '0.75rem', color: muted, margin: '0 0 1.5rem', lineHeight: 1.6 }}>
                    Votre cours a été réservé avec succès. Confirmez le paiement depuis vos réservations, puis contactez votre professeur.
                  </p>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <button
                      onClick={() => navigate('/student/reservations')}
                      style={{ width: '100%', background: '#111827', color: '#fff', border: 'none', borderRadius: '10px', padding: '12px', fontSize: '0.78rem', fontWeight: 800, cursor: 'pointer', fontFamily: 'monospace', letterSpacing: '0.06em' }}
                    >
                      MES RÉSERVATIONS
                    </button>
                    <button
                      onClick={() => navigate('/student/messages')}
                      style={{ width: '100%', background: 'rgba(224,79,0,0.08)', color: '#e04f00', border: '1px solid rgba(224,79,0,0.25)', borderRadius: '10px', padding: '12px', fontSize: '0.78rem', fontWeight: 800, cursor: 'pointer', fontFamily: 'monospace', letterSpacing: '0.06em', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
                    >
                      <MessageCircle size={15} /> CONTACTER LE PROFESSEUR
                    </button>
                  </div>
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', maxHeight: '200px', overflowY: 'auto' }}>
                      {creneaux.length === 0 ? (
                        <div style={{ textAlign: 'center', padding: '1.5rem', color: muted, fontSize: '0.75rem', fontFamily: 'monospace', background: inputBg, borderRadius: '10px', border: `1px solid ${borderS}` }}>
                        ...
                        creneaux.map(c => (
                          <button
                            ...
                            style={{ padding: '12px 14px', borderRadius: '10px', textAlign: 'left', cursor: 'pointer', border: selected === c.id_creneau ? '1px solid #e04f00' : `1px solid ${border}`, background: selected === c.id_creneau ? 'rgba(224,79,0,0.08)' : inputBg, display: 'flex', alignItems: 'center', gap: '10px', transition: 'all 0.15s' }}
                          >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      style={{ width: '100%', background: inputBg, border: `1px solid ${border}`, color: text, borderRadius: '10px', padding: '11px 14px', fontSize: '0.8rem', outline: 'none', boxSizing: 'border-box' }}
                    />
                  </div>
      ...
      <style>{`
        @media (max-width: 900px) {
          ...
          .teacher-booking-sticky { position: relative !important; top: 0 !important; }
        }
      `}</style>
    </div>
  );
