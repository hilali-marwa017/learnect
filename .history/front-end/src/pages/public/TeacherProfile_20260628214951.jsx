...
import { MapPin, Star, Calendar, Check, X, Clock, Flag, Navigation, Home as HomeIcon, Video, AlertTriangle } from 'lucide-react';
...
export default function TeacherProfile() {
  ...
  // ✅ FIX BUG 4 : modal signalement inline au lieu de window.prompt
  ...
  async function handleEnvoyerSignalement() {
    if (motifSignal.trim().length < 10) { setSignalError('Motif trop court (minimum 10 caractères).'); return; }
    if (avis.length === 0) { setSignalError('Aucun avis disponible à signaler pour ce professeur.'); return; }
    ...
    <div style={{ minHeight: '100vh', background: bg, paddingTop: '2rem', paddingBottom: '4rem' }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 1.5rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        <div style={{ background: bgCard, border: '1px solid ' + border, borderRadius: '20px', padding: '2rem', display: 'flex', gap: '2rem', flexWrap: 'wrap', alignItems: 'flex-start' }}>
          ...
          {/* Boutons avis + signaler */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', flexShrink: 0 }}>
            {/* ✅ FIX BUG 3 : bouton avis toujours visible avec message explicatif si pas eligible */}
            {user?.role === 'etudiant' && (
              peutNoter ? (
            ...
            {/* ✅ FIX BUG 4 : modal signalement inline */}
            {user && (
        ...
        {/* Modal signalement inline — FIX BUG 4 */}
        {showSignalModal && (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {reservedInfo ? (
              <div style={{ background: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.25)', borderRadius: '16px', padding: '1.5rem' }}>
                ...
                <p style={{ fontSize: '0.85rem', color: text, fontWeight: 700, margin: '0 0 1rem' }}>Votre réservation est en attente de confirmation de paiement.</p>
            ...
            <div style={{ background: bgCard, border: '1px solid ' + border, borderRadius: '16px', padding: '1.25rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                ...
                {enseignant.cours_domicile && (
                  <div style={{ fontSize: '0.78rem', color: muted, display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <HomeIcon size={14} color="#16a34a" /> Cours à domicile
                  </div>
                )}
                {enseignant.cours_deplacement && (
                  <div style={{ fontSize: '0.78rem', color: muted, display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <Navigation size={14} color="#16a34a" /> Cours en déplacement
                  </div>
                )}
                {enseignant.langues && (
                  <div style={{ fontSize: '0.78rem', color: muted, display: 'flex', alignItems: 'center', gap: '8px' }}>
