...
function Home() {
  ...
  // Matières filtrées pour dropdown
      ...
      <div className="container py-5">
        <div className="row justify-content-center">
          <div className="col-lg-8 text-center">
                ...
                <div className="row g-3">
                  ...
                  <div className="col-md-6">
                    <label className="form-label small fw-semibold text-secondary mb-1">
                      <i className="bi bi-book text-primary me-1"></i>Matière
                    </label>
                    <div className="position-relative">
                      ...
                      {showMatiereDropdown && matiereInput && filteredMatieres.length > 0 && (
                        <div className="dropdown-menu show position-absolute w-100 mt-1 shadow-sm border-0 rounded-3" style={{ zIndex: 1000, maxHeight: '200px', overflowY: 'auto' }}>
                          {filteredMatieres.map((matiere) => (
                            <button
                              key={matiere.id}
                              type="button"
                              className="dropdown-item py-2"
                              onClick={() => handleMatiereSelect(matiere.nom)}
                            >
                              {matiere.nom}
                            </button>
                          ))}
                  ...
                  <div className="col-md-4">
                    <label className="form-label small fw-semibold text-secondary mb-1">
                      <i className="bi bi-geo-alt text-primary me-1"></i>Adresse ou ville
                    </label>
                    <input
                      type="text"
                      className="form-control py-2"
            ...
            {/* MATIÈRES POPULAIRES (COMME DANS L'IMAGE) */}
            <div className="d-flex flex-wrap justify-content-center gap-2 mb-4">
              {['Maths', 'Physique - Chimie', 'Français', 'Anglais', 'Arabe', 'Informatique'].map((m) => (
                <button
                  key={m}
                  onClick={() => {
                    setMatiereInput(m);
                ...
                >
                  <i className="bi bi-book me-1"></i> {m}
                </button>
              ))}
            </div>
      ...
      <div className="bg-light py-5">
        <div className="container">
          <div className="d-flex justify-content-between align-items-center mb-4">
            <h3 className="fw-bold fs-4 mb-0">Professeurs recommandés</h3>
            <button
              onClick={() => {
                setSearchMatiere('');
                setSearchVille('');
