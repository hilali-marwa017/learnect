...
// composant reutilisable pour creer differents inputs
...
export default function RegisterStudent() {
  ...
  const [error, setError] = useState(''); // erreur general
  const [errors, setErrors] = useState({}); // objet d'erreurs des champs
  ...
  // selectionner une ville de la liste
  ...
  function handleNext() {
    const v = validateStep1(); //valider les champs du 1ere etape
    setErrors(v);
    if (Object.keys(v).length === 0) setStep(2);// passer a etp 2 si aucune erreur 
  }
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    setError('');
    // essayer d'envoyer les donnees au serverur
    try{ 
      const data = {
      ...
      // sauvegarder token et infos user dans navigateur
      localStorage.setItem('token', res.data.token);
      localStorage.setItem('user', JSON.stringify(res.data.user));
      navigate('/student'); //rederiction vers espace student
    } catch (err) {
