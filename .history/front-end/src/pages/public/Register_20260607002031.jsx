...
function Register() {
  ...
  // ========== CHARGEMENT DES VILLES (METHODE OFPPT FETCH) ==========
  useEffect(() => {
  ...
  // ========== SOUMISSION FORMULAIRE (METHODE OFPPT FETCH) ==========
  const handleSubmit = (e) => {
    ...
    // Requête fetch POST (méthode OFPPT)
    fetch(url, {
      method: 'POST',
      body: data
      // Pas de header Content-Type car fetch le gère automatiquement avec FormData
    })
      .then(response => {
        if (!response.ok) {
          // Si erreur HTTP, on récupère le message d'erreur
