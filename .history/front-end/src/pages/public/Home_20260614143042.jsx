...
export default function Home() {
  ...
  // Filtrage DAIF avec prise en charge de "Webcam"
  function filterTutors() {
    ...
    for (var i = 0; i < tutors.length; i++) {
      ...
      if (queryCity === 'Webcam') {
        // Si l'utilisateur cherche "Webcam", filtrer les profs qui font cours en ligne
  ...
  function handleToggleFavorite(e, id) {
    e.stopPropagation();
    var newFav = favorites.filter(x => x !== id);
    if (newFav.length === favorites.length) newFav.push(id);
    setFavorites(newFav);
  }
  ...
  return (
    <div>
      ...
      <TeachersSection isDark={isDark} tutors={filteredTutors} loading={loading} error={error} onReset={handleReset} querySubject={querySubject} queryCity={queryCity} />
