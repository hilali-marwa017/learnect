...
// construit l url correcte de la photo sans doubler le domaine
...
export default function Teachers() {
  ...
  const { user } = useAuth();
  // recuperer isDark depuis le layout parent
  ...
  useEffect(() => {
    api.get('/villes')
      ...
      .catch(() => setVilles(['Casablanca','Rabat','Marrakech','Tanger','Agadir','Fes']));
  }, []);
  ...
  async function fetchTutors() {
    ...
    try {
      ...
      // transformer les donnees backend en format TeacherCard
      const mapped = res.data.map(e => ({
        ...
        delivery:    e.cours_enligne && e.cours_domicile ? 'face à face & webcam' : e.cours_enligne ? 'webcam' : e.cours_domicile ? 'face à face' : '',
        isFirstFree: true,
  ...
  // filtre local par nom ou matiere
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1.5rem' }}>
        ...
        <div style={{ borderBottom: '1px solid ' + border, paddingBottom: '1.5rem', marginBottom: '2rem' }}>
          ...
          <p style={{ fontSize: '0.8rem', color: muted, margin: 0 }}>
            Trouvez le tuteur certifie qui correspond a vos besoins.
          </p>
        </div>
        ...
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', marginBottom: '1.5rem', alignItems: 'center' }}>
          ...
          {/* Recherche */}
          <div style={{ flex: 2, minWidth: '200px', background: inputBg, border: '1px solid ' + border, borderRadius: '12px', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          {/* Input ville avec dropdown */}
          <div style={{ position: 'relative', minWidth: '180px', flex: 1 }}>
            <div style={{ background: inputBg, border: '1px solid ' + border, borderRadius: '12px', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            {/* Dropdown villes */}
            {showCityDrop && (
              <div style={{ position: 'absolute', top: 'calc(100% + 6px)', left: 0, right: 0, background: isDark ? '#1a1a1c' : '#ffffff', border: '1px solid ' + border, borderRadius: '12px', zIndex: 200, maxHeight: '250px', overflowY: 'auto', boxShadow: '0 8px 24px rgba(0,0,0,0.1)' }}>
                <div
                  ...
                  onMouseEnter={e => e.currentTarget.style.background = isDark ? '#2a2a2c' : '#f5f5f5'}
                  onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                >
                  <Wifi size={14} color="#e04f00" /> Webcam
                </div>
                {filteredCities.map((v, i) => (
                  <div key={i} onMouseDown={() => { setQueryCity(v); setShowCityDrop(false); }} style={{ padding: '10px 16px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px', color: text, fontSize: '0.82rem' }}
                    onMouseEnter={e => e.currentTarget.style.background = isDark ? '#2a2a2c' : '#f5f5f5'}
                    onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
          ...
          <div style={{ position: 'relative', minWidth: '160px' }}>
            ...
            <select
              ...
              style={{ width: '100%', background: inputBg, border: '1px solid ' + border, color: tarifMax ? text : muted, borderRadius: '12px', padding: '12px 12px 12px 34px', fontSize: '0.82rem', outline: 'none', cursor: 'pointer', appearance: 'none' }}
            >
          ...
          {/* Bouton reset — UN SEUL */}
          {hasFilter && (
            <button onClick={handleReset} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '12px 16px', borderRadius: '12px', border: '1px solid rgba(220,38,38,0.2)', background: 'rgba(220,38,38,0.06)', color: '#dc2626', fontSize: '0.78rem', fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap' }}>
        ...
        {!loading && (
          <div style={{ fontSize: '0.7rem', color: muted, fontFamily: 'monospace', fontWeight: 700, marginBottom: '1.5rem', letterSpacing: '0.06em' }}>
            {filtered.length} PROFESSEUR{filtered.length > 1 ? 'S' : ''} TROUVE{filtered.length > 1 ? 'S' : ''}
          </div>
        )}
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '4rem 2rem', background: bgCard, border: '1px solid ' + border, borderRadius: '20px' }}>
            <p style={{ fontSize: '0.82rem', color: muted, fontFamily: 'monospace', margin: '0 0 1rem' }}>
              Aucun professeur trouve avec ces criteres.
            </p>
            <button onClick={handleReset} style={{ padding: '10px 24px', background: '#e04f00', color: '#fff', border: 'none', borderRadius: '10px', fontSize: '0.78rem', fontWeight: 800, cursor: 'pointer', fontFamily: 'monospace' }}>
