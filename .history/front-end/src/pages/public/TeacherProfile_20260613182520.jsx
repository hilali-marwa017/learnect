      ...
      <div className="max-w-4xl mx-auto px-6 mt-6">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-surface-card border border-hairline-strong rounded-xl p-6">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div><span className="text-mute">Langues:</span> <span className="text-ink font-medium">{teacher.langues || 'Non spécifié'}</span></div>
                <div><span className="text-mute">Vérifié:</span> <span className="text-ink font-medium">{teacher.estVerifie ? '✓ Oui' : 'En attente'}</span></div>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'avis' && (
          <div className="space-y-4">
            {avis.length > 0 ? avis.map((a, idx) => (
              <div key={idx} className="bg-surface-card border border-hairline-strong rounded-xl p-6">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <div className="h-8 w-8 rounded-full bg-accent-blue-glow flex items-center justify-center text-accent-blue text-xs font-bold">
                      {a.etudiant?.prenom?.[0] || 'E'}
                    </div>
                    <span className="text-sm font-semibold text-ink">{a.etudiant?.prenom} {a.etudiant?.nom}</span>
                  </div>
                  <div className="flex items-center gap-1 text-accent-yellow">
                    <Star className="h-4 w-4 fill-accent-yellow" />
                    <span className="font-bold text-sm">{a.note}</span>
                  </div>
                </div>
                <p className="text-charcoal text-sm">{a.commentaire}</p>
              </div>
            )) : (
              <p className="text-center text-mute py-12">Aucun avis pour le moment.</p>
            )}
          </div>
        )}
        ...
        {activeTab === 'creneaux' && (
          <div className="space-y-4">
            {creneaux.length > 0 ? creneaux.map((c, idx) => (
              <div key={idx} className="bg-surface-card border border-hairline-strong rounded-xl p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Calendar className="h-5 w-5 text-accent-blue" />
                  <div>
                    <p className="text-sm font-semibold text-ink capitalize">{c.jour}</p>
                    <p className="text-xs text-charcoal flex items-center gap-1"><Clock className="h-3 w-3" /> {c.heureDebut} - {c.heureFin}</p>
                  </div>
                </div>
                <span className={`px-3 py-1 rounded-full text-xs font-bold ${c.estDisponible ? 'bg-accent-green-glow text-accent-green' : 'bg-accent-red-glow text-accent-red'}`}>
                  {c.estDisponible ? 'Disponible' : 'Réservé'}
                </span>
              </div>
            )) : (
              <p className="text-center text-mute py-12">Aucun créneau disponible.</p>
            )}
          </div>
        )}
      </div>
