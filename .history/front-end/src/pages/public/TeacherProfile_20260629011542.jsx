...
export default function TeacherProfile() {
  ...
  async function handleEnvoyerAvis() {
    ...
    if (commentaire.trim().length < 10) {
      setAvisError('Commentaire trop court (min 10 caracteres).');
      return;
    }
  ...
  async function handleEnvoyerSignalement() {
    ...
    if (motifSignal.trim().length < 10) {
      setSignalError('Motif trop court (minimum 10 caracteres).');
      return;
    }
    if (avis.length === 0) {
      setSignalError('Aucun avis disponible a signaler pour ce professeur.');
      return;
    }
  ...
  const infosPratiques = [
    ...
    enseignant.cours_domicile
      ? { icon: <HomeIcon size={14} color="#16a34a" />, label: 'Cours a domicile' }
      : null,
    enseignant.cours_deplacement
      ? { icon: <Navigation size={14} color="#16a34a" />, label: 'Cours en deplacement' }
      : null,
