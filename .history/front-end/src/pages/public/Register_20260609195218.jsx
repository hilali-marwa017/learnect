...
function Register() {
  ...
  if (success) {
    return (
      <div className="d-flex align-items-center justify-content-center" style={{ minHeight: '100vh', background: '#f5f5f5' }}>
        <div className="card p-4 text-center" style={{ maxWidth: '400px', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
  ...
  return (
    <div className="d-flex align-items-center justify-content-center" style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', padding: '24px' }}>
      <div className="card" style={{ maxWidth: '550px', width: '100%', borderRadius: '24px', border: 'none', boxShadow: '0 20px 40px rgba(0,0,0,0.15)' }}>
        <div className="card-body p-4 p-md-5">
          
          {/* Header avec X à gauche */}
          <div className="d-flex align-items-center mb-4">
            <Link to="/" className="text-muted text-decoration-none" style={{ transition: 'none' }}>
          ...
          {/* Progress bar enseignant */}
          {role === 'enseignant' && step === 1 && (
            <div className="mb-4">
              <div className="progress" style={{ height: '4px', borderRadius: '4px' }}>
                <div className="progress-bar bg-primary" style={{ width: '50%', borderRadius: '4px' }}></div>
              </div>
            </div>
          )}
          ...
          <div className="text-center mb-4">
            <div className="bg-primary text-white rounded-circle d-flex align-items-center justify-content-center mx-auto mb-3" style={{ width: '56px', height: '56px' }}>
              <i className={`bi ${role === 'enseignant' ? 'bi-briefcase fs-4' : 'bi-mortarboard fs-4'}`}></i>
            </div>
            <h3 className="fw-bold mb-1" style={{ fontSize: '1.5rem' }}>{role === 'enseignant' ? 'Devenir Enseignant' : 'Créer un compte'}</h3>
            <p className="text-muted small">{role === 'enseignant' ? 'Partagez votre savoir et gagnez de l\'argent' : 'Trouvez votre professeur idéal'}</p>
          </div>

          {/* Erreur */}
          {error && (
            <div className="alert alert-danger py-2 small" style={{ borderRadius: '12px' }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            {step === 1 && (
              <>
                <div className="mb-3">
                  ...
                  <input type="file" name="photo" className="form-control" accept="image/*" onChange={handleFile} style={{ borderRadius: '12px' }} />
                ...
                <div className="row">
                  <div className="col-md-6 mb-3">
                    ...
                    <input type="text" name="nom" className="form-control" value={form.nom} onChange={handleChange} required style={{ borderRadius: '12px' }} />
                  </div>
                  <div className="col-md-6 mb-3">
                    <label className="form-label small fw-semibold text-secondary">PRÉNOM</label>
                    <input type="text" name="prenom" className="form-control" value={form.prenom} onChange={handleChange} required style={{ borderRadius: '12px' }} />
                  </div>
                </div>

                <div className="mb-3">
                  <label className="form-label small fw-semibold text-secondary">EMAIL</label>
                  <input type="email" name="email" className="form-control" value={form.email} onChange={handleChange} required style={{ borderRadius: '12px' }} />
                </div>

                <div className="mb-3">
                  <label className="form-label small fw-semibold text-secondary">TÉLÉPHONE</label>
                  <input type="tel" name="telephone" className="form-control" value={form.telephone} onChange={handleChange} required style={{ borderRadius: '12px' }} />
                </div>

                <div className="mb-3">
                  <label className="form-label small fw-semibold text-secondary">VILLE</label>
                  <input type="text" name="ville" list="villes-list" className="form-control" value={form.ville} onChange={handleChange} placeholder="Tapez ou sélectionnez" required style={{ borderRadius: '12px' }} />
                ...
                {role === 'etudiant' && (
                  <>
                    <div className="mb-3">
                      ...
                      <select name="niveau" className="form-select" value={form.niveau} onChange={handleChange} style={{ borderRadius: '12px' }}>
                    ...
                    <div className="mb-3">
                      ...
                      <input type="number" name="budget" className="form-control" min="0" value={form.budget} onChange={handleChange} style={{ borderRadius: '12px' }} />
                    </div>
                  </>
                )}
                ...
                {role === 'enseignant' && (
                  <>
                    <div className="mb-3">
                      ...
                      <input type="text" name="matiere" list="matieres-list" className="form-control" value={form.matiere} onChange={handleChange} placeholder="Tapez ou sélectionnez" required style={{ borderRadius: '12px' }} />
                    ...
                    <div className="mb-3">
                      ...
                      <input type="number" name="tarifHeure" className="form-control" min="50" value={form.tarifHeure} onChange={handleChange} required style={{ borderRadius: '12px' }} />
                    </div>
                    <div className="mb-3">
                ...
                {/* Champs password sans séparateur visuel */}
                ...
                <button 
                  ...
                  style={{ borderRadius: '12px', transition: 'none' }}
                >
            ...
            {/* STEP 2 - Documents enseignant */}
            {role === 'enseignant' && step === 2 && (
              <>
                <div className="border rounded-3 p-3 mb-3 bg-light" style={{ borderRadius: '16px' }}>
                  ...
                  <div className="mb-3">
                    ...
                    <input type="file" name="cin_recto" className="form-control" accept="image/*" onChange={handleFile} required style={{ borderRadius: '12px' }} />
                  </div>
                  <div className="mb-3">
                    <label className="form-label small fw-semibold">CIN Verso</label>
                    <input type="file" name="cin_verso" className="form-control" accept="image/*" onChange={handleFile} required style={{ borderRadius: '12px' }} />
                  </div>
                  <div>
                    <label className="form-label small fw-semibold">Diplôme (PDF)</label>
                    <input type="file" name="diplome" className="form-control" accept=".pdf" onChange={handleFile} required style={{ borderRadius: '12px' }} />
                  </div>
                </div>
          ...
          <div className="text-center">
            ...
            <Link to="/login" className="text-primary text-decoration-none ms-1 small fw-semibold">Se connecter</Link>
          </div>
        </div>
      </div>
    </div>
