...
function Register() {
  ...
  useEffect(function () {
    fetch('http://localhost:8000/api/villes')
      ...
      .catch(function () { setVilles([]); });
    fetch('http://localhost:8000/api/matieres')
      .then(function (res) { return res.json(); })
      .then(function (data) { setMatieres(data); })
      .catch(function () { setMatieres([]); });
  }, []);
  ...
  function handleSubmit(e) {
    ...
    fetch('http://localhost:8000/api/register', {
      ...
      .then(function (result) {
        if (!result.ok) {
          if (result.data.errors) {
            const premiereErreur = Object.values(result.data.errors)[0][0];
            setError(premiereErreur);
      ...
      .catch(function () {
        setError('Impossible de joindre le serveur. Vérifiez que Laravel tourne sur le port 8000.');
      })
      .finally(function () {
        setLoading(false);
      });
            ...
            {step === 1 && (
              <>
                <div style={{ marginBottom: '12px' }}>
                  ...
                  <input type="file" name="photo" style={{ width: '100%', padding: '8px', border: '1px solid #E2E8F0', borderRadius: '8px', background: 'white' }} accept="image/*" onChange={handleFile} />
                ...
                <div style={{ marginBottom: '12px' }}>
                  ...
                  <datalist id="villes-list">
                    {villes.map(function (v, idx) { return <option key={idx} value={v.nom} />; })}
                  </datalist>
                </div>
                ...
                {role === 'etudiant' && (
                  <>
                    <div style={{ marginBottom: '12px' }}>
                      ...
                      <select name="niveau" style={{ width: '100%', padding: '8px 12px', border: '1px solid #E2E8F0', borderRadius: '8px', background: 'white' }} value={form.niveau} onChange={handleChange}>
                        <option value="">Sélectionnez</option>
                        {niveaux.map(function (n, idx) { return <option key={idx} value={n}>{n}</option>; })}
                      </select>
                    </div>
                    <div style={{ marginBottom: '12px' }}>
                  ...
                  <>
                    <div style={{ marginBottom: '12px' }}>
                      ...
                      <datalist id="matieres-list">
                        {matieres.map(function (m, idx) { return <option key={idx} value={m.nom} />; })}
                      </datalist>
                    </div>
                    <div style={{ marginBottom: '12px' }}>
                ...
                <div style={{ display: 'flex', gap: '12px', marginBottom: '12px' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ position: 'relative' }}>
                      ...
                      <button type="button" onClick={function () { setShowPass(!showPass); }} style={{ position: 'absolute', right: '8px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: '#94A3B8', cursor: 'pointer' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ position: 'relative' }}>
                      ...
                      <button type="button" onClick={function () { setShowPass2(!showPass2); }} style={{ position: 'absolute', right: '8px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: '#94A3B8', cursor: 'pointer' }}>
              ...
              <>
                <div style={{ border: '1px solid #E2E8F0', borderRadius: '8px', padding: '12px', marginBottom: '16px', background: '#F8FAFC' }}>
                  ...
                  <div style={{ marginBottom: '12px' }}>
                    ...
                    <input type="file" name="cin_recto" style={{ width: '100%', padding: '6px', border: '1px solid #E2E8F0', borderRadius: '8px', background: 'white' }} accept="image/*" onChange={handleFile} required />
                  </div>
                  <div style={{ marginBottom: '12px' }}>
                    <label style={{ fontSize: '11px', fontWeight: 'bold', display: 'block', marginBottom: '4px' }}>CIN Verso</label>
                    <input type="file" name="cin_verso" style={{ width: '100%', padding: '6px', border: '1px solid #E2E8F0', borderRadius: '8px', background: 'white' }} accept="image/*" onChange={handleFile} required />
                  </div>
                  <div>
                    <label style={{ fontSize: '11px', fontWeight: 'bold', display: 'block', marginBottom: '4px' }}>Diplôme (PDF)</label>
                    <input type="file" name="diplome" style={{ width: '100%', padding: '6px', border: '1px solid #E2E8F0', borderRadius: '8px', background: 'white' }} accept=".pdf" onChange={handleFile} required />
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: '12px' }}>
                  <button type="button" style={{ flex: 1, padding: '10px', background: '#f1f5f9', border: '1px solid #E2E8F0', borderRadius: '8px', cursor: 'pointer' }} onClick={function () { setStep(1); setError(''); }}>
                    ← Retour
                  </button>
