...
function Field({ label, icon: Icon, name, type = 'text', placeholder, value, onChange, error, showToggle, onToggle, showPass, isDark }) {
  ...
  return (
    <div>
      <label style={{ display: 'block', fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: muted, marginBottom: 8 }}>{label}</label>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: bgSurf, border: `1px solid ${error ? '#e24b4a' : bdr}`, borderRadius: 10, padding: '12px 14px' }}>
        ...
        <input type={showToggle ? (showPass ? 'text' : 'password') : type} name={name} value={value} onChange={onChange} placeholder={placeholder} style={{ background: 'none', border: 'none', outline: 'none', color: ink, fontSize: '0.85rem', width: '100%' }} />
        {showToggle && (
