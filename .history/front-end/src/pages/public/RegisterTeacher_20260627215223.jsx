...
import {GraduationCap, User, Mail, Lock, Phone, MapPin, BookOpen, Wallet, FileText, ArrowRight, ArrowLeft, CheckCircle, File, Shield, Eye, EyeOff, X} from 'lucide-react';
...
export default function RegisterTeacher() {
  ...
  const { registerMultipart } = useAuth(); // ✅ Utiliser le contexte Auth
  ...
  const [checkboxes, setCheckboxes] = useState({
    cours_enligne: false, cours_domicile: false, cours_deplacement: false, certifie: false
  });
  ...
  // ✅ CORRIGE : utilise registerMultipart du contexte Auth
  async function handleSubmit(e) {
    ...
    try {
      // ✅ Utiliser registerMultipart du contexte Auth
      const result = await registerMultipart(formData);
      
      if (result.success) {
        setSuccess(true);
        // ✅ Rediriger vers le dashboard enseignant après 3 secondes
        setTimeout(() => {
