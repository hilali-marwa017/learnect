...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    // DEBUG 1: Afficher ce qui est envoyé
    ...
    try {
      // DEBUG 2: Appel API direct
      console.log('Envoi de la requête...');
      const response = await api.post('/login', { email, password });
      
      // DEBUG 3: Réponse reçue
      ...
      if (result.success) {
        console.log('Redirection vers:', result.role);
      ...
      if (err.response?.status === 401) {
        ...
        setError('API non trouvée. Vérifiez que le serveur Laravel tourne (php artisan serve)');
