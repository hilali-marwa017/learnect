...
function Register() {
  ...
  if (success) {
    return (
      <div className="d-flex align-items-center justify-content-center" style={{ minHeight: '100vh', background: '#f5f5f5' }}>
        <div className="card p-4 text-center" style={{ maxWidth: '400px', borderRadius: '20px', border: 'none', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
          ...
          <p className="text-muted mb-4">Votre dossier est en cours de vérification</p>
  ...
  return (
    <div className="d-flex align-items-center justify-content-center" style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #F8FAFC 0%, #F1F5F9 100%)', padding: '24px' }}>
      <div className="card border-0 shadow-sm" style={{ maxWidth: '580px', width: '100%', borderRadius: '28px' }}>
        <div className="card-body p-4 p-md-5">
          
          {/* Header avec X à gauche */}
          <div className="d-flex justify-content-between align-items-center mb-4">
            <Link to="/" className="text-muted text-decoration-none" style={{ width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '50%', transition: 'background 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = '#f1f5f9'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
              <i className="bi bi-x-lg fs-5"></i>
            </Link>
            {role === 'enseignant' && (
              <span className="text-muted small">Étape {step}/2</span>
            )}
          </div>
          ...
          {role === 'enseignant' && step === 1 && (
            <div className="mb-4">
              <div className="progress rounded-pill" style={{ height: '6px', background: '#e9ecef' }}>
                <div className="progress-bar bg-primary rounded-pill" style={{ width: '50%' }}></div>
              </div>
            </div>
          )}
          ...
          <div className="text-center mb-4">
            <div className="bg-primary bg-opacity-10 rounded-circle d-flex align-items-center justify-content-center mx-auto mb-3" style={{ width: '56px', height: '56px' }}>
              <i className={`bi ${role === 'enseignant' ? 'bi-briefcase fs-4 text-primary' : 'bi-mortarboard fs-4 text-primary'}`}></i>
            </div>
            ...
            {step === 1 && (
              <>
                <div className="mb-3">
                  ...
                  <div className="border border-dashed rounded-3 p-2 bg-light">
                    <input type="file" name="photo" className="form-control form-control-sm border-0 bg-transparent" accept="image/*" onChange={handleFile} />
                ...
                {role === 'etudiant' && (
                  <>
                    ...
                    <div className="mb-3">
                      ...
                      <input type="number" name="budget" className="form-control" min="0" value={form.budget} onChange={handleChange} placeholder="Ex: 150" />
                    </div>
                  </>
                )}
                    ...
                    <div className="mb-3">
                      ...
                      <div className="d-flex gap-3 flex-wrap">
                        <div className="form-check">
                          ...
                          <label className="form-check-label small">En ligne</label>
                        </div>
                        <div className="form-check">
                          ...
                          <label className="form-check-label small">À domicile</label>
                        </div>
                        <div className="form-check">
                          ...
                          <label className="form-check-label small">Déplacement</label>
                        </div>
                      </div>
                    </div>
                  </>
                ...
                <button type="submit" className="btn btn-primary w-100 py-2 fw-bold rounded-3 mt-2">
                  {role === 'enseignant' ? 'Étape suivante →' : (loading ? 'Envoi en cours...' : 'Créer mon compte')}
                </button>
              </>
            )}
            ...
            {role === 'enseignant' && step === 2 && (
              <>
                <div className="bg-light rounded-3 p-3 mb-3">
                  <p className="fw-semibold mb-3 small text-primary"><i className="bi bi-shield-check me-2"></i>DOCUMENTS REQUIS</p>
                  <div className="mb-3">
                    <label className="form-label small fw-semibold">CIN Recto</label>
                    <input type="file" name="cin_recto" className="form-control" accept="image/*" onChange={handleFile} required />
                  </div>
                  <div className="mb-3">
                    <label className="form-label small fw-semibold">CIN Verso</label>
                    <input type="file" name="cin_verso" className="form-control" accept="image/*" onChange={handleFile} required />
                  </div>
                  <div>
                    <label className="form-label small fw-semibold">Diplôme (PDF)</label>
                    <input type="file" name="diplome" className="form-control" accept=".pdf" onChange={handleFile} required />
                  </div>
                </div>
                ...
                <div className="d-flex gap-3">
                  <button type="button" className="btn btn-outline-secondary flex-grow-1 py-2 rounded-3" onClick={() => { setStep(1); setError(''); window.scrollTo(0, 0); }}>
                    ← Retour
                  </button>
                  <button type="submit" className="btn btn-primary flex-grow-2 py-2 rounded-3" disabled={loading}>
