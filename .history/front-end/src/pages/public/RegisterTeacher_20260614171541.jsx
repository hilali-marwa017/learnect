...
import { 
  ...
  Upload, Camera, File, Shield, Eye, EyeOff, X
} from 'lucide-react';
...
export default function RegisterTeacher() {
  ...
  // Style - fond blanc par défaut
  const orange = '#e04f00';
  const text = '#07090d';
  const muted = '#718096';
  const border = 'rgba(0,0,0,0.10)';
  const card = '#f8f9fc';
  const bgInput = '#f4f4f6';
  ...
  const iconStyle = { position: 'absolute', left: '14px', top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none', color: '#9ca3af' };
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f0f0f2' }}>
        <div style={{ background: '#fff', borderRadius: '24px', padding: '2rem', textAlign: 'center', maxWidth: '400px' }}>
          ...
          <p style={{ color: '#718096', marginBottom: '1.5rem' }}>Votre dossier est en cours de vérification par notre équipe.</p>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#ffffff', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      <div style={{ width: '100%', maxWidth: '560px', background: '#fff', borderRadius: '24px', overflow: 'hidden' }}>

        {/* Header avec X pour revenir à l'accueil */}
        <div style={{ padding: '1.5rem 2rem 0.5rem 2rem', display: 'flex', justifyContent: 'flex-end' }}>
          <button 
            onClick={() => navigate('/')} 
            style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px', borderRadius: '8px', color: '#9ca3af' }}
              ...
              <>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                  ...
                  <div>
                    ...
                    <div style={{ position: 'relative' }}>
                      ...
                      <button type="button" onClick={() => setShowPass2(!showPass2)} style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#9ca3af' }}>
                ...
                <div style={{ marginBottom: '1.5rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 14px', background: card, border: `1px solid ${border}`, borderRadius: '10px', cursor: 'pointer', fontSize: '0.8rem' }}>
                ...
                <div style={{ marginBottom: '1rem' }}>
                  ...
                  <textarea name="description_profil" rows="3" placeholder="Présentez votre parcours, votre expérience et votre passion pour l'enseignement..." value={form.description_profil} onChange={handleChange} style={{ ...inputStyle, padding: '12px', resize: 'none' }} required />
                </div>

                <div style={{ marginBottom: '1.5rem' }}>
                  <label style={labelStyle}>DESCRIPTION DE VOTRE COURS</label>
                  <textarea name="description_cours" rows="3" placeholder="Décrivez votre méthode pédagogique, les matières enseignées, et comment vous aidez vos élèves à progresser..." value={form.description_cours} onChange={handleChange} style={{ ...inputStyle, padding: '12px', resize: 'none' }} />
                </div>
              ...
              <>
                ...
                <div style={{ marginBottom: '1.5rem' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
                    ...
                    <span style={{ fontSize: '0.85rem', color: text }}>Je certifie que les informations fournies sont exactes et vérifiables</span>
                  </label>
                </div>
