...
export default function Login() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: bgPage, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      <div style={{ width: '100%', maxWidth: '900px', background: bgCard, border: `1px solid ${borderColor}`, borderRadius: '20px', overflow: 'hidden', display: 'grid', gridTemplateColumns: '1fr 1fr', boxShadow: isDark ? '0 4px 32px rgba(0,0,0,0.3)' : '0 4px 32px rgba(0,0,0,0.08)' }}>

        {/* PANNEAU GAUCHE */}
        <div style={{ background: bgLeft, padding: '3rem 2.5rem', display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', gap: '2rem', borderRight: `1px solid ${leftBorder}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <div style={{ width: '40px', height: '40px', background: orange, borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ padding: '3rem 2.5rem', display: 'flex', flexDirection: 'column', justifyContent: 'center', background: bgRight }}>
          ...
          <div style={{ marginBottom: '1.2rem' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="votre@email.com" style={{ ...inputStyle, paddingLeft: '40px' }} />
            </div>
          </div>
          ...
          <div style={{ marginBottom: '1.8rem' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" style={{ ...inputStyle, paddingLeft: '40px', paddingRight: '44px' }} />
              <button type="button" onClick={() => setShowPassword(!showPassword)} style={{ position: 'absolute', right: '14px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: showPassword ? orange : textRightMuted, padding: 0, display: 'flex', alignItems: 'center' }}>
          ...
          <button
            ...
            disabled={loading}
            style={{ width: '100%', background: loading ? orange : btnBg, color: '#fff', border: 'none', borderRadius: '10px', padding: '14px', fontWeight: 700, fontSize: '0.82rem', letterSpacing: '0.08em', textTransform: 'uppercase', cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', marginBottom: '1.5rem', transition: 'background 0.2s' }}
          >
            {loading ? (
              <>
                <span style={{ width: '18px', height: '18px', border: '2px solid rgba(255,255,255,0.4)', borderTopColor: '#ffffff', borderRadius: '50%', display: 'inline-block', animation: 'spin 0.8s linear infinite', flexShrink: 0 }} />
                Connexion en cours...
              </>
            ) : (
              <>
                <ArrowRight size={16} />
                Ouvrir ma session sécurisée
              </>
            )}
          </button>
      ...
      <style>{`
        ...
        @media (max-width: 640px) {
          ...
          div[style*="padding: 3rem 2.5rem"]:first-child { display: none !important; }
        }
      `}</style>
    </div>
  );
