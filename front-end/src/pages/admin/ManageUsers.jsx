...
export function VerificationsTab({ enseignantsAttente, setEnseignantsAttente, setUsers }) {
    ...
    async function valider(id) {
        ...
        try {
            ...
            // correction : synchroniser la liste "Comptes Utilisateurs" pour
            // que le statut "actif" s'affiche immediatement, sans F5
            setUsers(prev => prev.map(u =>
                u.utilisateur_id === id ? { ...u, statut: 'actif' } : u
            ));
        } catch(e) {
            alert('Erreur');
        }
    ...
    async function refuser(id) {
        ...
        try {
            ...
            setEnseignantsAttente(prev => prev.filter(e => e.utilisateur_id !== id));
            // correction : meme synchronisation en cas de refus
            setUsers(prev => prev.map(u =>
                u.utilisateur_id === id ? { ...u, statut: 'refuse' } : u
            ));
            setRefusModal(null);
            setRefusRaison('');
        } catch(e) {
