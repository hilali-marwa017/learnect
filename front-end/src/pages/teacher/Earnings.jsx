    ...
    <div style={{ minHeight: '100vh', background: bg, paddingTop: '2rem', paddingBottom: '4rem' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1.5rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        <div style={{ borderBottom: '1px solid ' + border, paddingBottom: '1.5rem' }}>
          ...
          <p style={{ fontSize: '0.8rem', color: muted, margin: 0 }}>Consultez vos gains par cours effectué.</p>
        </div>
        ...
        <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
              {soldeCard('Solde disponible', `${parseFloat(totalRevenu).toFixed(2)} MAD`, <><Check size={12} /> Prêt pour virement</>, '#4ade80')}
              {soldeCard('Cumul en cours', `${enAttente.toFixed(2)} MAD`, <><Clock size={12} /> En attente de validation</>, '#fbbf24')}
              {soldeCard('Total paiements', paiements.length, <><Download size={12} /> Depuis l'origine</>, '#60a5fa')}
            </div>
                  ...
                  {paiements.map((p, i) => (
                    <div key={p.id_paiement} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', padding: '14px 12px', borderRadius: '12px', borderBottom: i < paiements.length - 1 ? '1px solid ' + borderSub : 'none', flexWrap: 'wrap' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                        ...
                        <div style={{ fontSize: '0.75rem', color: muted }}>Étudiant : <span style={{ color: text, fontWeight: 700 }}>{p.reservation?.etudiant?.prenom} {p.reservation?.etudiant?.nom}</span></div>
                        <div style={{ fontSize: '0.7rem', color: muted, fontFamily: 'monospace' }}>Méthode : {p.methode} — {p.created_at?.slice(0, 10)}</div>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexShrink: 0 }}>
                        <div style={{ textAlign: 'right' }}>
                          ...
                          {p.statut === 'paye' ? (
                            ...
                            <div style={{ fontSize: '0.65rem', color: '#fbbf24', fontWeight: 700, fontFamily: 'monospace', display: 'flex', alignItems: 'center', gap: '4px', justifyContent: 'flex-end' }}>
